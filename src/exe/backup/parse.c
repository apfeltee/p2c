...
void free_stmt(register Stmt* sp)
{
    if(sp)
    {
        free_stmt(sp->stm1);
...
Expr* replaceexprexpr(Expr* ex, Expr* oldex, Expr* newex, int keeptype)
{
    ...
    if(exprsame(ex, oldex, 2))
    {
        ...
        else if(oldex->val.type->kind == TK_POINTER && oldex->val.type->basetype == ex->val.type)
        {
            freeexpr(ex);
        ...
        *spp = sp;
        ...
        switch(sp->kind)
        {
            ...
            case SK_IF:
                ...
                else if(checkconst(sp->exp1, 1) && (elimdeadcode > 1 || sp->quietelim))
                {
                    if(!sp->quietelim)
            ...
            case SK_CASE:
                ...
                else if(sp->stm1->kind != SK_CASELABEL)
                { /* default only */
...
Static void skipunitheader(void)
{
...
Static void p_nested_module(void)
{
    Meaning* mp;
