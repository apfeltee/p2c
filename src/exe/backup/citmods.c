...
Static Stmt* proc_na_fillbyte(Expr* ex)
{
    ...
    return makestmt_call(makeexpr_bicall_3("memset", tp_void, ex->args[0], makeexpr_arglong(ex->args[1], 0),
...
Static Stmt* proc_na_move(Expr* ex)
{
    ...
    ex->args[2]
    = convert_size(choosetype(argbasetype(ex->args[0]), argbasetype(ex->args[1])), ex->args[2], "NA_MOVE");
...
Static Stmt* proc_na_exch(Expr* ex)
{
    ...
    ex->args[2]
    = convert_size(choosetype(argbasetype(ex->args[0]), argbasetype(ex->args[1])), ex->args[2], "NA_EXCH");
...
Static Expr* func_na_comp(Expr* ex)
{
    ...
    ex->args[1] = gentle_cast(ex->args[1], tp_anyptr);
    ex->args[2]
    = convert_size(choosetype(argbasetype(ex->args[0]), argbasetype(ex->args[1])), ex->args[2], "NA_COMP");
    return makeexpr_bicall_3("memcmp", tp_int, ex->args[0], ex->args[1],
                             makeexpr_arglong(ex->args[2], (size_t_long != 0)));
}
...
Static Stmt* proc_na_new(Expr* ex)
{
    ...
    {
        sp = makestmt_seq(
        sp, makestmt_if(makeexpr_rel(EK_EQ, copyexpr(vex), makeexpr_nil()),
                        makestmt_call(makeexpr_bicall_1(name_ESCAPE, tp_int, makeexpr_long(-2))), NULL));
    }
...
Static Stmt* proc_na_alloc(Expr* ex)
{
    ...
    ex2 = ex->args[0];
    if(ex2->val.type->kind == TK_POINTER && ex2->val.type->basetype->kind == TK_POINTER)
        ex->args[1] = convert_size(ex2->val.type->basetype->basetype, ex->args[1], "NA_ALLOC");
    return makestmt_call(ex);
}
...
Static Expr* func_na_bic(Expr* ex)
{
    ...
    ex0 = makeexpr_unlongcast(ex->args[0]);
    ex1 = makeexpr_unlongcast(ex->args[1]);
    return makeexpr_bin(EK_BAND, tp_integer, ex0, makeexpr_un(EK_BNOT, ex1->val.type, ex1));
}
...
Static Expr* func_na_test(Expr* ex)
{
    ...
    return makeexpr_rel(EK_NE,
                        makeexpr_bin(EK_BAND, tp_integer, ex1,
                                     makeexpr_bin(EK_LSH, tp_integer, makeexpr_longcast(makeexpr_long(1), longness),
...
Static Stmt* proc_na_set(Expr* ex)
{
    ...
    sp = makestmt_seq(
    sp, makestmt_assign(vex, makeexpr_bin(EK_BOR, tp_integer, copyexpr(vex),
                                          makeexpr_bin(EK_LSH, tp_integer, makeexpr_longcast(makeexpr_long(1), 1),
                                                       makeexpr_unlongcast(ex->args[0])))));
    return sp;
}
...
Static Stmt* proc_na_clear(Expr* ex)
{
    ...
    sp = makestmt_seq(
    sp, makestmt_assign(
        vex, makeexpr_bin(EK_BAND, tp_integer, copyexpr(vex),
...
Static Expr* func_na_po2(Expr* ex)
{
    if(!check0_31(ex->args[0]))
        return ex;
    return makeexpr_bin(EK_LSH, tp_integer, makeexpr_longcast(makeexpr_long(1), 1),
...
Static Expr* func_na_hibits(Expr* ex)
{
    if(!check0_31(ex->args[0]))
        return ex;
    return makeexpr_bin(EK_LSH, tp_integer, makeexpr_longcast(makeexpr_long(-1), 1),
                        makeexpr_minus(makeexpr_long(32), makeexpr_unlongcast(grabarg(ex, 0))));
}
...
Static Expr* func_na_asl(Expr* ex)
{
    ...
    if(expr_is_neg(ex2))
    {
        ...
        return makeexpr_bin(EK_RSH, tp_integer, grabarg(ex, 1), makeexpr_neg(ex2));
    }
    else
    {
...
Static Expr* func_na_lsl(Expr* ex)
{
    ...
    if(expr_is_neg(ex2))
    {
        ...
        return makeexpr_bin(EK_RSH, tp_integer, force_unsigned(grabarg(ex, 1)), makeexpr_neg(ex2));
    }
    else
    {
        if(possiblesigns(ex2) & 1)
...
{
    return makeexpr_bicall_2("P_imin2", tp_integer, ex->args[0], ex->args[1]);
}

Static Expr* func_imax(Expr* ex)
{
    return makeexpr_bicall_2("P_imax2", tp_integer, ex->args[0], ex->args[1]);
}
...
{
    return makeexpr_plus(makeexpr_unlongcast(ex->args[0]), makeexpr_unlongcast(ex->args[1]));
}

Static Expr* func_na_sub(Expr* ex)
{
    return makeexpr_minus(makeexpr_unlongcast(ex->args[0]), makeexpr_unlongcast(ex->args[1]));
}

extern Stmt* proc_exit(); /* from funcs.c */

Static Stmt* proc_return(void)
{
    return proc_exit();
}
...
{
    return makeexpr_bicall_1("tolower", tp_char, grabarg(ex, 0));
}
...
Static Expr* func_strint(Expr* ex)
{
    ...
    return makeexpr_bicall_3("sprintf", ex->val.type, ex->args[0],
                             makeexpr_string((exprlongness(ex2) > 0) ? "%ld" : "%d"), ex2);
}
...
Static Expr* func_strint2(Expr* ex)
{
    ...
    ex = makeexpr_bicall_4("sprintf", ex->val.type, ex->args[0], fmt, len, ex2);
...
Static Expr* func_strhex(Expr* ex)
{
    ...
    if(isliteralconst(ex->args[2], &val) == 2)
    {
        ...
        if(val.i < 1 || val.i > 8)
        {
            ex = makeexpr_bicall_3("sprintf", ex->val.type, ex->args[0],
                                   makeexpr_string((exprlongness(ex2) > 0) ? "%lX" : "%X"), ex2);
        }
        else
        {
            if(val.i < 8)
            ...
            "sprintf", ex->val.type, ex->args[0],
            makeexpr_string(format_d((exprlongness(ex2) > 0) ? "%%.%ldlX" : "%%.%ldX", val.i)), ex2);
        }
    }
    return ex;
}
...
{
    return makeexpr_bicall_3("sprintf", ex->val.type, ex->args[0], makeexpr_string("%g"), ex->args[1]);
}

Static Expr* func_strchar(Expr* ex)
{
    return makeexpr_bicall_3("sprintf", ex->val.type, ex->args[0], makeexpr_string("%c"), ex->args[1]);
}

Static Expr* func_strreadint(Expr* ex)
{
    return makeexpr_bicall_3("strtol", tp_integer, grabarg(ex, 0), makeexpr_nil(), makeexpr_long(0));
}

Static Expr* func_strreadreal(Expr* ex)
{
    return makeexpr_bicall_1("atof", tp_longreal, grabarg(ex, 0));
}
...
Static Expr* func_strbegins(Expr* ex)
{
    ...
    if(ex2->kind == EK_CONST)
    {
        if(ex2->val.i == 1)
        {
            return makeexpr_rel(EK_EQ, makeexpr_hat(ex1, 0), makeexpr_char(ex2->val.s[0]));
        }
        else
        {
            return makeexpr_rel(
            EK_EQ,
            makeexpr_bicall_3("strncmp", tp_int, ex1, ex2,
...
{
    return makeexpr_rel(EK_NE, makeexpr_bicall_2("strpbrk", tp_strptr, ex->args[0], ex->args[1]),
                        makeexpr_nil());
}
...
Static Expr* func_strequal(Expr* ex)
{
    if(!*strcicmpname)
        return ex;
    return makeexpr_rel(EK_EQ, makeexpr_bicall_2(strcicmpname, tp_int, ex->args[0], ex->args[1]),
                        makeexpr_long(0));
}
...
{
    return makeexpr_bicall_4("sprintf", ex->val.type, ex->args[0], makeexpr_string("%-*s"),
                             makeexpr_longcast(ex->args[2], 0), ex->args[1]);
}
...
Static Stmt* proc_strnew(Expr* ex)
{
    return makestmt_assign(makeexpr_hat(ex->args[0], 0),
                           makeexpr_bicall_1("strdup", ex->args[1]->val.type, ex->args[1]));
}
...
Static Stmt* proc_strlist_add(Expr* ex)
{
    ...
    makeexpr_hat(ex->args[1], 0),
    makeexpr_bicall_2("strlist_add", ex->args[0]->val.type->basetype, ex->args[0], ex->args[2]));
}
...
Static Stmt* proc_strlist_append(Expr* ex)
{
    return makestmt_assign(
    makeexpr_hat(ex->args[1], 0),
    makeexpr_bicall_2("strlist_append", ex->args[0]->val.type->basetype, ex->args[0], ex->args[2]));
}
...
Static Stmt* proc_strlist_insert(Expr* ex)
{
    return makestmt_assign(
    makeexpr_hat(ex->args[1], 0),
    makeexpr_bicall_2("strlist_insert", ex->args[0]->val.type->basetype, ex->args[0], ex->args[2]));
}
...
Static Stmt* proc_m_color(Expr* ex)
{
    ...
    {
        if(ex->val.i >= 16 && ex->val.i < 255)
        { /* accept true colors and m_trans */
            note("M_COLOR called with suspicious argument [508]");
        }
    }
    return makestmt_call(ex);
...
void citmods(char* name, int defn)
{
