...
Static void makePascalwords(void)
{
...
Static void recordsideeffects(void)
{
    deterministic("abs");
    deterministic("acos");
    deterministic("asin");
...
void setup_lex(void)
{
    ...
    if(!strcmp(language, "MODCAL"))
        ...
        sysprog_flag = 0;
    if(shortcircuit < 0)
        partial_eval_flag = (which_lang == LANG_TURBO || which_lang == LANG_VAX || which_lang == LANG_OREGON
                             || which_lang == LANG_TIP || modula2 || hpux_lang);
    else
...
void counterror(void)
{
    if(maxerrors > 0)
    {
        if(--maxerrors == 0)
        {
            ...
            if(verbose)
                fprintf(logfh, "Translation aborted: Too many errors.\n");
            closelogfile();
            exit_failure();
        }
    }
           ...
           char* msg)
{
    ...
    if(verbose)
    {
        fprintf(logfh, "%s:%d:%d: %s\n", infname, inf_lnum, outf_lnum, msg);
        fprintf(logfh, "Translation aborted.\n");
    }
    closelogfile();
    exit_failure();
}

void interror(/* does not return */
              char* proc,
              char* msg)
{
...
void warning(char* msg)
{
    ...
    {
        if(verbose)
            fprintf(logfh, "%s:%d:%d: Omitted warning: %s\n", infname, inf_lnum, outf_lnum, msg);
        return;
    }
    beginerror();
...
void intwarning(char* proc, char* msg)
{
    ...
    {
        if(verbose)
            fprintf(logfh, "%s:%d:%d: Omitted internal error in %s: %s\n", infname, inf_lnum,
                    outf_lnum, proc, msg);
        return;
    }
    beginerror();
...
void note(char* msg)
{
    ...
    {
        if(verbose)
            fprintf(logfh, "%s:%d:%d: Omitted note: %s\n", infname, inf_lnum, outf_lnum, msg);
        return;
    }
    beginerror();
...
void endnote(char* msg)
{
    ...
    {
        if(verbose)
            fprintf(logfh, "%s:%d:%d: Omitted end-note: %s\n", infname, inf_lnum, outf_lnum, msg);
        return;
    }
    if(verbose)
        fprintf(logfh, "%s:%d:%d: Recorded end-note: %s\n", infname, inf_lnum, outf_lnum, msg);
    (void)strlist_add(&endnotelist, msg);
}

void showendnotes(void)
{
    while(initialcalls)
    {
...
{
    while(curtok != tok1 && curtok != tok2 && curtok != TOK_END && curtok != TOK_RPAR
...
Symbol* findsymbol(char* name)
{
    ...
    while((sp = *prev) != 0 && (i = strcmp(sp->name, name)) != 0)
    {
        if(i < 0)
            prev = &(sp->left);
        else
...
void replacestrings(char* buf, Strlist* sl)
{
    Strlist* p = sl;
    while(p)
    {
...
void push_input_file(fp, fname, isinclude) FILE* fp;
char* fname;
int isinclude;
{
    push_input();
...
void push_input_strlist(Strlist* sp, char* fname)
{
    push_input();
    ...
    if(fname)
    {
        ...
        inf_lnum--; /* adjust for extra getlinep() */
    *inbuf = 0;
    inbufptr = inbuf;
    gettok();
}
...
int undooption(int i, char* name)
{
...
int handle_include(char* fn)
{
    ...
    else
    {
        ...
        if(verbose)
            fprintf(logfh, "Reading include file \"%s\"\n", fn);
        if(expandincludes == 0)
        {
...
int turbo_directive(char* closing, char* after)
{
    char *cp, *cp2;
    int i, result;
    ...
    switch(inbufptr[2])
    {
        ...
        case 'c':
        case 'C':
            if(toupper(inbufptr[1]) == 'S' && (inbufptr[3] == '+' || inbufptr[3] == '-') && inbufptr + 4 == closing)
            {
                if(shortcircuit < 0)
    ...
    switch(kind)
    {
        ...
        case MAC_VAR:
            ...
            for(mp = curtoksym->mbase; mp; mp = mp->snext)
            {
                if(mp->kind == MK_VAR)
                    warning(format_s(
                    "VarMacro must be defined before declaration of variable %s [231]", curtokcase));
            }
        ...
        case MAC_CONST:
            ...
            for(mp = curtoksym->mbase; mp; mp = mp->snext)
            {
                if(mp->kind == MK_CONST)
                    warning(format_s(
                    "ConstMacro must be defined before declaration of variable %s [232]", curtokcase));
            }
...
Static int parsecomment(int p2c_only, int starparen)
{
    ...
    if(!p2c_only)
    {
        ...
        if(!strncmp(inbufptr, fixedcomment, strlen(fixedcomment)) && *fixedcomment
        ...
        if(!strncmp(inbufptr, permanentcomment, strlen(permanentcomment)) && *permanentcomment
        ...
        if(!strncmp(inbufptr, interfacecomment, strlen(interfacecomment)) && *interfacecomment
        ...
        if(!strncmp(inbufptr, skipcomment, strlen(skipcomment)) && *skipcomment
        ...
        if(!strncmp(inbufptr, signedcomment, strlen(signedcomment)) && *signedcomment && !p2c_only
        ...
        if(!strncmp(inbufptr, unsignedcomment, strlen(unsignedcomment)) && *unsignedcomment
           && !p2c_only && inbufptr + strlen(unsignedcomment) == closing)
        {
            inbufptr = after;
            gettok();
            if(curtok == TOK_IDENT && curtokmeaning && curtokmeaning->kind == MK_TYPE
            ...
            else if(curtok == TOK_IDENT && curtokmeaning && curtokmeaning->kind == MK_TYPE
        ...
        if(!strncmp(inbufptr, tagcomment, strlen(tagcomment)) && *tagcomment
    ...
    *cp2 = 0;
    ...
    if(cp == closing)
    {
        if(kind == 'S' || kind == 'I' || kind == 'D' || kind == 'L' || kind == 'R' || kind == 'B'
           || kind == 'C' || kind == 'U')
        {
            undooption(i, "");
            inbufptr = after;
    ...
    switch(kind)
    {
        ...
        case 'L':
            val = oldval = (kind == 'L') ?
                           *((long*)rctable[i].ptr) :
                           (kind == 'S') ? *((short*)rctable[i].ptr) : *((int*)rctable[i].ptr);
            switch(*cp)
            {
                ...
                case '=':
                    ...
                    else if(toupper(cp[0]) == 'D' && toupper(cp[1]) == 'E' && toupper(cp[2]) == 'F')
                    {
                        val = rctable[i].def;
                        cp += 3;
                    }
        ...
        case 'D':
            ...
            if(*cp++ != '=')
                return 0;
            skipspc(cp);
            if(toupper(cp[0]) == 'D' && toupper(cp[1]) == 'E' && toupper(cp[2]) == 'F')
            {
                val = rctable[i].def;
                cp += 3;
            }
            ...
            if(*cp == '=')
            {
                ...
                *cp2 = 0;
                ...
                if(cp != closing)
                    return 0;
                inbufptr = after;
                if(!strcmp(rctable[i].name, "LANGUAGE") && !strcmp((char*)rctable[i].ptr, "MODCAL"))
                    sysprog_flag |= 2;
                return 1;
            }
            return 0;
        ...
        case 'X':
            switch(rctable[i].def)
            {
                ...
                case 4: /* strlist with string values */
                    ...
                    {
                        while(isalnum(*cp) || *cp == '_' || *cp == '$' || *cp == '%' || *cp == '.' || *cp == '-'
                              || (*cp == '\'' && cp[1] && cp[2] == '\'' && cp + 1 != closing && cp[1] != '='))
                        {
                            if(*cp == '\'')
                            {
                                *cp2++ = *cp++;
...
Static void comment(int starparen /* 0={ }, 1=(* *), 2=C comments, 3=" " */
)
{
    register char ch;
...
char* getinlinepart(void)
{
    ...
    cp = inbufptr;
    while(isspace(*cp) || isalnum(*cp) || *cp == '_' || *cp == '$' || *cp == '+' || *cp == '-'
          || *cp == '<' || *cp == '>')
        cp++;
    if(cp == inbufptr)
        return "";
            ...
            case '$':
                ...
                do
                {
                    ...
                    else if(curtok == TOK_IDENT)
                    {
                        ...
                        else if(ignore_directives || if_flag || !strcmp(curtokbuf, "SEARCH")
                                || !strcmp(curtokbuf, "REF") || !strcmp(curtokbuf, "DEF"))
                        {
                            skiptotoken(TOK_DOLLAR);
                        }
    ...
    /* Not safe---consider "var ch:char; begin ch:=#$ff end". */
            ...
            default:
                if(isdigit(ch))
                {
                    ...
                    if(ch == 'e' || ch == 'E' || ch == 'd' || ch == 'D' || ch == 'l' || ch == 'L'
                       || ch == 'q' || ch == 'Q')
                    {
                        ch = *inbufptr;
                        if(isdigit(ch) || ch == '+' || ch == '-')
                ...
                ident:
                {
                    ...
                    *cp++ = pascalcasesens ? ch : toupper(ch);
                    while(isalnum((ch = *inbufptr++)) || ch == '_' || (ch == '%' && !C_lex)
                          || (ch == '$' && dollar_idents))
                    {
                        *cp2++ = symcase ? ch : tolower(ch);
                    ...
                    *cp = 0;
                    ...
                    if(*curtokbuf == '%')
                    {
                        ...
                        else if((i = 2, !strcicmp(curtokbuf, "%B")) || (i = 8, !strcicmp(curtokbuf, "%O"))
                    ...
                    }
                    {
                        ...
                        if(sp->kwtok && !(sp->flags & KWPOSS) && (pascalcasesens != 2 || !islower(*curtokbuf))
...
void checkmodulewords(void)
{
    if(modula2)
    {
        ...
        checkkeyword(TOK_EXPORT);
            ...
            && (curtoksym->kwtok == TOK_MODULE || curtoksym->kwtok == TOK_IMPORT
                || curtoksym->kwtok == TOK_EXPORT || curtoksym->kwtok == TOK_IMPLEMENT))
    {
        if(!strcmp(curtokbuf, "UNIT") || !strcmp(curtokbuf, "USES")
           || !strcmp(curtokbuf, "INTERFACE") || !strcmp(curtokbuf, "IMPLEMENTATION"))
        {
            modulenotation = 0;
