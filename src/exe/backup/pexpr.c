...
Expr* dots_n_hats(Expr* ex, Type* target)
{
    Expr *ex2, *ex3;
    Type *tp, *tp2;
    Meaning *mp, *tvar;
    ...
    for(;;)
    {
        if((ex->val.type->kind == TK_PROCPTR || ex->val.type->kind == TK_CPROCPTR) && curtok != TOK_ASSIGN
           && ((mp = (tp2 = ex->val.type)->basetype->fbase) == NULL
               || (mp->isreturn && mp->xnext == NULL) || curtok == TOK_LPAR)
           && (tp2->basetype->basetype != tp_void || target == tp_void)
           && (!target || (target->kind != TK_PROCPTR && target->kind != TK_CPROCPTR)))
        {
            hassl = tp2->escale;
            ex2 = ex;
            ex3 = copyexpr(ex2);
            if(hassl != 0)
                ex3 = makeexpr_cast(makeexpr_dotq(ex3, "proc", tp_anyptr), makepointertype(tp2->basetype));
            ...
            else
            {
                ...
                ex = makeexpr_cond(makeexpr_rel(EK_NE, ex2, makeexpr_nil()), ex3, ex);
            }
            if(tp2->basetype->fbase && tp2->basetype->fbase->isreturn && tp2->basetype->fbase->kind == MK_VARPARAM)
                ex = makeexpr_hat(ex, 0); /* returns pointer to structured result */
            continue;
        }
        else if(ex->val.type->kind == TK_FUNCTION && ex->val.type->meaning
                && ex->val.type->meaning->kind == MK_FUNCTION && ex->val.type->meaning->rectype)
        {
            if(curtok == TOK_LPAR || !target || (target->kind != TK_PROCPTR && target->kind != TK_CPROCPTR))
            {
...
Expr* p_index(Expr* ex, Expr* ex2)
{
    ...
    else if(tp->kind == TK_ARRAY || tp->kind == TK_SMALLARRAY)
    {
        if(tp->smax)
        {
...
void var_reference(Meaning* mp)
{
    ...
    if(mp->ctx && mp->ctx->kind == MK_FUNCTION && mp->ctx->needvarstruct
       && (mp->kind == MK_VAR || mp->kind == MK_VARREF || mp->kind == MK_VARMAC
           || mp->kind == MK_PARAM || mp->kind == MK_VARPARAM
           || (mp->kind == MK_CONST && (mp->type->kind == TK_ARRAY || mp->type->kind == TK_RECORD))))
    {
        if(debug > 1)
        {
...
Expr* packset(Expr* ex, Type* type)
{
    ...
    {
        if(!strcmp(ex->val.s, setexpandname) && (mp = istempvar(ex->args[0])) != NULL)
        {
            canceltempvar(mp);
            return grabarg(ex, 1);
        }
        if(!strcmp(ex->val.s, setunionname) && (mp = istempvar(ex->args[0])) != NULL
           && !exproccurs(ex->args[1], ex->args[0]) && !exproccurs(ex->args[2], ex->args[0]))
        {
            canceltempvar(mp);
        ...
        if(!strcmp(ex->val.s, setaddrangename))
        {
            if(ord_range(type->indextype, NULL, &max2) && max2 == setbits - 1)
                note(
                "Range construction was implemented by a subtraction which may overflow [278]");
            ex2 = makeexpr_minus(
            makeexpr_bin(EK_LSH, type, makeexpr_longcast(makeexpr_long(1), 1),
                         makeexpr_plus(ex->args[2], makeexpr_long(1))),
            makeexpr_bin(EK_LSH, type, makeexpr_longcast(makeexpr_long(1), 1), ex->args[1]));
            ex = packset(ex->args[0], type);
            if(checkconst(ex, 0))
                return ex2;
            else
...
Expr* p_setfactor(Type* target, int sure)
{
    ...
    for(;;)
    {
        ...
        doneflag[num] = 0;
        if(curtok == TOK_DOTS || curtok == TOK_COLON)
        { /* UCSD? */
            val = eval_expr(ex);
            if(val.type)
            {
    ...
    if(smallsetconst > 0)
    {
        ...
        for(i = 0; i < num; i++)
        {
            if(first[i]->kind == EK_CONST && first[i]->val.i < setbits
               && (!last[i] || (last[i]->kind == EK_CONST && last[i]->val.i >= 0 && last[i]->val.i < setbits)))
            {
                if(last[i])
                {
    ...
    if(type->kind == TK_SMALLSET)
    {
        for(i = 0; i < num; i++)
        {
            if(!doneflag[i])
            {
                ex2 = makeexpr_bin(EK_LSH, type, makeexpr_longcast(makeexpr_long(1), 1),
                                   enum_to_int(first[i]));
                if(last[i])
                {
                    if(ord_range(type->indextype, NULL, &max2) && max2 == setbits - 1)
                        note("Range construction was implemented by a subtraction which may "
                             "overflow [278]");
                    ex2
                    = makeexpr_minus(makeexpr_bin(EK_LSH, type, makeexpr_longcast(makeexpr_long(1), 1),
                                                  makeexpr_plus(enum_to_int(last[i]), makeexpr_long(1))),
                                     ex2);
                }
                if(ex)
    ...
    else
    {
        ...
        for(i = 0; i < num; i++)
        {
            ...
            {
                if(last[i])
                    ex = makeexpr_bicall_3(setaddrangename, type, ex,
                                           makeexpr_arglong(enum_to_int(first[i]), 0),
                                           makeexpr_arglong(enum_to_int(last[i]), 0));
                else
                    ex = makeexpr_bicall_2(setaddname, type, ex, makeexpr_arglong(enum_to_int(first[i]), 0));
            }
        }
    }
    return ex;
...
Expr* p_funcarglist(Expr* ex, Meaning* args, int firstarg, int ismacro)
{
    ...
    while(args)
    {
            ...
            if(curtok == TOK_RPAR)
            {
                ...
                while(args)
                {
                    ...
                    if(!ex->args[i])
                    {
                        ...
                        {
                            warning(format_s("Missing value for parameter %s [291]", args->name));
        ...
        if(curtok == TOK_IDENT)
        {
            ...
            if(mp && (peeknextchar() == 1 || !curtokmeaning || isnonpos))
            {
                gettok();
                wneedtok(TOK_ASSIGN);
                prevarg = mp2;
        ...
        while(ex->nargs <= i)
            insertarg(&ex, ex->nargs, NULL);
        if(ex->args[i])
            warning(format_s("Multiple values for parameter %s [294]", args->name));
        tp = args->type;
        ex2 = p_expr(tp);
        ...
        if(isfiletype(tp, 1) && is_std_file(ex2))
        {
            ...
            ex2 = makeexpr_comma(
            makeexpr_comma(makeexpr_assign(filebasename(makeexpr_var(mp2)), ex2),
                           makeexpr_assign(filenamepart(makeexpr_var(mp2)), makeexpr_string(""))),
            makeexpr_var(mp2));
        }
        tp2 = ex2->val.type;
        isconf = ((tp->kind == TK_ARRAY || tp->kind == TK_STRING) && tp->structdefd);
        switch(args->kind)
        {
            ...
            case MK_PARAM:
                if(args->isref)
                {
                    if(tp2 != tp && !isconf && (tp2->kind != TK_STRING || tp->kind != TK_STRING))
                        warning(format_s("Type mismatch in VAR parameter %s [295]", args->name));
                }
                else if(castit && tp->kind == TK_REAL && ex2->val.type->kind != TK_REAL)
                    ex2 = makeexpr_cast(ex2, tp);
                else if(ord_type(tp)->kind == TK_INTEGER && !ismacro)
                    ex2 = makeexpr_arglong(ex2, long_type(tp));
                else if(args->othername && args->rectype != tp && tp->kind != TK_STRING && args->type == tp2)
            ...
            case MK_VARPARAM:
                ...
                { /* regular VAR parameter */
                    if(!expr_is_lvalue(ex2) || (tp->kind == TK_REAL && ord_type(tp2)->kind == TK_INTEGER))
                    {
                        mp2 = makestmttempvar(tp, name_TEMP);
                        ex2 = makeexpr_comma(makeexpr_assign(makeexpr_var(mp2), ex2),
                                             makeexpr_addrf(makeexpr_var(mp2)));
                    }
                    else
                        ex2 = makeexpr_addrf(ex2);
                    if(args->anyvarflag
                       || (tp->kind == TK_POINTER && tp2->kind == TK_POINTER && (tp == tp_anyptr || tp2 == tp_anyptr)))
                    {
                        if(!ismacro)
            ...
            default:
                intwarning("p_funcarglist",
                           format_s("Parameter type is %s [296]", meaningkindname(args->kind)));
                break;
        }
           ...
           (!prevarg || prevarg->type != args->type))
        {
            while(tp->kind == TK_ARRAY && tp->structdefd)
            {
                if(tp2->kind == TK_SMALLARRAY)
                {
                    ...
                    ex->args[ex->nargs - 1] = makeexpr_addr(ex->args[ex->nargs - 1]);
                }
                else if(tp2->kind == TK_STRING)
                {
                    ex->args[fakenum++] = makeexpr_arglong(makeexpr_long(1), integer16 == 0);
                    ex->args[fakenum++]
                    = makeexpr_arglong(strmax_func(ex->args[ex->nargs - 1]), integer16 == 0);
                    break;
                }
                ...
                ex->args[fakenum++]
                = makeexpr_arglong(expr_reference(copyexpr(tp2->indextype->smin)), integer16 == 0);
                ex->args[fakenum++]
                = makeexpr_arglong(expr_reference(copyexpr(tp2->indextype->smax)), integer16 == 0);
                tp = tp->basetype;
                tp2 = tp2->basetype;
            }
            if(tp->kind == TK_STRING && tp->structdefd)
            {
                ex->args[fakenum] = makeexpr_arglong(strmax_func(ex->args[ex->nargs - 1]), integer16 == 0);
            }
        }
        fakenum = -1;
        if(!isnonpos)
...
Expr* replacemacargs(Expr* ex, Expr* fex)
{
    int i;
    Expr* ex2;
...
Expr* p_noarglist(Expr* ex, Meaning* mp, Meaning* args)
{
...
void func_reference(Meaning* func)
{
    Meaning* mp;
...
Expr* p_memcall(Expr* expr, Meaning* mp)
{
    Meaning *tvar, *mp2;
    Expr* ex;
    int firstarg = 0;
...
Expr* accumulate_strlit(void)
{
    ...
    if(danger && !(unsignedchar == 1 || (unsignedchar != 0 && signedchars == 0)))
        note(format_s("Character%s >= 128 encountered [281]", (danger > 1) ? "s" : ""));
    return makeexpr_lstring(buf, len);
}

Expr* pascaltypecast(Type* type, Expr* ex2)
{
    if(type->kind == TK_POINTER || type->kind == TK_STRING || type->kind == TK_ARRAY)
        ex2 = makeexpr_stringcast(ex2);
    else
        ex2 = makeexpr_charcast(ex2);
    if((ex2->val.type->kind == TK_INTEGER || ex2->val.type->kind == TK_CHAR || ex2->val.type->kind == TK_BOOLEAN
        || ex2->val.type->kind == TK_ENUM || ex2->val.type->kind == TK_SUBR || ex2->val.type->kind == TK_REAL
        || ex2->val.type->kind == TK_POINTER || ex2->val.type->kind == TK_STRING)
       && (type->kind == TK_INTEGER || type->kind == TK_CHAR || type->kind == TK_BOOLEAN || type->kind == TK_ENUM
           || type->kind == TK_SUBR || type->kind == TK_REAL || type->kind == TK_POINTER))
    {
    ...
    {
        return makeexpr_hat(makeexpr_cast(makeexpr_addr(ex2), makepointertype(type)), 0);
    }
}
    ...
    switch(curtok)
    {
        ...
        case TOK_ADDR:
            ...
            if(curtok == TOK_IDENT && 0 && /***/
               curtokmeaning && (curtokmeaning->kind == MK_FUNCTION || curtokmeaning->kind == MK_SPECIAL))
            {
        ...
        case TOK_LBR:
        case TOK_LBRACE:
            return p_setfactor(target && target->kind == TK_SET ? target->indextype : NULL, 0);
        ...
        case TOK_INHERITED:
            ...
            if(type)
            {
                if(mp2->kind == MK_FUNCTION)
                    ...
                    ex = makeexpr_name(format_ss("%s::%s", type->meaning->name, mp2->name), mp2->type);
            }
            else
            {
                warning(format_s("No field called %s in that object [332]", curtokbuf));
            ...
            switch((mp) ? mp->kind : MK_VAR)
            {
                ...
                case MK_TYPE:
                    ...
                    switch(curtok)
                    {
                        ...
                        case TOK_LPAR: /* Turbo type cast */
                            gettok();
                            if(type->kind == TK_RECORD && type->issigned && !turboobjects)
                        ...
                        case TOK_DOT:
                            ...
                            if(type->kind == TK_RECORD)
                            {
                                ...
                                if(type)
                                {
                                    if(mp2->kind == MK_FUNCTION)
                                        ...
                                        ex = makeexpr_name(
                                        format_ss("%s::%s", type->meaning->name, mp2->name), mp2->type);
                                }
                                else
                                {
                                    warning(format_s("No field called %s in that object [332]", curtokbuf));
                            ...
                            {
                                warning(format_s("%s is not an object type", type->meaning->name));
                                return p_variable(target);
                            }
                ...
                case MK_SPECIAL:
                    if(mp->handler && mp->isfunction
                       && (peeknextchar() == '(' || !target || mp == mp_self_func
                           || (target->kind != TK_PROCPTR && target->kind != TK_CPROCPTR)))
                    {
                        gettok();
                    ...
                    {
                        if(target && (target->kind == TK_PROCPTR || target->kind == TK_CPROCPTR))
                ...
                case MK_FUNCTION:
                    ...
                    else
                    {
                        ...
                        if(target && target->kind == TK_CPROCPTR)
                        {
                            ...
                            ex = makeexpr_comma(makeexpr_assign(makeexpr_var(mp2), makeexpr_name(mp->name, tp_text)),
                                                makeexpr_var(mp2));
                        ...
                        else
                        {
                            ...
                            mp2 = makestmttempvar(type, name_TEMP);
                            ex = makeexpr_comma(
                            makeexpr_comma(makeexpr_assign(makeexpr_dotq(makeexpr_var(mp2), "proc", tp_anyptr),
                                                           makeexpr_name(mp->name, tp_text)),
                                           /* handy pointer type */
                                           makeexpr_assign(makeexpr_dotq(makeexpr_var(mp2), "link", tp_anyptr),
                                                           makeexpr_ctx(mp->ctx))),
                            makeexpr_var(mp2));
                        }
                        return ex;
...
Static Expr* p_powterm(Type* target)
{
    ...
    if(curtok == TOK_STARSTAR)
    {
        ...
        if(ex->val.type->kind == TK_REAL || ex2->val.type->kind == TK_REAL)
        {
            if(checkconst(ex2, 2))
            {
        ...
        {
            freeexpr(ex);
            ex = makeexpr_bin(EK_LSH, tp_integer, makeexpr_longcast(makeexpr_long(1), 1), ex2);
        }
        else if(checkconst(ex, 0) || checkconst(ex, 1) || checkconst(ex2, 1))
        {
            freeexpr(ex2);
        }
        ...
        else if(isliteralconst(ex, NULL) == 2 && isliteralconst(ex2, NULL) == 2 && ex2->val.i > 0)
        {
            v = ex->val.i;
            i = ex2->val.i;
            while(--i > 0)
        ...
        {
            ex = makeexpr_bicall_2("ipow", tp_integer, makeexpr_arglong(ex, 1), makeexpr_arglong(ex2, 1));
        }
    }
    return ex;
}
        ...
        switch(curtok)
        {
            ...
            case TOK_STAR:
                ...
                if(ex->val.type->kind == TK_SET || ex->val.type->kind == TK_SMALLSET)
                {
                    ...
                    {
                        tvar = makestmttempvar(type, name_SET);
                        ex = makeexpr_bicall_3(setintname, type, makeexpr_var(tvar), ex, ex2);
                    }
                }
                else
            ...
            case TOK_AMP:
                ...
                else if(partial_eval_flag || useshort || (shortopt && nosideeffects(ex2, 1)))
...
Expr* p_sexpr(Type* target)
{
    ...
    if(curtok == TOK_PLUS
       && (ex->val.type->kind == TK_STRING || ord_type(ex->val.type)->kind == TK_CHAR
            ...
            switch(curtok)
            {
                ...
                case TOK_PLUS:
                    ...
                    if(ex->val.type->kind == TK_SET || ex->val.type->kind == TK_SMALLSET)
                    {
                        ...
                        {
                            tvar = makestmttempvar(type, name_SET);
                            ex = makeexpr_bicall_3(setunionname, type, makeexpr_var(tvar), ex, ex2);
                        }
                    }
                    else
                ...
                case TOK_MINUS:
                    ...
                    if(ex->val.type->kind == TK_SET || ex->val.type->kind == TK_SMALLSET)
                    {
                        ...
                        if(type->kind == TK_SMALLSET)
                        {
                            ex = makeexpr_bin(EK_BAND, type, ex, makeexpr_un(EK_BNOT, type, ex2));
                        }
                        else
                        {
                            tvar = makestmttempvar(type, name_SET);
                            ex = makeexpr_bicall_3(setdiffname, type, makeexpr_var(tvar), ex, ex2);
                        }
                    }
                    else
        ...
        case TOK_IN:
            ...
            if(ex2->val.type->kind == TK_SMALLSET)
            {
                ...
                if(ex->kind == EK_CONST && smallsetconst)
                    ...
                    ex = makeexpr_bin(EK_LSH, ex2->val.type, makeexpr_longcast(makeexpr_long(1), 1),
                                      enum_to_int(ex));
                ...
                {
                    ex = makeexpr_and(
                    makeexpr_range(enum_to_int(ex4), makeexpr_long(0), makeexpr_setbits(), 0), ex);
                }
                else
                    freeexpr(ex4);
                ex = makeexpr_comma(ex3, ex);
            ...
            else
            {
                ...
                if(ex3->kind == EK_BICALL && !strcmp(ex3->val.s, setexpandname)
                   && (tvar = istempvar(ex3->args[0])) != NULL && isconstexpr(ex3->args[1], &mask))
                {
                    canceltempvar(tvar);
                    if(!nosideeffects(ex, 0))
                    {
                        ...
                        if(mask & (1L << i++))
                        {
                            if(i + 1 < setbits && (mask & (2L << i)))
                            {
                                for(j = i; j < setbits && (mask & (1L << j)); j++)
                                    ;
                                ex4 = makeexpr_or(
                                ex4, makeexpr_range(copyexpr(ex), makeexpr_val(make_ord(type, i - 1)),
                                                    makeexpr_val(make_ord(type, j - 1)), 1));
                                i = j;
                            }
                            else
                            {
                                ex4 = makeexpr_or(ex4, makeexpr_rel(EK_EQ, copyexpr(ex),
                    ...
                    for(;;)
                    {
                        if(!strcmp(ex2->val.s, setaddrangename))
                        {
                            ...
                            {
                                ex4 = makeexpr_or(
                                ex4, makeexpr_range(copyexpr(ex), ex2->args[1], ex2->args[2], 1));
                            }
                        }
                    ...
                    if(mask & 0x1)
                        ex4 = makeexpr_or(ex4, makeexpr_range(copyexpr(ex), makeexpr_char('a'),
                                                              makeexpr_char('z'), 1));
                    if(mask & 0x2)
                        ex4 = makeexpr_or(ex4, makeexpr_range(copyexpr(ex), makeexpr_char('A'),
                                                              makeexpr_char('Z'), 1));
                    if(mask & 0x4)
                        ex4 = makeexpr_or(ex4, makeexpr_range(copyexpr(ex), makeexpr_char('0'),
                                                              makeexpr_char('9'), 1));
                    freeexpr(ex);
...
{
    if(!strcicmp(name, "malloc") || !strcicmp(name, mallocname))
    {
        return tp_anyptr;
    }
    return tp_integer;
...
int istypespec(int iscast)
{
    ...
    switch(curtok)
    {
        ...
        case TOK_IDENT:
            ...
            return !strcmp(curtokcase, "volatile") || !strcmp(curtokcase, "void")
                   || !strcmp(curtokcase, "char") || !strcmp(curtokcase, "short")
                   || !strcmp(curtokcase, "int") || !strcmp(curtokcase, "long")
                   || !strcmp(curtokcase, "float") || !strcmp(curtokcase, "double")
                   || !strcmp(curtokcase, "signed") || !strcmp(curtokcase, "unsigned")
                   || !strcmp(curtokcase, "struct") || !strcmp(curtokcase, "union")
                   || !strcmp(curtokcase, "class") || !strcmp(curtokcase, "enum")
                   || !strcmp(curtokcase, "typedef") || (curtokmeaning && curtokmeaning->kind == MK_TYPE);
    ...
    for(;;)
    {
        switch(curtok)
        {
            ...
            case TOK_ARROW:
                ...
                if(ex->val.type->kind != TK_POINTER)
                    ex->val.type = makepointertype(ex->val.type);
                ex = makeexpr_dotq(makeexpr_hat(ex, 0), curtokcase, tp_integer);
                gettok();
                break;
...
Expr* pc_expr_str(char* buf)
{
    ...
    ex = pc_expr();
    if(curtok != TOK_EOF)
        warning(format_s("Junk (%s) at end of C expression [306]", tok_name(curtok)));
    pop_input();
    C_lex--;
    strlist_empty(&defsl);
    return ex;
...
Expr* fixexpr(Expr* ex, int env)
{
    ...
    switch(ex->kind)
    {
        ...
        case EK_BICALL:
            ...
            if(!strcmp(cp, "strlen"))
            {
                if(ex->args[0]->kind == EK_BICALL && !strcmp(ex->args[0]->val.s, "sprintf") && sprintf_value == 0)
                { /* does sprintf return char count? */
                    ex = grabarg(ex, 0);
            ...
            {
                ex->args[0] = fixexpr(ex->args[0], ENV_BOOL);
                ex = makeexpr_cond(ex->args[0], makeexpr_long(0),
                                   (ex->nargs == 2) ?
                                   makeexpr_bicall_1(name_ESCIO, tp_int, ex->args[1]) :
                                   makeexpr_bicall_2(name_ESCIO2, tp_int, ex->args[1], ex->args[2]));
            }
            ...
            else if((!strcmp(cp, setaddname) || !strcmp(cp, setaddrangename)) && (ex2 = ex->args[0])->kind == EK_BICALL
                    && (!strcmp(ex2->val.s, setaddname) || !strcmp(ex2->val.s, setaddrangename)))
            {
            ...
            else if(!strcmp(cp, setunionname) && (ex3 = singlevar(ex->args[0])) != NULL
                    && ((i = 1, exprsame(ex->args[0], ex->args[i], 0))
                        || (i = 2, exprsame(ex->args[0], ex->args[i], 0)))
                    && !exproccurs(ex3, ex->args[3 - i]) && ex->args[3 - i]->kind == EK_BICALL
                    && (!strcmp(ex->args[3 - i]->val.s, setaddname) || !strcmp(ex->args[3 - i]->val.s, setaddrangename)
            ...
            else if(!strcmp(cp, setdiffname) && *setremname && (ex3 = singlevar(ex->args[0])) != NULL
                    && exprsame(ex->args[0], ex->args[1], 0) && !exproccurs(ex3, ex->args[2])
                    && ex->args[2]->kind == EK_BICALL
                    && (!strcmp(ex->args[2]->val.s, setaddname)
                        || (!strcmp(ex->args[2]->val.s, setexpandname) && checkconst(ex->args[2]->args[1], 0)))
                    && totempvar(ex->args[2]))
            {
            ...
            else
            {
                ...
                {
                    if(env == ENV_STMT && ex->args[1]->kind == EK_BICALL
                       && !strcmp(ex->args[1]->val.s, "strcpy") && nosideeffects(ex->args[1]->args[0], 1))
                    {
                        ex2 = ex->args[1];
			...
			if (ex2 == ex->args[2]) {
			    ...
			    *ep = ex->args[1];
                ...
                else if(!strcmp(cp, setexpandname) && env == ENV_STMT && checkconst(ex->args[1], 0))
                {
                    ex = makeexpr_assign(makeexpr_hat(ex->args[0], 0), ex->args[1]);
                }
                else if(!strcmp(cp, getbitsname))
                {
                    ...
                    sbuf[2] = 0;
                    if(sbuf[1] == 'S' && type->smax->val.type == tp_boolean)
                    {
                        ex = makeexpr_rel(
                        EK_NE,
                        makeexpr_bin(EK_BAND, tp_integer, ex->args[0],
                                     makeexpr_bin(EK_LSH, tp_integer,
                                                  makeexpr_longcast(makeexpr_long(1), type->basetype == tp_unsigned),
                                                  ex->args[1])),
                        makeexpr_long(0));
                        ex = fixexpr(ex, env);
                    }
                ...
                else if(!strcmp(cp, putbitsname))
                {
                    ...
                    if(sbuf[1] == 'S' && type->smax->val.type == tp_boolean)
                    {
                        ex = makeexpr_assign(
                        ex->args[0],
                        makeexpr_bin(EK_BOR, tp_integer, copyexpr(ex->args[0]),
                                     makeexpr_bin(EK_LSH, tp_integer,
                                                  makeexpr_longcast(ex->args[2], type->basetype == tp_unsigned),
                                                  ex->args[1])));
                    }
                    else
                        strchange(&ex->val.s, format_s(cp, sbuf));
                ...
                else if(!strcmp(cp, clrbitsname))
                {
                    ...
                    sbuf[1] = 0;
                    if(sbuf[0] == 'S' && type->smax->val.type == tp_boolean)
                    {
                        ex = makeexpr_assign(
                        ex->args[0],
                        makeexpr_bin(
                        EK_BAND, tp_integer, copyexpr(ex->args[0]),
                ...
                {
                    if(which_lang == LANG_HP && ex->args[0]->kind == EK_CONST
                       && ex->args[0]->val.type->kind == TK_STRING && ex->args[0]->val.i >= 1
                       && ex->args[0]->val.i <= 2 && isdigit(ex->args[0]->val.s[0])
                       && (ex->args[0]->val.i == 1 || isdigit(ex->args[0]->val.s[1])))
                    {
        ...
        case EK_NE:
            ...
            if(checkconst(ex->args[1], 0) && env == ENV_BOOL && ord_type(ex->args[1]->val.type)->kind != TK_ENUM
               && (implicitzero > 0
            ...
            if(checkconst(ex->args[1], 0))
            {
                ...
                if(i >= 0)
                {
                    ex = makeexpr_val(make_ord(tp_boolean, (ex->kind == EK_EQ) ? i : !i));
                    break;
                }
            }
        ...
        case EK_CHECKNIL:
            ...
            type = ex->args[0]->val.type;
            type2 = ex->val.type;
            ex = makeexpr_cond(makeexpr_rel(EK_NE, ex2, makeexpr_nil()), ex3,
                               makeexpr_cast(makeexpr_bicall_0(name_NILCHECK, tp_int), type));
            ex->val.type = type2;
            ex = fixexpr(ex, env);
            break;
...
#define relOp(k) \
    ((k) == EK_EQ || (k) == EK_LT || (k) == EK_GT || (k) == EK_NE || (k) == EK_GE || (k) == EK_LE)

#define mathOp(k) \
    ((k) == EK_PLUS || (k) == EK_TIMES || (k) == EK_NEG || (k) == EK_DIV || (k) == EK_DIVIDE || (k) == EK_MOD)
...
Static int incompat(Expr* ex, int num, int prec)
{
    ...
    {
        if(logOp(ex->kind) || bitOp(ex->kind) || (divOp(ex->kind) && num == 0))
    ...
    if(extraparens == 2)
        return 15;
    if(divOp(ex->kind) && num == 0 && (subex->kind == EK_TIMES || divOp(subex->kind)))
        return -99;
...
#define setprec3(p)                                               \
    ...
    {                                                             \
        parens = 2;                                               \
        output((prec > 2 && breakparens != 0) ? "\010" : "\003"); \
    }
...
#define checkbreak(code)                               \
    do                                                 \
    {                                                  \
        breakflag = (code);                            \
        if((prec != -99) && (breakflag & BRK_ALLNONE)) \
            output("\007");                            \
    } while(0)
...
Static void out_ctx(Meaning* ctx, int address)
{
    ...
    if(ctx->kind == MK_FUNCTION && ctx->varstructflag)
    {
        if(curctx != ctx)
        {
            ...
            if(ctx2 != ctx)
                intwarning("out_ctx",
                           format_s("variable from %s not present in context path [307]", ctx->name));
            if(address && curctx->ctx && curctx->ctx != ctx)
                output("\004");
            if(!address)
                outop2("->");
...
void out_var(Meaning* mp, int prec)
{
    switch(mp->kind)
    {
    ...
    while(mp && mp->kind != MK_VARIANT)
    {
        if(mp == field)
        {
            for(i = 0; i < lev; i++)
            {
                ...
                if(variants[i]->ctx->cnext || variants[i]->ctx->kind != MK_FIELD)
                {
...
void out_field(Meaning* mp)
{
    ...
    else if(mp->warnifused)
    {
        if(mp->rectype->meaning)
            note(format_ss("Reference to field %s of record %s [282]", mp->name,
                           mp->rectype->meaning->name));
        else
...
Static void wrexpr(Expr* ex, int prec)
{
    ...
    switch(ex->kind)
    {
            ...
            for(i = 0; i < ex->nargs; i++)
            {
                ...
                if(ex2->kind == EK_VAR)
                {
                    ...
                    if(mp->kind == MK_CONST && mp->val.type
                       && (mp->val.type->kind == TK_RECORD || mp->val.type->kind == TK_ARRAY))
                    {
                        if(foldconsts != 1)
        ...
        case EK_FUNCTION:
            ...
            for(i = 0; i < ex->nargs; i++)
            {
                if((j == FSPCARG1 && i == 1) || (j == FSPCARG2 && i == 2) || (j == FSPCARG3 && i == 3))
                    if(spacecommas)
                        output(",\011 ");
                    else
                        output(",\011");
            ...
            if(mp->ctx->kind == MK_FUNCTION && mp->ctx->varstructflag)
            {
                if(i > 0)
                    if(spacecommas)
                        ...
                        output(",\002");
                if(curctx->ctx->kind == MK_FUNCTION && !curctx->ctx->varstructflag && mp->ctx != curctx)
                    note(format_ss("Discovered too late that %s needs a %s [340]", curctx->name,
        ...
        case EK_NEW:
            setprec(16);
            output("new ");
            if(ex->nargs >= 2 && (ex->args[1]->kind != EK_NAME || *ex->args[1]->val.s))
        ...
        case EK_INDEX:
            ...
            if(lookback(1) == ']')
                output("\001");
            output("[");
            if(ex->args[1]->kind == EK_PLUS && ex->args[1]->nargs == 2 && ex->args[1]->args[0]->kind == EK_VAR
               && (ex->args[1]->args[1]->kind == EK_CONST || ex->args[1]->args[1]->kind == EK_LONGCONST)
        ...
        case EK_ACTCAST:
            ...
            else if(ord_type(ex->args[0]->val.type)->kind == TK_ENUM && ex->val.type == tp_int && !useenum)
            {
                wrexpr(ex->args[0], prec);
            }
            else if(callcasts && onewordtype(ex->val.type, ODECL_ARRAYPTRS))
            {
        ...
        case EK_LITCAST:
            if(callcasts
               && ((ex->args[0]->kind == EK_TYPENAME && onewordtype(ex->args[0]->val.type, 0))
                   || (ex->args[0]->kind == EK_NAME && onewordstring(ex->args[0]->val.s))))
            {
                out_expr(ex->args[0]);
                output("(\002");
                out_expr(ex->args[1]);
        ...
        case EK_PLUS:
            ...
            {
                j = 1;
                while(j < ex2->nargs && expr_looks_neg(ex2->args[j]))
                    j++;
                if(j < ex2->nargs)
                    swapexprs(ex2->args[0], ex2->args[j]);
            }
            else if(ex2->val.i && ex2->nargs == 2)
            { /* this was originally "a-b" */
        ...
        case EK_ASSIGN:
            if(ex->args[1]->kind == EK_PLUS && exprsame(ex->args[1]->args[0], ex->args[0], 2)
               && ex->args[1]->args[1]->kind == EK_CONST && ex->args[1]->args[1]->val.type->kind == TK_INTEGER
            ...
            else
            {
                ...
                if(extraparens != 0
                   && (ex2->kind == EK_EQ || ex2->kind == EK_NE || ex2->kind == EK_GT || ex2->kind == EK_LT
                       || ex2->kind == EK_GE || ex2->kind == EK_LE || ex2->kind == EK_AND || ex2->kind == EK_OR))
                    wrexpr(ex2, 16);
                else
