...
void select_outfile(fp) FILE* fp;
{
    flush_outfilebuf();
    if(outf == codef)
    {
...
void eatblanklines(void)
{
    eatblanks = 1;
}

Static void
flush_outbuf(int numbreaks, int* breakpos, int* breakindent, int numedits, int* editpos, char* editold, char* editnew)
{
...
char* rawCstring(char* fmt, register char* s, int len, int special)
{
    char buf[500];
    register char* cp;
...
char* makeCstring(register char* s, int len)
{
