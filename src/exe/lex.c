...
int checkeatnote(char* msg)
{
    ...
    for(lp = eatnotes; lp; lp = lp->next)
    {
        ...
        len = strlen(lp->s);
        cp = msg;
        while(*cp && (*cp != lp->s[0] || strncmp(cp, lp->s, len) != 0))
        {
            cp++;
        }
        if(*cp)
...
void push_input_file(FILE* fp, char* fname, int isinclude)
{
    push_input();
...
int turbo_directive(const char* closing, char* after)
{
    char* cp;
    char* cp2;
    int i;
...
Static int parsecomment(int p2c_only, int starparen)
{
    ...
    *cp2 = 0;
    i = numparams;
    while(--i >= 0 && strcmp(rctable[i].name, namebuf) != 0)
    {
        ;
    }
    if(i < 0)
