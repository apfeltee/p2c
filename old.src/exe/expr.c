...
char* value_name(Value val, char* intfmt, int islong)
{
    ...
    switch(type->kind)
    {
        ...
        case TK_BOOLEAN:
            if(!intfmt)
            {
                if(val.i == 1 && *name_TRUE && strcmp(name_TRUE, "1") != 0 && !islong)
                {
                    intfmt = name_TRUE;
                }
                else if(val.i == 0 && *name_FALSE && strcmp(name_FALSE, "0") != 0 && !islong)
                {
                    intfmt = name_FALSE;
                }
                else
...
/** TODO: breaks too often - figure out who keeps calling it with invalid values */
void freeexpr(Expr* ex)
{
    int i;
    /*
    fprintf(stderr, "freeexpr(ex=%p): causes segfaults, disabled for now!!!\n", ex);
    */
    return;
    if(ex != NULL)
    {
...
Expr* makeexpr_un(enum exprkind kind, Type* type, Expr* arg1)
{
    ...
    if(debug > 2)
    {
        fprintf(outf, "makeexpr_un returns ");
        dumpexpr(ex);
        fprintf(outf, "\n");
    }
    return ex;
...
Expr* makeexpr_bin(enum exprkind kind, Type* type, Expr* arg1, Expr* arg2)
{
    ...
    if(debug > 2)
    {
        fprintf(outf, "makeexpr_bin returns ");
        dumpexpr(ex);
        fprintf(outf, "\n");
    }
    return ex;
...
Expr* makeexpr_val(Value val)
{
    ...
    if(val.type->kind == TK_INTEGER && (val.i < -32767 || val.i > 32767) && sizeof_int < 32)
    {
    ...
    if(debug > 2)
    {
        fprintf(outf, "makeexpr_val returns ");
        dumpexpr(ex);
        fprintf(outf, "\n");
    }
    return ex;
...
Expr* makeexpr_var(Meaning* mp)
{
    ...
    if(debug > 2)
    {
        fprintf(outf, "makeexpr_var returns ");
        dumpexpr(ex);
        fprintf(outf, "\n");
    }
    return ex;
...
Expr* makeexpr_name(char* name, Type* type)
{
    ...
    if(debug > 2)
    {
        fprintf(outf, "makeexpr_name returns ");
        dumpexpr(ex);
        fprintf(outf, "\n");
    }
    return ex;
...
Expr* makeexpr_bicall_0(char* name, Type* type)
{
    ...
    if(debug > 2)
    {
        fprintf(outf, "makeexpr_bicall returns ");
        dumpexpr(ex);
        fprintf(outf, "\n");
    }
    return ex;
...
Expr* makeexpr_bicall_3(char* name, Type* type, Expr* arg1, Expr* arg2, Expr* arg3)
{
    Expr* ex;
...
Expr* makeexpr_bicall_4(char* name, Type* type, Expr* arg1, Expr* arg2, Expr* arg3, Expr* arg4)
{
    Expr* ex;
...
Expr* makeexpr_bicall_5(char* name, Type* type, Expr* arg1, Expr* arg2, Expr* arg3, Expr* arg4, Expr* arg5)
{
    Expr* ex;
...
int exprsame(Expr* a, Expr* b, int strict)
{
    ...
    switch(a->kind)
    {
        ...
        case EK_DOT:
            if(a->val.i != b->val.i || (!a->val.i && strcmp(a->val.s, b->val.s) != 0))
            {
                return 0;
            }
            break;
...
int exprequiv(Expr* a, Expr* b)
{
    ...
    switch(a->kind)
    {
        ...
        case EK_NE:
            ...
            for(i = 0; i < b->nargs; i++)
            {
                if(exprequiv(a->args[0], b->args[i]))
                {
                    for(j = 0; j < b->nargs; j++)
                    {
                        if(j != i && exprequiv(a->args[1], b->args[i]))
                        {
                            if(a->nargs == 2)
                            {
                                return 1;
                            ...
                            for(k = 0; k < b->nargs; k++)
                            {
                                if(k != i && k != j && exprequiv(a->args[2], b->args[k]))
                                {
                                    return 1;
                                }
                            }
        ...
        case EK_GE:
            ...
            if(exprequiv(a->args[0], b->args[1]) && exprequiv(a->args[1], b->args[0]))
            {
                return 1;
            }
            break;
...
void deletearg(Expr** ex, int n)
{
    ...
    *ex = ex2;
    ...
    if(debug > 2)
    {
        fprintf(outf, "deletearg returns ");
        dumpexpr(*ex);
        fprintf(outf, "\n");
    }
}
...
void insertarg(Expr** ex, int n, Expr* arg)
{
    ...
    *ex = ex2;
    ...
    if(debug > 2)
    {
        fprintf(outf, "insertarg returns ");
        dumpexpr(*ex);
        fprintf(outf, "\n");
    }
}
...
int checkconst(Expr* ex, long val)
{
    ...
    else if(ex->kind == EK_VAR && (mp = (Meaning*)ex->val.i)->kind == MK_CONST && mp->val.type && foldconsts != 0)
    {
        exval = mp->val;
    }
    else
...
Static Expr* docast(Expr* a, Type* type)
{
    ...
    if(a->val.type->kind == TK_SMALLSET && type->kind == TK_SET)
    {
        ...
        return makeexpr_bicall_2(setexpandname, type, makeexpr_var(mp), makeexpr_arglong(a, 1));
    }
    if(a->val.type->kind == TK_SET && type->kind == TK_SMALLSET)
    {
    ...
    switch(a->kind)
    {
        ...
        case EK_VAR:
            ...
            if(mp->kind == MK_CONST)
            {
                if(mp->val.type && mp->val.type->kind == TK_STRING && type->kind == TK_CHAR)
                {
...
Expr* makeexpr_actcast(Expr* a, Type* type)
{
    if(debug > 2)
    {
        fprintf(outf, "makeexpr_actcast(");
        dumpexpr(a);
        fprintf(outf, ", ");
        dumptypename(type, 1);
        fprintf(outf, ")\n");
...
Expr* makeexpr_cast(Expr* a, Type* type)
{
    Expr* ex;

    if(debug > 2)
    {
        fprintf(outf, "makeexpr_cast(");
        dumpexpr(a);
        fprintf(outf, ", ");
        dumptypename(type, 1);
        fprintf(outf, ")\n");
    ...
    if(a->kind == EK_CAST && a->args[0]->val.type->kind == TK_POINTER
...
Expr* gentle_cast(Expr* a, Type* type)
{
    Expr* ex;
    Type* btype;
    long smin;
    ...
    if(a->val.type->kind == TK_POINTER && type->kind == TK_POINTER)
    {
        ...
        {
            if(type == tp_anyptr && a->kind == EK_CAST && a->args[0]->val.type->kind == TK_POINTER)
            {
    ...
    if(type->kind == TK_ARRAY && (a->val.type->kind == TK_STRING || a->val.type->kind == TK_CHAR)
       && isliteralconst(a, &val) && val.type && ord_range(type->indextype, &smin, &smax))
    {
        smax = smax - smin + 1;
    ...
    if((a->kind == EK_CAST || a->kind == EK_ACTCAST) && btype->kind == TK_INTEGER
...
Expr* makeexpr_charcast(Expr* ex)
{
    ...
    if(ex->kind == EK_VAR && (mp = (Meaning*)ex->val.i)->kind == MK_CONST && mp->val.type
...
Expr* makeexpr_stringcast(Expr* ex)
{
    char ch;
...
Static Expr* dolongcast(Expr* a, int tolong)
{
    ...
    switch(a->kind)
    {
        ...
        case EK_VAR:
            ...
            {
                if(mp->val.i <= ((sizeof_int > 0) ? sizeof_int : 16) && !(tolong & 1))
                {
                    return a;
                }
            }
            else if(mp->kind == MK_VAR || mp->kind == MK_VARREF || mp->kind == MK_PARAM
                    || mp->kind == MK_VARPARAM || mp->kind == MK_FIELD)
            {
        ...
        case EK_CAST:
            if(ord_type(a->val.type)->kind == TK_INTEGER && long_type(a->val.type) == (tolong & 1))
            {
                return a;
            }
            if(tolong == 0)
...
int exprlongness(Expr* ex)
{
    if(sizeof_int >= 32)
    {
        return -1;
    }
    return (dolongcast(ex, 3) != NULL) - (dolongcast(ex, 2) != NULL);
}

Expr* makeexpr_longcast(Expr* a, int tolong)
{
    Expr* ex;
    Type* type;
...
Expr* makeexpr_arglong(Expr* a, int tolong)
{
...
Expr* makeexpr_unlongcast(Expr* a)
{
    switch(a->kind)
    {
        ...
        case EK_CAST:
            if((a->val.type == tp_integer || a->val.type == tp_int)
...
Expr* makeexpr_forcelongness(/* force a to have a definite longness */
                             Expr* a)
{
    Expr* ex;
...
Expr* makeexpr_ord(Expr* ex)
{
    ...
    switch(ord_type(ex->val.type)->kind)
    {
        ...
        case TK_CHAR:
            if(ex->kind == EK_CONST && (ex->val.i >= 32 && ex->val.i < 127))
            {
...
Expr* enum_to_int(Expr* a)
{
    if(ord_type(a->val.type)->kind == TK_ENUM)
    {
        if(a->kind == EK_CAST && ord_type(a->args[0]->val.type)->kind == TK_INTEGER)
        {
            return grabarg(a, 0);
        }
        {
...
Expr* makeexpr_neg(Expr* a)
{
    int i;

    if(debug > 2)
    {
        fprintf(outf, "makeexpr_neg(");
        dumpexpr(a);
        fprintf(outf, ")\n");
    }
    a = enum_to_int(a);
    switch(a->kind)
    {
        ...
        case EK_CAST:
            if(a->val.type != tp_unsigned && a->val.type != tp_uint && a->val.type != tp_ushort
               && a->val.type != tp_ubyte && a->args[0]->val.type != tp_unsigned
               && a->args[0]->val.type != tp_uint && a->args[0]->val.type != tp_ushort
               && a->args[0]->val.type != tp_ubyte && expr_looks_neg(a->args[0]))
            {
...
Type* true_type(Expr* ex)
{
    ...
    if(ex->kind == EK_INDEX)
    {
        ...
        if((tp->kind == TK_ARRAY || tp->kind == TK_SMALLARRAY || tp->kind == TK_STRING)
...
int ischartype(Expr* ex)
{
    ...
    if(ISCONST(ex->kind) && ex->nargs > 0 && ex->args[0]->kind == EK_NAME && ex->args[0]->val.s[0] == '\'')
    {
        return 1;
    }
    return 0;
...
Expr* makeexpr_plus(Expr* a, Expr* b)
{
    int i;
    int j;
    int k;
    ...
    for(i = 0; i < a->nargs; i++)
    {
        if(ord_type(a->args[i]->val.type)->kind == TK_CHAR
           || a->args[i]->val.type->kind == TK_POINTER || a->args[i]->val.type->kind == TK_STRING)
        { /* for string literals */
        ...
        for(j = i + 1; j < a->nargs; j++)
        {
            if(ISCONST(a->args[j]->kind))
            {
                if((ord_type(a->args[i]->val.type) == ord_type(a->args[j]->val.type)
                   ...
                   && (!(ischartype(a->args[i]) || ischartype(a->args[j])) || chararith
                       || a->args[i]->val.i == -a->args[j]->val.i || a->args[i]->val.i == 0
                       || a->args[j]->val.i == 0)
                   && (a->args[i]->val.type->kind != TK_REAL && a->args[i]->val.type->kind != TK_STRING
                       && a->args[j]->val.type->kind != TK_REAL && a->args[j]->val.type->kind != TK_STRING))
                {
                    a->args[i]->val.i += a->args[j]->val.i;
                    delfreearg(&a, j);
                    j--;
                }
                else if(a->args[i]->val.type->kind == TK_STRING && ord_type(a->args[j]->val.type)->kind == TK_INTEGER
                        && a->args[j]->val.i < 0 && a->args[j]->val.i >= -stringleaders)
                {
    ...
    {
        if(a->args[a->nargs - 1]->val.type->kind == TK_REAL && a->args[0]->val.type->kind != TK_REAL)
        {
            castdouble = 1;
        }
    ...
    {
        if(a->nargs == 2 && ISCONST(a->args[1]->kind) && a->args[1]->val.i <= -127
           && true_type(a->args[0]) == tp_char && signedchars != 0)
        {
            a->args[0] = force_unsigned(a->args[0]);
        }
        if(a->nargs > 2 && ISCONST(a->args[a->nargs - 1]->kind) && ISCONST(a->args[a->nargs - 2]->kind)
           && ischartype(a->args[a->nargs - 1]) && ischartype(a->args[a->nargs - 2]))
        {
               ...
               && (j == 'a' || j == 'A' || j == -'a' || j == -'A'))
            {
                if(abs(i + j) == 32)
                {
                    ...
                    a = makeexpr_bicall_1((i + j > 0) ? "_tolower" : "_toupper", tp_char, a);
                }
            }
        }
    }
...
Expr* makeexpr_minus(Expr* a, Expr* b)
{
    int okneg;

    if(debug > 2)
    {
        fprintf(outf, "makeexpr_minus(");
        dumpexpr(a);
        fprintf(outf, ", ");
        dumpexpr(b);
        fprintf(outf, ")\n");
...
Expr* makeexpr_inc(Expr* a, Expr* b)
{
    Type* type;

    type = a->val.type;
    a = makeexpr_plus(makeexpr_charcast(a), b);
    if(ord_type(type)->kind != TK_INTEGER && ord_type(type)->kind != TK_CHAR)
    {
...
Expr* distribute_plus(Expr* ex)
{
    ...
    for(;;)
    {
            ...
            if(ex2->kind == EK_TIMES)
            {
                for(j = ex2->nargs; --j >= 0;)
                {
                    if(exprsame(ex2->args[j], common, 1)
                       || (ex2->args[j]->kind == EK_NEG && exprsame(ex2->args[j]->args[0], common, 1)))
                    {
                        break;
                    }
                }
        ...
        if(i == ex->nargs)
        {
            if(debug > 2)
            {
                fprintf(outf, "distribute_plus does ");
                dumpexpr(common);
                fprintf(outf, "\n");
            }
            ...
            for(i = 0; i < ex->nargs; i++)
            {
                ...
                if(ex2->kind == EK_TIMES)
                {
                    for(j = ex2->nargs; --j >= 0;)
                    {
                        ...
                        if(ex2->args[j]->kind == EK_NEG && exprsame(ex2->args[j]->args[0], common, 1))
                        {
                            freeexpr(ex2->args[j]);
...
Expr* makeexpr_times(Expr* a, Expr* b)
{
    int i;
    int n;
    int castdouble = 0;
    ...
    if(n & 1)
    {
        for(i = 0; i < a->nargs; i++)
        {
            if(ISCONST(a->args[i]->kind) && expr_neg_cost(a->args[i]) >= 0)
            {
                a->args[i] = makeexpr_neg(a->args[i]);
                n++;
                break;
...
Expr* makeexpr_sqr(Expr* ex, int cube)
{
    Expr* ex2;
    Meaning* tvar;
    Type* type;
...
Expr* makeexpr_divide(Expr* a, Expr* b)
{
    Expr* ex;
    int p;
    ...
    if(a->val.type->kind != TK_REAL && b->val.type->kind != TK_REAL)
    { /* must do a real division */
...
int negsigns(int mask)
{
    return (mask & 2) | ((mask & 1) << 2) | ((mask & 4) >> 2);
}

int possiblesigns(Expr* ex)
{
    Value val;
    Type* tp;
    char* cp;
    ...
    if(ex->kind == EK_CAST && similartypes(ex->val.type, ex->args[0]->val.type))
    {
    ...
    if(ex->kind == EK_EQ || ex->kind == EK_LT || ex->kind == EK_GT || ex->kind == EK_NE || ex->kind == EK_LE
       || ex->kind == EK_GE || ex->kind == EK_AND || ex->kind == EK_OR || ex->kind == EK_NOT)
    {
        return 2 | 4;
    }
    ...
    {
        cp = ex->val.s;
        if(!strcmp(cp, "strlen") || !strcmp(cp, "abs") || !strcmp(cp, "labs") || !strcmp(cp, "fabs"))
        {
            return 2 | 4;
        }
    }
...
Expr* dodivmod(char* funcname, enum exprkind ekind, Expr* a, Expr* b)
{
    Meaning* tvar;
    Type* type;
    Expr* asn;
        ...
        if(*funcname)
        {
            ...
            if(*funcname == '*')
            {
                ...
                if(exprspeed(b) >= 5 || !nosideeffects(b, 0))
                {
                    ...
                    asn = makeexpr_comma(asn, makeexpr_assign(makeexpr_var(tvar), b));
                    b = makeexpr_var(tvar);
                }
            }
            return makeexpr_comma(asn, makeexpr_bicall_2(funcname, type, a, b));
        }
...
Expr* makeexpr_div(Expr* a, Expr* b)
{
    Meaning* mp;
    Type* type;
    long i;
    ...
    if(((b->kind == EK_CONST && (i = b->val.i))
        || (b->kind == EK_VAR && (mp = (Meaning*)b->val.i)->kind == MK_CONST && mp->val.type
            && (i = mp->val.i) && foldconsts != 0))
       && i > 0)
    {
        if(i == 1)
        {
...
Expr* makeexpr_mod(Expr* a, Expr* b)
{
    Meaning* mp;
    Type* type;
    long i;

    if(a->kind == EK_CONST && b->kind == EK_CONST && a->val.i >= 0 && b->val.i > 0)
    {
        a->val.i %= b->val.i;
        freeexpr(b);
        return a;
       ...
       && i > 0)
    {
        ...
        if(mod_po2 != 0)
        {
            ...
            {
                type = promote_type_bin(a->val.type, b->val.type);
                return makeexpr_bin(EK_BAND, type, a, makeexpr_minus(b, makeexpr_long(1)));
            }
        }
    }
    if(checkconst(b, 0))
...
Expr* makeexpr_rem(Expr* a, Expr* b)
{
...
int expr_not_cost(Expr* a)
{
    ...
    switch(a->kind)
    {
        ...
        case EK_BICALL:
            if(!strcmp(a->val.s, oddname) || !strcmp(a->val.s, evenname))
            {
                return 0;
            }
            return 1;
...
Type* mixsets(Expr** ep1, Expr** ep2)
{
    Expr* ex1 = *ep1;
    Expr* ex2 = *ep2;
    Meaning* tvar;
    ...
    if(ex1->val.type->kind == TK_SMALLSET && ex2->val.type->kind == TK_SMALLSET)
    {
        return ex1->val.type;
    }
    if(ex1->val.type->kind == TK_SMALLSET)
    {
        ...
        ex1 = makeexpr_bicall_2(setexpandname, ex2->val.type, makeexpr_var(tvar), makeexpr_arglong(ex1, 1));
    }
    if(ex2->val.type->kind == TK_SMALLSET)
    {
        tvar = makestmttempvar(ex1->val.type, name_SET);
        ex2 = makeexpr_bicall_2(setexpandname, ex1->val.type, makeexpr_var(tvar), makeexpr_arglong(ex2, 1));
    }
    if(ord_range(ex1->val.type->indextype, &min1, &max1) && ord_range(ex2->val.type->indextype, &min2, &max2))
    {
...
Meaning* istempprocptr(Expr* ex)
{
    Meaning* mp;

    if(debug > 2)
    {
        fprintf(outf, "istempprocptr(");
        dumpexpr(ex);
        fprintf(outf, ")\n");
    }
    if(ex->kind == EK_COMMA && ex->nargs == 3)
    {
        if((mp = istempvar(ex->args[2])) != NULL && mp->type->kind == TK_PROCPTR
           && ex->args[0]->kind == EK_ASSIGN && ex->args[0]->args[0]->kind == EK_DOT
           && exprsame(ex->args[0]->args[0]->args[0], ex->args[2], 1)
           && ex->args[1]->kind == EK_ASSIGN && ex->args[1]->args[0]->kind == EK_DOT
    ...
    {
        if((mp = istempvar(ex->args[1])) != NULL && mp->type->kind == TK_CPROCPTR
           && ex->args[0]->kind == EK_ASSIGN && exprsame(ex->args[0]->args[0], ex->args[1], 1))
        {
            return mp;
        }
    }
...
Expr* makeexpr_stringify(Expr* ex)
{
...
Expr* makeexpr_rel(enum exprkind rel, Expr* a, Expr* b)
{
    int i;
    int sign;
    Expr* ex;
    ...
    if(debug > 2)
    {
        fprintf(outf, "makeexpr_rel(%s,", exprkindname(rel));
        dumpexpr(a);
        fprintf(outf, ", ");
        dumpexpr(b);
        fprintf(outf, ")\n");
    ...
    if((compenums == 0 || (compenums < 0 && ansiC <= 0)) && (rel != EK_EQ && rel != EK_NE))
    {
        a = enum_to_int(a);
        b = enum_to_int(b);
    }
    ...
    {
        if(a->val.type->kind == TK_STRING && a->kind != EK_CONST)
        {
            b = makeexpr_stringify(b);
        }
        else if(b->val.type->kind == TK_STRING && b->kind != EK_CONST)
        {
            a = makeexpr_stringify(a);
        }
        else if(ord_type(a->val.type)->kind == TK_CHAR || a->val.type->kind == TK_ARRAY)
        {
            b = gentle_cast(b, ord_type(a->val.type));
        }
        else if(ord_type(b->val.type)->kind == TK_CHAR || b->val.type->kind == TK_ARRAY)
        {
    ...
    if((ord_type(b->val.type)->kind == TK_INTEGER || ord_type(b->val.type)->kind == TK_CHAR)
       && b->val.type->kind != TK_STRING)
    {
        for(;;)
        {
            if(a->kind == EK_PLUS && ISCONST(a->args[a->nargs - 1]->kind)
               ...
               && (ISCONST(b->kind) || (b->kind == EK_PLUS && ISCONST(b->args[b->nargs - 1]->kind))))
            {
                chararith++;
            ...
            if(b->kind == EK_PLUS && ISCONST(b->args[b->nargs - 1]->kind)
               && b->args[b->nargs - 1]->val.i && ISCONST(a->kind))
            {
                chararith++;
            ...
            if(b->kind == EK_PLUS && sign && ISCONST(b->args[b->nargs - 1]->kind)
               && checkconst(b->args[b->nargs - 1], sign)
               && (a->val.type->kind == TK_INTEGER || a->val.type->kind == TK_CHAR))
            {
        ...
        if(a->kind == EK_BICALL && !strcmp(a->val.s, "strlen") && checkconst(b, 0))
        {
            if(rel == EK_LT || rel == EK_GE)
            {
        ...
        if((a->val.type == tp_char || true_type(a) == tp_char) && ISCONST(b->kind) && signedchars != 0)
        {
            i = (b->val.i == 128 && sign == 1) || (b->val.i == 127 && sign == -1);
            if(highcharbits && (highcharbits > 0 || signedchars < 0) && i)
            {
                ...
                return makeexpr_rel((rel == EK_GE || rel == EK_GT) ? EK_NE : EK_EQ,
                                    makeexpr_bin(EK_BAND, tp_integer, eatcasts(a), b), makeexpr_long(0));
            }
            if(signedchars == 1 && i)
            {
                return makeexpr_rel((rel == EK_GE || rel == EK_GT) ? EK_LT : EK_GE, eatcasts(a),
                                    makeexpr_long(0));
            }
            ...
            else if(b->val.i >= 128 || (b->val.i == 127 && sign != 0))
            {
                if(highcharbits && (highcharbits > 0 || signedchars < 0))
                {
                    a = makeexpr_bin(EK_BAND, a->val.type, eatcasts(a), makeexpr_long(255));
                }
                else
                {
                    a = force_unsigned(a);
    ...
    else if(a->val.type->kind == TK_STRING && b->val.type->kind == TK_STRING)
    {
    ...
    else if((a->val.type->kind == TK_ARRAY || a->val.type->kind == TK_STRING || a->val.type->kind == TK_RECORD)
            && (b->val.type->kind == TK_ARRAY || b->val.type->kind == TK_STRING || b->val.type->kind == TK_RECORD))
    {
        ...
        name = (usestrncmp && a->val.type->kind == TK_ARRAY && a->val.type->basetype->kind == TK_CHAR) ?
               "strncmp" :
               "memcmp";
        a = makeexpr_bicall_3(name, tp_int, makeexpr_addr(a), makeexpr_addr(b), ex);
        b = makeexpr_long(0);
    }
    else if(a->val.type->kind == TK_SET || a->val.type->kind == TK_SMALLSET)
    {
        if(rel == EK_GE)
        {
            swapexprs(a, b);
        ...
        if(mixsets(&a, &b)->kind == TK_SMALLSET)
        {
            if(rel == EK_LE)
            {
                a = makeexpr_bin(EK_BAND, tp_integer, a, makeexpr_un(EK_BNOT, tp_integer, b));
                b = makeexpr_long(0);
                rel = EK_EQ;
            }
        }
        else if(b->kind == EK_BICALL && !strcmp(b->val.s, setexpandname)
                && (mp = istempvar(b->args[0])) != NULL && checkconst(b->args[1], 0))
        {
            canceltempvar(mp);
            a = makeexpr_hat(a, 0);
            b = grabarg(b, 1);
        ...
        {
            ex = makeexpr_bicall_2((rel == EK_LE) ? subsetname : setequalname, tp_boolean, a, b);
            return (rel == EK_NE) ? makeexpr_not(ex) : ex;
        }
    }
    else if(a->val.type->kind == TK_PROCPTR || a->val.type->kind == TK_CPROCPTR)
    {
...
Expr* makeexpr_and(Expr* a, Expr* b)
{
    Expr* ex;
    Expr** exp;
    Expr* low;
...
Expr* makeexpr_or(Expr* a, Expr* b)
{
    Expr* ex;
    Expr** exp;
    Expr* low;
    ...
    if(((b->kind == EK_BICALL && !strcmp(b->val.s, "isdigit") && ex->kind == EK_BICALL
         && !strcmp(ex->val.s, "isalpha"))
        || (b->kind == EK_BICALL && !strcmp(b->val.s, "isalpha") && ex->kind == EK_BICALL
            && !strcmp(ex->val.s, "isdigit")))
    ...
    if(((b->kind == EK_BICALL && !strcmp(b->val.s, "islower") && ex->kind == EK_BICALL
         && !strcmp(ex->val.s, "isupper"))
        || (b->kind == EK_BICALL && !strcmp(b->val.s, "isupper") && ex->kind == EK_BICALL
            && !strcmp(ex->val.s, "islower")))
       && exprsame(ex->args[0], b->args[0], 1))
    {
...
Expr* makeexpr_range(Expr* ex, Expr* exlow, Expr* exhigh, int higheq)
{
    Expr* ex2;
...
Expr* makeexpr_cond(Expr* c, Expr* a, Expr* b)
{
    Expr* ex;
    ...
    if(debug > 2)
    {
        fprintf(outf, "makeexpr_cond returns ");
        dumpexpr(ex);
        fprintf(outf, "\n");
    }
    return ex;
...
int expr_is_lvalue(Expr* ex)
{
    ...
    switch(ex->kind)
    {
        ...
        case EK_VAR:
            mp = (Meaning*)ex->val.i;
            return (mp->kind == MK_VAR || mp->kind == MK_PARAM || mp->kind == MK_VARPARAM
                    || (mp->kind == MK_CONST
                        && (mp->type->kind == TK_ARRAY || mp->type->kind == TK_RECORD || mp->type->kind == TK_SET)));
...
int expr_has_address(Expr* ex)
{
    if(ex->kind == EK_DOT && ((Meaning*)ex->val.i)->val.i)
    {
    ...
    if(nilcheck == 1)
    {
        ...
        else
        {
            ex = makeexpr_bin(EK_CHECKNIL, ex->val.type, ex,
                              makeexpr_var(makestmttempvar(ex->val.type, name_PTR)));
        }
    }
    return ex;
}

int checkvarinlists(Strlist* yes, Strlist* no, int def, Meaning* mp)
{
    char* cp;
    Meaning* ctx;
...
void requirefilebuffer(Expr* ex)
{
    Meaning* mp;
    ...
    else if(!mp->bufferedfile && checkvarinlists(bufferedfiles, unbufferedfiles, 1, mp))
    {
        if(mp->wasdeclared)
        {
            note(format_s("Discovered too late that %s should be buffered [143]", mp->name));
        }
        mp->bufferedfile = 1;
    }
}

Expr* makeexpr_hat(Expr* a, int check)
{
    Expr* ex;

    if(debug > 2)
    {
        fprintf(outf, "makeexpr_hat(");
        dumpexpr(a);
        fprintf(outf, ")\n");
    }
    if(isfiletype(a->val.type, -1))
    {
        if(*chargetfbufname && filebasetype(a->val.type)->kind == TK_CHAR)
        {
            return makeexpr_bicall_1(chargetfbufname, filebasetype(a->val.type), filebasename(a));
        }

        requirefilebuffer(a);
        if(*arraygetfbufname && filebasetype(a->val.type)->kind == TK_ARRAY)
        {
            return makeexpr_bicall_2(arraygetfbufname, filebasetype(a->val.type), filebasename(a),
                                     makeexpr_type(filebasetype(a->val.type)));
        }
        {
            return makeexpr_bicall_2(getfbufname, filebasetype(a->val.type), filebasename(a),
                                     makeexpr_type(filebasetype(a->val.type)));
        }
    }
    if(a->kind == EK_PLUS && (ex = a->args[0])->val.type->kind == TK_POINTER
       && (ex->val.type->basetype->kind == TK_ARRAY || ex->val.type->basetype->kind == TK_STRING
    ...
    if(a->val.type->kind == TK_STRING || a->val.type->kind == TK_ARRAY || a->val.type->kind == TK_SET)
    {
        if(starindex == 0)
        {
    ...
    if(a->kind == EK_CAST && a->val.type->basetype->kind == TK_POINTER
       && a->args[0]->val.type->kind == TK_POINTER && a->args[0]->val.type->basetype->kind == TK_POINTER)
    {
        return makeexpr_cast(makeexpr_hat(a->args[0], 0), a->val.type->basetype);
    }
    switch(a->val.type->basetype->kind)
    {
        ...
        case TK_STRING:
        case TK_SET:
            if(a->kind != EK_HAT || 1 || a->val.type == a->args[0]->val.type->basetype)
            {
...
Expr* un_sign_extend(Expr* a)
{
    if(a->kind == EK_BICALL && !strcmp(a->val.s, signextname) && *signextname)
    {
        return grabarg(a, 0);
    }
    return a;
...
Expr* makeexpr_addr(Expr* a)
{
    ...
    if(a->kind == EK_INDEX
       && (a->val.type->kind != TK_ARRAY && a->val.type->kind != TK_SET && a->val.type->kind != TK_STRING)
       && (addindex == 1 || (addindex != 0 && checkconst(a->args[1], 0))))
    {
    ...
    else if(a->kind == EK_LITCAST)
    {
        ...
        {
            if(my_strchr(a->args[0]->val.s, '(') || my_strchr(a->args[0]->val.s, '['))
            {
                note("Constructing pointer type by adding '*' may be incorrect [322]");
            }
            return makeexpr_bin(EK_LITCAST, tp_integer,
                                makeexpr_name(format_s("%s*", a->args[0]->val.s), tp_integer),
                                makeexpr_addr(a->args[1]));
        }
        {
            return makeexpr_bin(EK_LITCAST, tp_integer, makeexpr_type(type), makeexpr_addr(a->args[1]));
        }
    }
    else
    {
        switch(a->val.type->kind)
        {
            ...
            case TK_SET:
                if(a->val.type->smin)
                {
                    return makeexpr_un(EK_ADDR, type, makeexpr_index(a, copyexpr(a->val.type->smin), NULL));
                }
                a->val.type = type;
                return a;
            ...
            default:
                ...
                {
                    if(a->kind == EK_VAR && (mp = (Meaning*)a->val.i)->kind == MK_PARAM
                       && !mp->isref && mp->type != promote_type(mp->type) && fixpromotedargs)
                    {
                        note(format_s("Taking & of possibly promoted param %s [324]", mp->name));
...
Expr* makeexpr_addrstr(Expr* a)
{
    if(debug > 2)
    {
        fprintf(outf, "makeexpr_addrstr(");
        dumpexpr(a);
        fprintf(outf, ")\n");
    }
    ...
    if(is_std_file(a))
    {
        if(addrstdfiles == 0)
        {
            note(format_s("Taking address of %s; consider setting VarFiles = 0 [144]",
                          (a->kind == EK_VAR) ? ((Meaning*)a->val.i)->name : a->val.s));
            tvar = makestmttempvar(tp_text, name_TEMP);
            return makeexpr_comma(makeexpr_assign(makeexpr_var(tvar), a), makeexpr_addr(makeexpr_var(tvar)));
        }
    }
    if((a->kind == EK_VAR && mp->kind == MK_FIELD && mp->val.i)
       || (a->kind == EK_BICALL && !strcmp(a->val.s, getbitsname)))
    {
...
Expr* makeexpr_index(Expr* a, Expr* b, Expr* offset)
{
    Type* indextype;
    Type* btype;
    ...
    indextype = (a->val.type->kind == TK_ARRAY) ? a->val.type->indextype : tp_integer;
    ...
    {
        return makeexpr_bin(EK_INDEX, btype, a, gentle_cast(b, indextype));
    }
}

Expr* makeexpr_type(Type* type)
{
    Expr* ex;
...
Expr* makeexpr_sizeof(Expr* ex, int incskipped)
{
    Expr* ex2;
    Expr* ex3;
    Type* btype;
    ...
    {
        name = find_special_variant(ex->val.type->meaning->name, "SpecialSizeOf", specialsizeofs, 1);
        if(name)
        {
            freeexpr(ex);
    ...
    switch(ex->val.type->kind)
    {
        ...
        case TK_ARRAY:
               ...
               || (!incskipped && ex->val.type->smin))
            {
                ...
                return makeexpr_times(ex3, makeexpr_sizeof(makeexpr_type(ex->val.type->basetype), 1));
            }
            break;
        ...
        case TK_SET:
            ...
            freeexpr(ex);
            return makeexpr_times(
            makeexpr_plus(makeexpr_div(copyexpr(ex2), makeexpr_setbits()), makeexpr_long(2)),
    ...
    if(ex->kind != EK_CONST
       && (findbasetype(ex->val.type, 0)->meaning || /* if type has a name... */
           ...
           ex->val.type->kind == TK_ARRAY || ex->val.type->kind == TK_SET))
    {
...
int exprspeed(Expr* ex)
{
    ...
    switch(ex->kind)
    {
        ...
        case EK_CAST:
        case EK_ACTCAST:
            i = (ord_type(ex->val.type)->kind == TK_REAL) != (ord_type(ex->args[0]->val.type)->kind == TK_REAL);
            return (i + exprspeed(ex->args[0]));

        case EK_COND:
            return 2 + exprspeed(ex->args[0]) + MAX(exprspeed(ex->args[1]), exprspeed(ex->args[2]));
        ...
        default:
            ...
            for(i = 0; i < ex->nargs; i++)
            {
                if(!isliteralconst(ex->args[i], NULL) || ex->val.type->kind == TK_REAL)
                {
...
int noargdependencies(Expr* ex, int vars)
{
    int i;
...
int nodependencies(Expr* ex, int vars /* 1 if explicit dependencies on vars count as dependencies */
)
{ /* 2 if global but not local vars count as dependencies */
    ...
    switch(ex->kind)
    {
        ...
        case EK_VAR:
            ...
            if(vars == 2 && mp->ctx == curctx && mp->ctx->kind == MK_FUNCTION && !mp->varstructflag)
            {
                return 1;
            }
            return (!vars
                    && (mp->kind == MK_VAR || mp->kind == MK_VARREF || mp->kind == MK_PARAM
                        || mp->kind == MK_VARPARAM));
...
int exprdependsvar(Expr* ex, Meaning* mp)
{
    int i;

    if(mp->isref
       && (ex->kind != EK_VAR || ((Meaning*)ex->val.i)->ctx != curctx || ((Meaning*)ex->val.i)->isref))
    {
        return 1;
    }
    i = ex->nargs;
    ...
    switch(ex->kind)
    {
        ...
        case EK_VAR:
            return ((Meaning*)ex->val.i == mp || (((Meaning*)ex->val.i)->isref && mp->ctx != curctx));
        ...
        case EK_FUNCTION:
        case EK_SPCALL:
            return (mp->ctx != curctx || mp->ctx->kind != MK_FUNCTION || mp->varstructflag);
...
int exprdepends(Expr* ex, Expr* ex2 /* Expression ex somehow depends on value of ex2 */
)
{
    switch(ex2->kind)
    {
...
int nosideeffects_func(Expr* ex)
{
    Meaning* mp;
    Symbol* sp;
...
int deterministic_func(Expr* ex)
{
    Meaning* mp;
    Symbol* sp;
...
int noargsideeffects(Expr* ex, int mode)
{
    int i;
...
int nosideeffects(Expr* ex, int mode)
{
    if(debug > 2)
    {
        fprintf(outf, "nosideeffects(");
        dumpexpr(ex);
        fprintf(outf, ")\n");
    }
...
int exproccurs(Expr* ex, Expr* ex2)
{
    int i;
    int count = 0;
...
Expr* singlevar(Expr* ex)
{
    if(debug > 2)
    {
        fprintf(outf, "singlevar(");
        dumpexpr(ex);
        fprintf(outf, ")\n");
    }
    switch(ex->kind)
...
Meaning* istempvar(Expr* ex)
{
    Meaning* mp;

    if(debug > 2)
    {
        fprintf(outf, "istempvar(");
        dumpexpr(ex);
        fprintf(outf, ")\n");
    }
    if(ex->kind == EK_VAR)
...
Meaning* isretvar(Expr* ex)
{
    Meaning* mp;

    if(debug > 2)
    {
        fprintf(outf, "isretvar(");
        dumpexpr(ex);
        fprintf(outf, ")\n");
    }
    if(ex->kind == EK_HAT)
    ...
    {
        mp = (Meaning*)ex->val.i;
        if(mp->ctx && mp->ctx->kind == MK_FUNCTION && mp->ctx->isfunction && mp == mp->ctx->cbase)
        {
            return mp;
        }
        {
    ...
    if(ex->kind == EK_NAME && !strcmp(ex->val.s, name_RETV) && curctx && curctx->kind == MK_FUNCTION
       && curctx->rectype)
    {
        return curctx->cbase;
    }
...
Expr* bumpstring(Expr* ex, Expr* index, int offset)
{
    ...
    if(addindex != 0)
    {
        ex = makeexpr_plus(makeexpr_addrstr(ex), makeexpr_minus(index, makeexpr_long(offset)));
    }
    else
    {
...
int isarithkind(enum exprkind kind)
{
    return (kind == EK_EQ || kind == EK_LT || kind == EK_GT || kind == EK_NE || kind == EK_LE
            || kind == EK_GE || kind == EK_PLUS || kind == EK_TIMES || kind == EK_DIVIDE
            || kind == EK_DIV || kind == EK_MOD || kind == EK_NEG || kind == EK_AND || kind == EK_OR
            || kind == EK_NOT || kind == EK_BAND || kind == EK_BOR || kind == EK_BXOR || kind == EK_LSH
            || kind == EK_RSH || kind == EK_BNOT || kind == EK_FUNCTION || kind == EK_BICALL);
}

Expr* makeexpr_assign(Expr* a, Expr* b)
{
    int i;
    int j;
    Expr* ex;
    ...
    if(debug > 2)
    {
        fprintf(outf, "makeexpr_assign(");
        dumpexpr(a);
        fprintf(outf, ", ");
        dumpexpr(b);
        fprintf(outf, ")\n");
    }
    if(stringtrunclimit > 0 && a->val.type->kind == TK_STRING && (i = strmax(a)) <= stringtrunclimit
       && strmax(b) > i)
    {
    ...
    a = un_sign_extend(a);
    b = gentle_cast(b, a->val.type);
    if(b->kind == EK_BICALL && !strcmp(b->val.s, "sprintf") && (mp = istempvar(b->args[0])) != NULL
       && b->nargs >= 2 && b->args[1]->kind == EK_CONST && /* all this handles string appending */
       b->args[1]->val.i > 2 && /*   of the form, "s := s + ..." */
       !strncmp(b->args[1]->val.s, "%s", 2) && exprsame(a, b->args[2], 1) && nosideeffects(a, 0)
       && (ex = singlevar(a)) != NULL)
    {
        ex2 = copyexpr(b);
        delfreearg(&ex2, 2);
        freeexpr(ex2->args[1]);
        ex2->args[1] = makeexpr_lstring(b->args[1]->val.s + 2, b->args[1]->val.i - 2);
           ...
           /* noargdependencies(ex2) && */ !exproccurs(ex2, ex))
        {
               ...
               !strncmp(ex2->args[1]->val.s, "%s", 2))
            {
                canceltempvar(mp);
                tp = ex2->val.type;
                return makeexpr_bicall_2("strcat", tp, makeexpr_addrstr(a), grabarg(ex2, 2));
            }
            if(sprintflength(ex2, 0) >= 0)
            { /* s := s + 's2' */
                tp = ex2->val.type;
                return makeexpr_bicall_2("strcat", tp, makeexpr_addrstr(a), makeexpr_unsprintfify(ex2));
            }
            else
            { /* general case */
                canceltempvar(mp);
    ...
    if(b->kind == EK_BICALL && !strcmp(b->val.s, "sprintf") && istempvar(b->args[0])
       && (ex = singlevar(a)) != NULL)
    {
        ...
        if(i == b->nargs && j > 0)
        {
            b->args[j] = makeexpr_bicall_2("strcpy", tp_str255, makeexpr_addrstr(b->args[0]),
    ...
    if(structuredfunc(b) && (ex2 = singlevar(a)) != NULL)
    {
        ...
        if((mp = istempvar(ex)) != NULL && (i || !exproccurs(b, ex2)))
        {
            canceltempvar(mp);
            freeexpr(*ep);
    ...
    if(a->val.type->kind == TK_PROCPTR && (mp = istempprocptr(b)) != NULL && nosideeffects(a, 0))
    {
        ...
        {
            freeexpr(b->args[1]);
            b->args[1] = makeexpr_assign(makeexpr_dotq(a, "link", tp_anyptr), makeexpr_nil());
        }
        canceltempvar(mp);
        return b;
    }
    if(a->val.type->kind == TK_PROCPTR && (b->val.type->kind == TK_CPROCPTR || checkconst(b, 0)))
    {
        ex = makeexpr_dotq(copyexpr(a), "proc", tp_anyptr);
        b = makeexpr_comma(makeexpr_assign(ex, b),
                           makeexpr_assign(makeexpr_dotq(a, "link", tp_anyptr), makeexpr_nil()));
        return b;
    }
    if(a->val.type->kind == TK_CPROCPTR && (mp = istempprocptr(b)) != NULL && nosideeffects(a, 0))
    {
    ...
    if(a->val.type->kind == TK_CPROCPTR && b->val.type->kind == TK_PROCPTR)
    {
        b = makeexpr_dotq(b, "proc", tp_anyptr);
    }
    if(a->val.type->kind == TK_STRING)
    {
        if(b->kind == EK_CONST && b->val.i == 0 && !isretvar(a))
        {
            ...
            return makeexpr_assign(makeexpr_hat(a, 0), makeexpr_char(0));
        }
    ...
    {
        if(b->kind == EK_CAST && ord_type(b->args[0]->val.type)->kind == TK_INTEGER)
        {
            b = grabarg(b, 0);
        }
        j = (b->kind == EK_PLUS && /* handle "s[0] := xxx" */
             b->args[0]->kind == EK_BICALL && !strcmp(b->args[0]->val.s, "strlen")
             && exprsame(a->args[0], b->args[0]->args[0], 0) && isliteralconst(b->args[1], NULL) == 2);
        if(j && b->args[1]->val.i > 0 && b->args[1]->val.i <= 5)
    ...
    if(a->val.type->kind == TK_ARRAY || (a->val.type->kind == TK_PROCPTR && copystructs < 1)
       || (a->val.type->kind == TK_RECORD && (copystructs < 1 || a->val.type != b->val.type)))
    {
        ex = makeexpr_sizeof(copyexpr(a), 0);
        ex2 = makeexpr_sizeof(copyexpr(b), 0);
        if(!exprsame(ex, ex2, 1))
        {
            if(a->val.type->kind == TK_ARRAY && b->val.type->kind == TK_ARRAY
               && a->val.type->basetype->kind == TK_CHAR
               && (!ISCONST(ex->kind) || !ISCONST(ex2->kind) || ex->val.i > ex2->val.i))
            {
                ex = makeexpr_arglong(ex, (size_t_long != 0));
                ex2 = makeexpr_arglong(ex2, (size_t_long != 0));
                a = makeexpr_addrstr(a);
                b = makeexpr_addrstr(b);
                b = makeexpr_bicall_3("memcpy", a->val.type, copyexpr(a), b, copyexpr(ex2));
                ex3 = copyexpr(ex2);
                return makeexpr_comma(b, makeexpr_bicall_3("memset", a->val.type, makeexpr_plus(a, ex3),
                                                           makeexpr_char(' '), makeexpr_minus(ex, ex2)));
            }
            if(!(a->val.type->kind == TK_ARRAY && b->val.type->kind != TK_ARRAY))
            {
    ...
    if(ex3->kind == EK_BICALL)
    {
        if(!strcmp(ex3->val.s, getbitsname))
        {
            ...
            if(checkconst(b, 0) && *clrbitsname)
            {
                strchange(&ex3->val.s, clrbitsname);
                    ...
                    && ((ISCONST(b->kind) && (b->val.i | ~((1L << (1 << tp->escale)) - 1)) == -1)
        ...
        if(!strcmp(ex3->val.s, getfbufname))
        {
            ex3->val.type = tp_void;
    ...
    while(a->kind == EK_CAST || a->kind == EK_ACTCAST || a->kind == EK_LITCAST)
    {
        if(a->kind == EK_LITCAST)
        {
            ...
            a = grabarg(a, 1);
                ...
                || (a->val.type->kind == TK_POINTER && a->args[0]->val.type->kind == TK_POINTER))
        {
            if(a->kind == EK_CAST)
            {
    ...
    if(a->kind == EK_BNOT)
    {
        return makeexpr_assign(grabarg(a, 0), makeexpr_un(EK_BNOT, b->val.type, b));
    }
    if(a->kind == EK_PLUS)
    {
    ...
    if(a->kind == EK_TIMES)
    {
        for(i = 0; i < a->nargs && a->nargs > 1;)
        {
            if(isconstantexpr(a->args[i]))
            {
                ...
                {
                    if(ISCONST(b->kind) && ISCONST(a->args[i]->kind) && (b->val.i % a->args[i]->val.i) != 0)
                    {
                        break;
                    }
    ...
    if((a->kind == EK_DIVIDE || a->kind == EK_DIV) && isconstantexpr(a->args[1]))
    {
...
Expr* makeexpr_comma(Expr* a, Expr* b)
{
    Type* type;
...
int strmax(Expr* ex)
{
    ...
    if(ex->kind == EK_VAR && foldstrconsts != 0 && (mp = (Meaning*)(ex->val.i))->kind == MK_CONST
       && mp->val.type)
    {
        return mp->val.i;
    }
...
int istempsprintf(Expr* ex)
{
    return (ex->kind == EK_BICALL && !strcmp(ex->val.s, "sprintf") && ex->nargs >= 2 && istempvar(ex->args[0])
            && ex->args[1]->kind == EK_CONST && ex->args[1]->val.type->kind == TK_STRING);
}
...
Expr* makeexpr_sprintfify(Expr* ex)
{
    ...
    if(debug > 2)
    {
        fprintf(outf, "makeexpr_sprintfify(");
        dumpexpr(ex);
        fprintf(outf, ")\n");
    }
    if(istempsprintf(ex))
    ...
    if(ex->val.type->kind == TK_ARRAY && ex->val.type->basetype->kind == TK_CHAR)
    {
        ex2 = arraysize(ex->val.type, 0);
        return cleansprintf(makeexpr_bicall_4("sprintf", tp_str255, makeexpr_var(tvar),
                                              makeexpr_string("%.*s"), ex2, makeexpr_addrstr(ex)));
    }
    else
    {
        if(ex->val.type->kind == TK_STRING)
...
Expr* makeexpr_unsprintfify(Expr* ex)
{
    ...
    if(debug > 2)
    {
        fprintf(outf, "makeexpr_unsprintfify(");
        dumpexpr(ex);
        fprintf(outf, ")\n");
    }
    if(!istempsprintf(ex))
...
int sprintflength(Expr* ex, int allownulls)
{
    int i;
    int len;
    ...
    for(i = 2; i < ex->nargs; i++)
    {
        if(!allownulls || ex->args[i]->val.type->kind != TK_CHAR || !checkconst(ex, 0))
        {
            return -1;
        }
    }
    ...
    for(i = 0; i < ex->args[1]->val.i; i++)
    {
        ...
        if(ex->args[1]->val.s[i] == '%')
        {
            ...
            if(ex->args[1]->val.s[i] != 'c' && ex->args[1]->val.s[i] != '%')
            {
                return -1;
            }
        }
...
Expr* makeexpr_concat(Expr* a, Expr* b, int usesprintf)
{
    ...
    if(debug > 2)
    {
        fprintf(outf, "makeexpr_concat returns ");
        dumpexpr(ex);
        fprintf(outf, "\n");
    }
    return ex;
...
Expr* makeexpr_dotq(Expr* ex, char* name, Type* type)
{
...
Expr* strmax_func(Expr* ex)
{
    ...
    if(ex->kind == EK_VAR && (mp = (Meaning*)ex->val.i)->kind == MK_CONST && mp->type == tp_str255
       && mp->val.type)
    {
        return makeexpr_long(mp->val.i);
    }
    if(ex->kind == EK_VAR && (mp = (Meaning*)ex->val.i)->kind == MK_VARPARAM && mp->type == tp_strptr)
    {
        if(mp->anyvarflag)
        {
    ...
    switch(type->kind)
    {
        ...
        case TK_SUBR:
            ...
            if(pasc)
            {
                ...
                if((type == tp_ubyte || type == tp_sbyte) && (which_lang == LANG_TURBO))
                {
                    return 1;
                }
                else
            ...
            else
            {
                ...
                if(type == tp_ushort || type == tp_sshort)
                {
        ...
        case TK_RECORD:
            if(pasc && type->meaning)
            {
                ...
                if(!strcmp(type->meaning->sym->name, "NA_LONGWORD"))
                {
                    return 4;
                }
...
Static Value eval_expr_either(Expr* ex, int pasc)
{
    ...
    switch(ex->kind)
    {
        ...
        case EK_VAR:
            mp = (Meaning*)ex->val.i;
            if(mp->kind == MK_CONST && (foldconsts != 0 || mp == mp_maxint || mp == mp_minint))
            {
                return mp->val;
            }
            break;
        ...
        case EK_DIV:
            ...
            if(val.type && ord_type(val.type)->kind == TK_INTEGER && val2.type
               && ord_type(val2.type)->kind == TK_INTEGER && val2.i)
            {
                val.i /= val2.i;
                val.type = tp_integer;
                return val;
...
Value eval_expr_consts(Expr* ex)
{
    Value val;
...
Value eval_expr_pasc(Expr* ex)
{
    return eval_expr_either(ex, 1);
}

int expr_is_const(Expr* ex)
{
    int i;
