...
Static int readrc(char* rcname, int need)
{
    ...
    rc = fopen(rcname, "r");
    complain("readrc: trying to open '%s' ... ", rcname);
    if(!rc)
    {
        complain("failed to open rcfile '%s'\n", rcname);
        if(need)
        {
            perror(rcname);
        }
    ...
    while(fgets(buf, 500, rc))
    {
        ...
        if(cp && *cp != '#')
        {
            //complain("readrc: cp=\"%s\" ...", cp);
            upc(cp);
            i = numparams;
            while(--i >= 0 && strcmp(rctable[i].name, cp) != 0)
            {
                ;
            }
            if(i >= 0)
...
Static void postrc()
{
    ...
    which_unix = UNIX_ANY;
    if(!strcmp(target, "CHIPMUNK") || !strcmp(target, "HPUX-300") || !strcmp(target, "SUN-68K")
    ...
    scharname = (*name_SCHAR) ?
                name_SCHAR :
                (signedchars == 1) ? "char" : (useAnyptrMacros == 1) ? "Signed char" : "signed char";
    ...
    else if(!strncmp(language, "BERK", 4) || !strcmp(language, "SUN"))
    {
    ...
    {
        if(*language && strcmp(language, "HP") != 0 && strcmp(language, "MODCAL") != 0)
        {
    ...
    if(pascalcasesens < 0)
    {
        pascalcasesens = (which_lang == LANG_MODULA) ? 2 : (which_lang == LANG_BERK) ? 3 : 0;
    }
    if(implementationmodules < 0)
    {
        implementationmodules
        = (which_lang == LANG_VAX || which_lang == LANG_APOLLO || which_lang == LANG_BERK) ? 1 : 0;
    }
    if(integer16 < 0)
    {
    ...
    {
        doublereals
        = (hpux_lang || which_lang == LANG_OREGON || which_lang == LANG_VAX || which_lang == LANG_TIP) ? 0 : 1;
    }
    if(pascalenumsize < 0)
    {
    ...
    if(nestedcomments < 0)
    {
        nestedcomments = (which_lang == LANG_TURBO || which_lang == LANG_MPW
                          || which_lang == LANG_UCSD || which_lang == LANG_BERK) ?
                         2 :
                         0;
    }
    if(importall < 0)
    ...
    if(seek_base < 0)
    {
        seek_base = (which_lang == LANG_TURBO || which_lang == LANG_MPW || which_lang == LANG_UCSD
                     || which_lang == LANG_TIP) ?
                    0 :
                    1;
    }
    ...
    {
        dollar_idents
        = (which_lang == LANG_OREGON || which_lang == LANG_VAX || which_lang == LANG_TIP) ? 1 : 0;
    }
    if(ignorenonalpha < 0)
    {
    ...
    if(defaultsetsize < 0)
    {
        defaultsetsize = (which_lang == LANG_VAX) ?
                         256 :
                         (which_lang == LANG_BERK) ? 128 : (which_lang == LANG_MPW) ? 2040 : 8192;
    }
    if(enumbyte < 0)
    {
    ...
    charname = (useAnyptrMacros) ? "Char" : (unsignedchar == 1) ? ucharname : (unsignedchar == 0) ? scharname : "char";
    if(!*memcpyname)
    {
        strcpy(memcpyname, "memcpy");
    }
...
Static void openlogfile()
{
    ...
    if(logfh)
    {
        ...
        fprintf(logfh, "Translated");
        uname = getenv("USER");
        if(uname)
        {
            fprintf(logfh, " by %s", uname);
        }
        time(&starting_time);
        fprintf(logfh, " on %s", ctime(&starting_time));
        fprintf(logfh, "\n\n");
    }
    else
    {
...
void closelogfile()
{
    ...
    if(logfh)
    {
        ...
        fprintf(logfh, "Total memory used: %ld bytes.\n", (long)sbrk(0));
#endif
        time(&ending_time);
        fprintf(logfh, "Processed %d source lines in %ld:%ld seconds.\n", inf_ltotal,
                (ending_time - starting_time) / 60, (ending_time - starting_time) % 60);
...
void usage()
{
    ...
    fprintf(stderr,
            ...
            " -h <path>    write header to <path>\n"
            " -a           produce ansi C\n"
            " -check       enable error checking\n"
            " -s <file>    read interface definitions from <file>\n"
            " -pn          display progress\n"
            " -c <file>    read configuration from <file>, or, if '-', use no configuration\n"
            " -V           be verbose\n"
            " -e           add pascal source surrounded by #ifdefs\n"
            " -L <lang>    specify language, i.e., VAX, TURBO\n"
            " -comp        "
            "\n");
    exit_failure();
}
...
int main(int argc, char** argv)
{
    ...
    *infname = 0;
    ...
    else if(!readrc("p2crc", 0))
    {
        readrc(".p2crc", 0);
    }
    if(!*codefnfmt)
    {
        fprintf(stderr, "Unable to find required system p2crc file (tried '%s')\n", format_s("%H/%s", "p2crc"));
        exit_failure();
    }
    codefname = codefnbuf;
    *codefname = 0;
    ...
    *hdrfname = 0;
    ...
    while(argc > 0)
    {
        if(**argv == '-' && (*argv)[1])
        {
            ...
            else if(argv[0][1] == 'B')
            {
                ...
                if(argc == 2 && strlen(argv[1]) > 2 && !strcmp(argv[1] + strlen(argv[1]) - 2, ".c"))
                {
    ...
    complain("before modulefnfmt: requested_module=\"%s\", codefname=\"%s\"", requested_module, codefname);
    if(requested_module && !*codefname)
    {
        complain("doing modulefnfmt ...");
        strcpy(codefname, format_ss(modulefnfmt, infname, requested_module));
    }
    if(*infname && strcmp(infname, "-") != 0)
    {
        if(strlen(infname) > 2 && !strcmp(infname + strlen(infname) - 2, ".c"))
        {
            fprintf(stderr, "What is wrong with this picture? (infname(%llu)=\"%s\")\n", strlen(infname), infname);
            fprintf(stderr, "maybe you meant to give the .pas file ...\n");
            exit_failure();
        }
        inf = fopen(infname, "r");
        if(!inf)
...
void dumptype(Type* tp)
{
    ...
    fprintf(outf, ", meaning=%lx, basetype=%lx, indextype=%lx\n", (long)tp->meaning,
            (long)tp->basetype, (long)tp->indextype);
    tp->dumped = 1;
    if(tp->basetype)
...
void dumpmeaning(Meaning* mp)
{
    ...
    fprintf(outf, "   Meaning %lx, name=%s, kind=%s", (long)mp, ((mp->name) ? mp->name : "<null>"),
            meaningkindname(mp->kind));
#ifdef HASDUMPS
    fprintf(outf, ", ctx=%lx, cbase=%lx, cnext=%lx, type=%lx\n", (long)mp->ctx, (long)mp->cbase,
            (long)mp->cnext, (long)mp->type);
...
void dumptypename(Type* tp, int waddr)
{
    ...
    if(tp == tp_int)
        ...
        fprintf(outf, "SC");
    else if(tp == tp_boolean)
        fprintf(outf, "B");
    else if(tp == tp_longreal)
        fprintf(outf, "R");
    else if(tp == tp_real)
        fprintf(outf, "F");
    else if(tp == tp_anyptr)
        fprintf(outf, "A");
    else if(tp == tp_void)
        fprintf(outf, "V");
    else if(tp == tp_text)
        fprintf(outf, "T");
    else if(tp == tp_bigtext)
        fprintf(outf, "BT");
    else if(tp == tp_sshort)
        fprintf(outf, "SS");
    else if(tp == tp_ushort)
        fprintf(outf, "US");
    else if(tp == tp_abyte)
        fprintf(outf, "AB");
    else if(tp == tp_sbyte)
        ...
        fprintf(outf, "UB");
    else if(tp == tp_str255)
        fprintf(outf, "S");
    else if(tp == tp_strptr)
        fprintf(outf, "SP");
    else if(tp == tp_charptr)
        fprintf(outf, "CP");
    else if(tp == tp_smallset)
        fprintf(outf, "SMS");
    else if(tp == tp_proc)
        fprintf(outf, "PR");
    else if(tp == tp_jmp_buf)
        fprintf(outf, "JB");
    else
    {
        if(tp->meaning && !ISBOGUS(tp->meaning) && tp->meaning->name && !ISBOGUS(tp->meaning->name)
           && tp->meaning->name[0])
        {
        ...
        switch(tp->kind)
        {
            ...
            case TK_ARRAY:
                ...
                if(tp->smax)
                {
                    ...
                    fprintf(outf, "{%d%s}", tp->escale, tp->issigned ? "S" : "U");
                }
                fprintf(outf, ":");
                dumptypename(tp->basetype, 0);
                break;
...
void dumptypename_file(f, tp) FILE* f;
Type* tp;
{
    FILE* save = outf;
    outf = f;
...
void dumpexpr_file(f, ex) FILE* f;
Expr* ex;
{
    FILE* save = outf;
    outf = f;
...
void dumpstmt_file(f, sp) FILE* f;
Stmt* sp;
{
    FILE* save = outf;
    Stmt* savenext = NULL;
...
void mem_summary()
{
    ...
    printf("Meanings =    %d of %d (%d of %d)\n", final_meanings, total_meanings,
           final_meanings / sizeof(Meaning), total_meanings / sizeof(Meaning));
...
anyptr test_malloc(int size, int* total, int* final)
{
    anyptr p;
...
anyptr test_realloc(anyptr p, int size)
{
    anyptr p2;
