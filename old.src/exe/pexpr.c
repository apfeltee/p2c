...
Expr* dots_n_hats(Expr* ex, Type* target)
{
    ...
    for(;;)
    {
        ...
        if(ex->val.type->kind == TK_FUNCTION && ex->val.type->meaning
...
Expr* p_funcarglist(Expr* ex, Meaning* args, int firstarg, int ismacro)
{
    Meaning* mp;
    Meaning* mp2;
    Meaning* arglist = args;
    Meaning* prevarg = NULL;
    Expr* ex2;
    int i;
    int fi;
    int fakenum = -1;
    ...
    while(args)
    {
        ...
        if(curtok == TOK_IDENT)
        {
            ...
            while(mp && strcmp(curtokbuf, mp->sym->name) != 0)
            {
                if(mp->fakeparam)
                {
                    if(fi < 0)
    ...
    switch(curtok)
    {
        ...
        case TOK_IN:
               ...
               && (tvar = istempvar(ex3->args[0])) != NULL && isconstexpr(ex3->args[1], &mask))
            {
                ...
                {
                    ex4 = makeexpr_or(
                    ex4, makeexpr_range(copyexpr(ex), makeexpr_char('a'), makeexpr_char('z'), 1));
                }
                if(mask & 0x2)
                {
                    ex4 = makeexpr_or(
                ...
                {
                    ex4 = makeexpr_or(
                    ex4, makeexpr_range(copyexpr(ex), makeexpr_char('0'), makeexpr_char('9'), 1));
                }
                freeexpr(ex);
