    ...
    while(ctx2 && ctx2 != ctx && ctx2->kind == MK_FUNCTION)
    {
        for(mp = ctx2->cbase; mp; mp = mp->cnext)
        {
            if(mp->kind == MK_VAR && isfiletype(mp->type, -1) && !mp->istemporary)
            {
                var_reference(mp);
                sp = makestmt_if(
                makeexpr_rel(EK_NE, filebasename(makeexpr_var(mp)), makeexpr_nil()),
...
#define addstmt(kind) *spp = sp = makestmt(kind), spp = &(sp->next)

#define newstmt(kind) \
    addstmt(kind), steal_comments(firstserial, sp->serial, sflags& SF_FIRST), sflags &= ~SF_FIRST
    ...
    switch(curtok)
    {
        ...
        case TOK_FOR:
            ...
            if(which_lang == LANG_TIP)
            {
                freeexpr(ep);
                mp2 = sym->mbase;
                if(mp2 && mp2->ctx == curctx && mp2->kind == MK_VAR && mp2->anyvarflag
            ...
            if(ep->kind == EK_VAR)
            {
                ...
                if((tp == tp_char || tp == tp_schar || tp == tp_uchar || tp == tp_abyte
                    || tp == tp_sbyte || tp == tp_ubyte || tp == tp_boolean)
                   && ((checkconst(sp->exp2, 0) && tp != tp_sbyte && tp != tp_schar)
                       || checkconst(sp->exp2, -128) || (checkconst(sp->exp2, 127) && tp != tp_ubyte && tp != tp_uchar)
                       || checkconst(sp->exp2, 255)
                       || (tp == tp_char && (useAnyptrMacros == 1 || unsignedchar != 1)
            ...
            if(ep3)
            {
                ...
                {
                    range = makeexpr_bin(EK_LSH, ep->val.type, makeexpr_longcast(makeexpr_long(1), 1),
            ...
            else
            {
                ...
                if((checkconst(forstep, 1) || checkconst(forstep, -1)) && sp->exp2->kind == EK_PLUS
                ...
                if(forevalorder && isunsafe)
                {
                    if(exprdepends(sp->exp2, ep))
                    {
                        ...
                        sp->exp1
                        = makeexpr_comma(makeexpr_comma(makeexpr_assign(makeexpr_var(tvar), copyexpr(ep2)),
                ...
                if(curtokmeaning->ctx != curctx)
                {
                    ...
                    *spp = close_files_to_ctx(curtokmeaning->ctx);
                    ...
                    sp->exp1 = makeexpr_bicall_2("longjmp", tp_void,
                                                 makeexpr_var(curtokmeaning->xnext), makeexpr_long(1));
                }
                else
                {
                    newstmt(SK_GOTO);
        ...
        case TOK_WITH:
            ...
            withlevel = i;
            if(mp)
            { /* if "with p^" for constant p, don't need temp ptr */
        ...
        case TOK_IDENT:
            ...
            if(!mp)
            {
                ...
                else if(curtok == TOK_HAT || curtok == TOK_ADDR || curtok == TOK_LBR || curtok == TOK_DOT)
                {
		    ...
		    wexpecttok(TOK_ASSIGN);
                ...
                if(curtok == TOK_ASSIGN)
                {
                    ...
                    { /* VAX Pascal foolishness */
                        ...
                        sp->exp1
                        = makeexpr_bicall_3("memset", tp_void, makeexpr_addr(ep), makeexpr_long(0), ep2);
                    }
                    else
                    {
                        sp->exp1 = makeexpr_assign(ep, p_expr(ep->val.type));
...
Static int usebraces(Stmt* sp, int opts)
{
    ...
    switch(bracesalways)
    {
        case 0:
            if(sp)
            {
                if(sp->next || sp->kind == SK_TRY || (sp->kind == SK_IF && !sp->stm2) || (opts & BR_REPEAT))
                {
                    return 1;
                }
            }
        ...
        default:
            if(sp)
            {
                if(sp->next || sp->kind == SK_IF || sp->kind == SK_WHILE || sp->kind == SK_REPEAT
                   || sp->kind == SK_TRY || sp->kind == SK_CASE || sp->kind == SK_FOR)
                {
                    return 1;
                }
            }
...
Static void out_block(Stmt* spbase, int opts, int serial)
{
    ...
    Strlist* curcmt;
    Strlist* cmt;
    Strlist* savecurcmt = curcomments;
    Strlist* trailcmt;
    Strlist* begincmt;
    Strlist* endcmt;
    ...
    if(opts & BR_ELSEPART)
    {
        ...
        if(cmt)
        {
            if(trailcmt)
            {
                out_spaces(bracecommentindent, commentoverindent, commentlen(cmt), 0);
                output("\001");
                outcomment(cmt);
            }
            else
    ...
    if(braces)
    {
        ...
        if(!line_start())
        {
            if(trailcmt && cur_column() + commentlen(trailcmt) + 2 > linewidth
               && outindent + commentlen(trailcmt) + 2 < linewidth)
            { /*close enough*/
                i = 0;
            }
    ...
    if(commentvisible(trailcmt))
    {
        ...
        {
            out_spaces(istrail ? commentindent : bracecommentindent, commentoverindent,
                       commentlen(trailcmt), delta);
            outcomment(trailcmt);
        } /*else {
            output("\n");
            singleindent(delta);
        ...
        switch(sp->kind)
        {
            ...
            case SK_RETURN:
                ...
                if(sp->exp1)
                {
                    switch(returnparens)
                    {
                        ...
                        default:
                            if(sp->exp1->kind == EK_VAR || sp->exp1->kind == EK_CONST || sp->exp1->kind == EK_LONGCONST
                               || sp->exp1->kind == EK_BICALL || sp->exp1->kind == EK_NAME)
                            {
                                output(" ");
                                out_expr(sp->exp1);
                            }
            ...
            case SK_IF:
                ...
                for(;;)
                {
                    ...
                    if(sp2->stm2)
                    {
                        ...
                        i = (!cmt && sp2->stm2->kind == SK_IF && !sp2->stm2->next
                             && ((sp2->stm2->exp2) ? checkconst(sp2->stm2->exp2, 1) : (elseif > 0)));
                        ...
                        if(i)
                        {
                            ...
                            if(sp2->stm1)
                            {
                                ...
                                changecomments(curcomments, CMT_POST, sp2->serial, CMT_PRE,
                                               sp2->stm1->serial);
                            }
                            if(cmtdebug)
                            {
            ...
            default:
                intwarning("out_block",
                           format_s("Misplaced statement kind %s [265]", stmtkindname(sp->kind)));
                break;
        }
    ...
    if(opts & BR_FUNCTION)
    {
        ...
        if(findcomment(curcomments, -1, -1) != NULL)
        { /* check for non-DONE */
            output("\n");
        }
...
Static int checkreturns(Stmt** spp, int nearret)
{
    ...
    if(debug > 2)
    {
        fprintf(outf, "checkreturns on:\n");
        dumpstmt(*spp, 5);
    }
    while((sp = *spp))
    {
        spnextreturn = (sp->next && sp->next->kind == SK_RETURN && sp->next->exp1
...
Expr* replaceexprexpr(Expr* ex, Expr* oldex, Expr* newex, int keeptype)
{
    ...
    {
        if(ex->val.type->kind == TK_POINTER && ex->val.type->basetype == oldex->val.type)
        {
            freeexpr(ex);
            return makeexpr_addr(copyexpr(newex));
        }
        if(oldex->val.type->kind == TK_POINTER && oldex->val.type->basetype == ex->val.type)
        {
            freeexpr(ex);
...
Stmt* mixassignments(Stmt* sp, Meaning* mp)
{
    ...
    if(sp->next && sp->kind == SK_ASSIGN && sp->exp1->kind == EK_ASSIGN
       && sp->exp1->args[0]->kind == EK_VAR && (!mp || mp == (Meaning*)sp->exp1->args[0]->val.i)
       && ord_type(sp->exp1->args[0]->val.type)->kind == TK_INTEGER
       && nodependencies(sp->exp1->args[1], 0) && sp->next->kind == SK_ASSIGN && sp->next->exp1->kind == EK_ASSIGN
       && (exprsame(sp->exp1->args[0], sp->next->exp1->args[0], 1) || (mp && mp->istemporary))
       && exproccurs(sp->next->exp1->args[1], sp->exp1->args[0]) == 1)
    {
        sp->next->exp1->args[1]
        = replaceexprexpr(sp->next->exp1->args[1], sp->exp1->args[0], sp->exp1->args[1], 1);
...
Static int isescape(Expr* ex)
{
    if(ex->kind == EK_BICALL
       && (!strcmp(ex->val.s, name_ESCAPE) || !strcmp(ex->val.s, name_ESCIO) || !strcmp(ex->val.s, name_ESCIO2)
           || !strcmp(ex->val.s, name_OUTMEM) || !strcmp(ex->val.s, name_CASECHECK)
           || !strcmp(ex->val.s, name_NILCHECK) || !strcmp(ex->val.s, "_exit") || !strcmp(ex->val.s, "exit")))
    {
        return 1;
    }
    if(ex->kind == EK_CAST)
...
Static int deadendblock(Stmt* sp, int breaks)
{
    ...
    return (sp->kind == SK_RETURN || sp->kind == SK_CASECHECK
            || ((sp->kind == SK_GOTO || sp->kind == SK_BREAK || sp->kind == SK_CONTINUE) && breaks)
...
int implies(Expr* c1, Expr* c2, int not1, int not2)
{
    ...
    {
        if(checkconst(c1->args[0], 1))
        { /* things like "flag = true" */
            return implies(c1->args[1], c2, not1, not2);
        }
        if(checkconst(c1->args[1], 1))
        {
    ...
    switch(c2->kind)
    {
        ...
        case EK_AND:
            if(not2)
            { /* c1 -> (!c2a || !c2b) */
                return (implies(c1, c2->args[0], not1, 1) || implies(c1, c2->args[1], not1, 1));
            }
            else
            { /* c1 -> (c2a && c2b) */
                return (implies(c1, c2->args[0], not1, 0) && implies(c1, c2->args[1], not1, 0));
            }
    ...
    switch(c1->kind)
    {
        ...
        case EK_AND:
            if(not1)
            { /* (!c1a || !c1b) -> c2 */
                return (implies(c1->args[0], c2, 1, not2) && implies(c1->args[1], c2, 1, not2));
            }
            else
            { /* (c1a && c1b) -> c2 */
                return (implies(c1->args[0], c2, 0, not2) || implies(c1->args[1], c2, 0, not2));
            }
...
Expr* print_func(Expr* ex)
{
    ...
    if((!strcmp(ex->val.s, "fprintf") || !strcmp(ex->val.s, "sprintf")) && ex->args[1]->kind == EK_CONST)
    {
        return ex->args[0];
    }
...
Expr* chg_printf(Expr* ex)
{
    ...
    else if(!strcmp(ex->val.s, "putc") || !strcmp(ex->val.s, "fputc") || !strcmp(ex->val.s, "fputs"))
    {
...
Expr* mix_printf(Expr* ex, Expr* ex2)
{
    ...
    if(debug > 2)
    {
        fprintf(outf, "chg_printf returns ");
        dumpexpr(ex);
        fprintf(outf, "\n");
    }
...
Static int findpropexpr(Expr* ex, int prop, int add)
{
    ...
    if(add && (j = propcheckexprokay(ex)) != 0 && numpropchecks < MAX_PROP_CHECKS)
    {
        flowexprs[i] = ex;
        flowprops[i] = prop;
        flowassigns[i] = NULL;
...
Static void findpropchecksexpr(Expr* ex)
{
    ...
    switch(ex->kind)
    {
        ...
        case EK_NAME:
        case EK_DOT:
            if(ex->val.type == tp_boolean || (ex->val.type && ex->val.type->kind == TK_POINTER))
            {
...
int proveexprprop(/* 0 = false, 1 = true, -1 = unknown */
                  Expr* ex,
                  Stmt* sp,
                  int prop)
{
...
Expr* flow_fixexpr(Expr* ex, Stmt* sp, int env)
{
    int i;
    ...
    switch(ex->kind)
    {
                    ...
                    if(exprsame(ex, flowexprs[i], 2))
                    {
                        switch(flowprops[i])
                        {
                            case PROP_ZERO:
                                if(sp->trueprops & (1L << i))
                                {
                                    return makeexpr_val(make_ord(ex->val.type, 0));
                                }
                                else if((sp->falseprops & (1L << i)) && ex->val.type == tp_boolean)
                                {
        ...
        case EK_ASSIGN:
            if(checkconst(ex->args[1], 0) && proveexprprop(ex->args[0], sp, PROP_ZERO) == 1)
            {
                ex = grabarg(ex, 1);
            }
            else if(checkconst(ex->args[1], 1) && ex->args[1]->val.type == tp_boolean
...
Static void checkpropzero(Expr* ex, long* truep, long* falsep, int nonzero)
{
    ...
    switch(ex->kind)
    {
        ...
        case EK_NE:
            if(checkconst(ex->args[1], 0))
            {
                checkpropzero(ex->args[0], truep, falsep, (ex->kind == EK_EQ) ? !nonzero : nonzero);
            }
...
Static void checkpropsexpr(Expr* ex, Stmt* sp, long* truep, long* falsep)
{
    ...
    if(ex->kind != EK_ASSIGN && ex->kind != EK_AND && ex->kind != EK_OR && ex->kind != EK_COND)
    {
        for(i = 0; i < ex->nargs; i++)
        {
    ...
    switch(ex->kind)
    {
        ...
        case EK_ASSIGN:
            ...
            if(ex1->kind == EK_VAR || ex1->kind == EK_NAME)
            {
                for(i = 0; i < numpropchecks; i++)
                {
                    ...
                    {
                        if((flowassigns[i] || ex1->val.type == tp_boolean) && !(globalprops & (1L << i)))
                        {
                            checkelimprops |= (1L << i);
                        }
                        flowassigns[i] = sp;
        ...
        case EK_SPCALL:
            ...
            for(; mp && i < ex->nargs; i++)
            {
                ...
                if(mp->kind == MK_VARPARAM && mp->type == tp_strptr && mp->anyvarflag)
                {
                    i++;
                }
                mp = mp->xnext;
        ...
        case EK_DOT:
            for(i = 0; i < numpropchecks; i++)
            {
                if(exprsame(ex, flowexprs[i], 2))
                {
                    if(flowprops[i] != PROP_INVALID && (*truep | *falsep) & (1L << i))
                    {
                        flowuseful = 1;
                    }
                    flowassigns[i] = NULL;
...
Static int checkelimexpr(Expr* ex, Expr* ex2)
{
    ...
    for(;;)
    {
        ...
        if(ex->kind == EK_DOT && ex3->kind == EK_DOT && exprsame(ex->args[0], ex3->args[0], 2))
        {
            return 0;
        }
        if(ex3->kind == EK_DOT)
...
Static int checkelims(Stmt* sp, Expr* ex, int depth, int last)
{
    ...
    switch(sp->kind)
    {
        ...
        case SK_ASSIGN:
            ...
            if(sp->exp1->kind == EK_ASSIGN && exprsame(sp->exp1->args[0], ex, 2))
            {
        ...
        case SK_FOR:
            ...
            if(sp->exp1 && sp->exp1->kind == EK_ASSIGN && exprsame(sp->exp1->args[0], ex, 2))
            {
                used = 0;
            }
            used |= checkelimexpr(sp->exp1, ex);
        ...
        case SK_IF:
            used = (checkelims(sp->stm1, ex, 0, used) | checkelims(sp->stm2, ex, 0, used));
            used |= checkelimexpr(sp->exp1, ex);
            break;
        ...
        for(i = 0; i < numpropchecks; i++)
        {
            ...
            switch(flowprops[i])
            {
                case PROP_ZERO:
                    ...
                    break;
                case PROP_INVALID:
                    fprintf(outf, "INVALID");
                    break;
                default:
                    fprintf(outf, "%d", flowprops[i]);
                    break;
            }
            if(globalprops & (1L << i))
            {
...
void fixblock(Stmt** spp, Stmt* thereturn)
{
    Stmt* sp;
    Stmt* sp1;
    Stmt* sp2;
    ...
    while((sp = *spp))
    {
        ...
        *spp = sp;
        ...
        else if(sp->next->kind == SK_RETURN || (sp->next->kind == SK_ASSIGN && isescape(sp->next->exp1)))
        {
            thisreturn = sp->next;
        }
        else
        ...
        switch(sp->kind)
        {
            ...
            case SK_ASSIGN:
                ...
                switch(sp->exp1->kind)
                {
                    ...
                    case EK_COND:
                        *spp = makestmt_if(sp->exp1->args[0], makestmt_call(sp->exp1->args[1]),
                    ...
                    case EK_ASSIGN:
                        if(sp->exp1->args[1]->kind == EK_COND && usecommas != 1)
                        {
                            *spp = makestmt_if(
                            ...
                            makestmt_assign(copyexpr(sp->exp1->args[0]), sp->exp1->args[1]->args[1]),
                            makestmt_assign(sp->exp1->args[0], sp->exp1->args[1]->args[2]));
                            (*spp)->next = sp->next;
                            continue;
                        }
                        ...
                        {
                            if(!strcmp(sp->exp1->args[1]->val.s, getfbufname) && buildreads == 1
                               && sp->next && sp->next->kind == SK_ASSIGN && sp->next->exp1->kind == EK_BICALL
                               && !strcmp(sp->next->exp1->val.s, getname)
                               && expr_has_address(sp->exp1->args[0])
                               && similartypes(sp->exp1->args[0]->val.type,
                                               filebasetype(sp->exp1->args[1]->args[0]->val.type))
                               && exprsame(sp->exp1->args[1]->args[0], sp->next->exp1->args[0], 1))
                            {
                                eatstmt(&sp->next);
                                ex = makeexpr_bicall_4("fread", tp_integer, makeexpr_addr(sp->exp1->args[0]),
                                                       makeexpr_sizeof(sp->exp1->args[1]->args[1], 0),
                                                       makeexpr_long(1), sp->exp1->args[1]->args[0]);
                                FREE(sp->exp1);
                                sp->exp1 = ex;
                                continue;
                            }
                            if(!strcmp(sp->exp1->args[1]->val.s, chargetfbufname) && buildreads != 0
                               && sp->next && sp->next->kind == SK_ASSIGN && sp->next->exp1->kind == EK_BICALL
                               && !strcmp(sp->next->exp1->val.s, chargetname)
                               && expr_has_address(sp->exp1->args[0])
                               && exprsame(sp->exp1->args[1]->args[0], sp->next->exp1->args[0], 1))
                            {
                                eatstmt(&sp->next);
                                strchange(&sp->exp1->args[1]->val.s, "getc");
                                continue;
                            }
                        }
                        break;
                    ...
                    case EK_BICALL:
                        if(!strcmp(sp->exp1->val.s, name_ESCAPE))
                        {
                            if(fixexpr_tryblock)
                            {
                                *spp = makestmt_assign(makeexpr_var(mp_escapecode), grabarg(sp->exp1, 0));
                                (*spp)->next = makestmt(SK_GOTO);
                                (*spp)->next->exp1 = makeexpr_name(
                                format_s(name_LABEL, format_d("try%d", fixexpr_tryblock)), tp_integer);
                        ...
                        else if(!strcmp(sp->exp1->val.s, name_ESCIO))
                        {
                            if(fixexpr_tryblock)
                            {
                                *spp = makestmt_assign(makeexpr_var(mp_escapecode), makeexpr_long(-10));
                                (*spp)->next
                                = makestmt_assign(makeexpr_var(mp_ioresult), grabarg(sp->exp1, 0));
                                (*spp)->next->next = makestmt(SK_GOTO);
                                (*spp)->next->next->exp1 = makeexpr_name(
                                format_s(name_LABEL, format_d("try%d", fixexpr_tryblock)), tp_integer);
                        ...
                        if(!strcmp(sp->exp1->val.s, putfbufname) && buildwrites == 1 && sp->next
                           && sp->next->kind == SK_ASSIGN && sp->next->exp1->kind == EK_BICALL
                           && !strcmp(sp->next->exp1->val.s, putname)
                           && exprsame(sp->exp1->args[0], sp->next->exp1->args[0], 1))
                        {
                            eatstmt(&sp->next);
                            if(!expr_has_address(sp->exp1->args[2])
                               || sp->exp1->args[2]->val.type != sp->exp1->args[1]->val.type)
                            {
                                tvar = maketempvar(sp->exp1->args[1]->val.type, name_TEMP);
                                sp2 = makestmt_assign(makeexpr_var(tvar), sp->exp1->args[2]);
                                sp2->next = sp;
                                *spp = sp2;
                                sp->exp1->args[2] = makeexpr_var(tvar);
                                freetempvar(tvar);
                            }
                            ex = makeexpr_bicall_4("fwrite", tp_integer, makeexpr_addr(sp->exp1->args[2]),
                                                   makeexpr_sizeof(sp->exp1->args[1], 0),
                                                   makeexpr_long(1), sp->exp1->args[0]);
                            FREE(sp->exp1);
                            sp->exp1 = ex;
                            continue;
                        }
                        if(!strcmp(sp->exp1->val.s, charputfbufname) && buildwrites != 0 && sp->next
                           && sp->next->kind == SK_ASSIGN && sp->next->exp1->kind == EK_BICALL
                           && !strcmp(sp->next->exp1->val.s, charputname)
                           && exprsame(sp->exp1->args[0], sp->next->exp1->args[0], 1))
                        {
                            eatstmt(&sp->next);
                            swapexprs(sp->exp1->args[0], sp->exp1->args[1]);
                            strchange(&sp->exp1->val.s, "putc");
                            continue;
                        }
                        if((!strcmp(sp->exp1->val.s, resetbufname) || !strcmp(sp->exp1->val.s, setupbufname))
                        ...
                        if(ex && sp->next && mixwritelns && sp->next->kind == SK_ASSIGN
                           && exprsame(ex, print_func(sp->next->exp1), 1)
                           && (printnl_func(sp->exp1) || printnl_func(sp->next->exp1)))
                        {
                            sp->exp1 = mix_printf(sp->exp1, sp->next->exp1);
                            eatstmt(&sp->next);
                            continue;
                        }
                        break;
            ...
            case SK_IF:
                ...
                if(thisreturn)
                {
                    if(thisreturn->kind == SK_WHILE)
                    {
                        if(usebreaks)
                        {
                            ...
                            if(sp->stm2)
                            {
                                ...
                                if(j >= breaklimit && i <= 2 && j > i * 2
                                   && ((implies(sp->exp1, thisreturn->exp1, 0, 1)
                                       ...
                                       || (sp1->kind == SK_ASSIGN && implies(sp1->exp1, thisreturn->exp1, 0, 1))))
                                {
                            ...
                            {
                                i = stmtcount(sp->stm1);
                                if(i >= breaklimit && implies(sp->exp1, thisreturn->exp1, 1, 1))
                                {
                    ...
                    else
                    {
                        if(usereturns)
                        {
                            ...
                            { /* if (x) foo; (return;)  =>  if (!x) return; foo; */
                ...
                if(checkconst(sp->exp1, 0) && (elimdeadcode > 1 || sp->quietelim))
                {
                    if(!sp->quietelim)
                    {
                ...
                if(checkconst(sp->exp1, 1) && (elimdeadcode > 1 || sp->quietelim))
                {
                    if(!sp->quietelim)
                    {
            ...
            case SK_WHILE:
                if(whilefgets && /* handle "while eof(f) do readln(f,...)" */
                   sp->stm1 && sp->stm1->kind == SK_ASSIGN && sp->stm1->exp1->kind == EK_BICALL
                   && !strcmp(sp->stm1->exp1->val.s, "fgets") && nosideeffects(sp->stm1->exp1->args[0], 1)
                   && nosideeffects(sp->stm1->exp1->args[1], 1) && nosideeffects(sp->stm1->exp1->args[2], 1))
                {
                    if((sp->exp1->kind == EK_NOT && sp->exp1->args[0]->kind == EK_BICALL && *eofname
                        && !strcmp(sp->exp1->args[0]->val.s, eofname)
                        && exprsame(sp->exp1->args[0]->args[0], sp->stm1->exp1->args[2], 1))
                       || (sp->exp1->kind == EK_EQ && sp->exp1->args[0]->kind == EK_BICALL
                           && !strcmp(sp->exp1->args[0]->val.s, "feof") && checkconst(sp->exp1->args[1], 0)
                           && exprsame(sp->exp1->args[0]->args[0], sp->stm1->exp1->args[2], 1)))
                    {
                        sp->stm1->exp1->val.type = tp_strptr;
                        sp->exp1 = makeexpr_rel(EK_NE, sp->stm1->exp1, makeexpr_nil());
            ...
            default:
                ...
                if(sp->next
                   && (sp->kind == SK_GOTO || sp->kind == SK_BREAK || sp->kind == SK_CONTINUE || sp->kind == SK_RETURN)
                   && !haslabels(sp->next))
                {
                ...
                else if(sp->kind == SK_RETURN && thisreturn && thisreturn->kind == SK_RETURN
...
int checkvarchangedexpr(Expr* ex, Meaning* mp, int addrokay)
{
    ...
    switch(ex->kind)
    {
        ...
        case EK_SPCALL:
            ...
            for(; i < ex->nargs && i < 16; i++)
            {
                ...
                if(mp3->kind == MK_PARAM
                   && (mp3->type->kind == TK_ARRAY || mp3->type->kind == TK_STRING || mp3->type->kind == TK_SET))
                {
                    safemask |= 1 << i;
                }
        ...
        case EK_VAR:
            ...
            {
                if((mp->type->kind == TK_ARRAY || mp->type->kind == TK_STRING || mp->type->kind == TK_SET)
        ...
        case EK_PLUS:
            if(addrokay)
            { /* to keep from being scared by pointer */
...
int checkvarchanged(Stmt* sp, Meaning* mp)
{
    ...
    if(mp->varstructflag || !mp->ctx || mp->ctx->kind != MK_FUNCTION || mp->volatilequal || alwayscopyvalues)
    {
        return 1;
    }
    while(sp)
    {
        if(/* sp->kind == SK_GOTO || */
           sp->kind == SK_LABEL || checkvarchanged(sp->stm1, mp) || checkvarchanged(sp->stm2, mp)
...
Expr* initfilevar(Expr* ex)
{
    ...
    {
        ex2 = copyexpr(ex);
        if(ex->kind == EK_VAR && (mp = (Meaning*)ex->val.i)->kind == MK_VAR
           && mp->ctx->kind != MK_FUNCTION && !is_std_file(ex) && literalfilesflag > 0
        ...
        return makeexpr_comma(makeexpr_assign(filebasename(ex), makeexpr_nil()),
                              makeexpr_assign(makeexpr_dotq(ex2, "name", tp_str255), makeexpr_string(name)));
    }

    return makeexpr_assign(ex, makeexpr_nil());
}

void initfilevars(Meaning* mp, Stmt*** sppp, Expr* exbase)
{
    Stmt* sp;
    Stmt* sp2;
    Type* tp;
    ...
    while(mp)
    {
        if((mp->kind == MK_VAR && mp->refcount > 0 && !mp->istemporary) || mp->kind == MK_FIELD)
        {
            tp = mp->type;
            if(isfiletype(tp, -1))
            {
            ...
            else if(tp->kind == TK_ARRAY)
            {
                ...
                if(isfiletype(tp, -1))
                {
                    note(format_s("Array of files %s should be initialized [257]", mp->name));
                }
            }
        }
        mp = mp->cnext;
...
Static void findinits(Stmt* sp, int depth, int okay)
{
    ...
    while(sp)
    {
        ...
        switch(sp->kind)
        {
            ...
            case SK_ASSIGN:
            case SK_FOR:
                if(sp->exp1 && sp->exp1->kind == EK_ASSIGN && sp->exp1->args[0]->kind == EK_VAR
                   && (sp->kind == SK_ASSIGN || useinits > 2))
                {
                    mp = (Meaning*)sp->exp1->args[0]->val.i;
                    findinitsexpr(sp->exp1->args[1]);
                    if(mp->kind == MK_VAR && mp->ctx == curctx && !mp->fakeparam
                       && !mp->varstructflag && !mp->constdefn && !mp->isforward && !mp->isfunction)
                    {
                        for(i = 0; i < depth; i++)
                        {
                        ...
                        {
                            mp->fakeparam = 1;
                            if(depth == 0 && sp->kind == SK_ASSIGN && sprev && !mp->wasdeclared
                               && (isconstantexpr(sp->exp1->args[1])
                                   || (sp->exp1->args[1]->kind == EK_VAR && sofar >= 0 && mp->type->kind != TK_RECORD
                               ...
                               && (useinits < 3 || tinyexpr(sp->exp1->args[1])))
                            {
                                ...
                                sofar++;
                                    ...
                                    || (useinits < 3
                                        && (sofar < 0 || depth > 0 || mp->ctx->kind == MK_MODULE
...
Static Stmt* p_body()
{
    ...
    if(verbose)
    {
        fprintf(logfh, "%s, %d/%d: Translating %s (in %s)\n", infname, inf_lnum, outf_lnum,
        ...
        for(mp = curctx->type->fbase; mp; mp = mp->xnext)
        {
            ...
            else if(mp->othername)
            {
                if(checkvarchanged(spbody, mp))
                {
                    ...
                    makeexpr_var(mp), makeexpr_hat(makeexpr_name(mp->othername, mp->rectype), 0));
                    mp->refcount++;
                }
                else
...
Static void out_function(Meaning* func)
{
    ...
    {
        if(mp->othername && strcmp(mp->name, mp->othername) != 0)
        {
...
Static void p_function(int isfunc)
{
    ...
    if((sl = strlist_find(attrlist, "OPTIMIZE")) != NULL && sl->value != -1
    ...
    if(blockkind == TOK_IMPORT)
    {
        ...
        if(curtok == TOK_IDENT
           && (!strcicmp(curtokbuf, "FORWARD") || strlist_cifind(externwords, curtokbuf)
    ...
    else
    {
        ...
        if(curtok == TOK_IDENT && !strcicmp(curtokbuf, "FORWARD"))
        {
            ...
            if(func->ctx->kind != MK_FUNCTION)
            {
                ...
                && (strlist_cifind(externwords, curtokbuf) || strlist_cifind(cexternwords, curtokbuf)))
        {
    ...
    do
    {
        ...
        if(found == 1)
        {
            if(!inheader)
            {
                ...
                name = (sl) ?
                       (char*)sl->value :
                       format_ss(*headerfnfmt2 ? headerfnfmt2 : headerfnfmt, infname, curtokmeaning->name);
        ...
        else if(found == 0)
        {
            ...
            if(!inheader)
            {
                out_include(format_ss(*headerfnfmt2 ? headerfnfmt2 : headerfnfmt, sym->name, sym->name),
                            quoteincludes);
            }
        }
        gettok();
...
void do_include(Token blkind)
{
    ...
    saveoldfile(fname);
    flush_outfilebuf();
    complain("*** in do_include: fopen(\"%s\")\n", fname);
    outf = fopen(fname, "w");
    if(!outf)
    {
        outf = oldfile;
...
void p_block(Token blkind)
{
    ...
    for(;;)
    {
        ...
        if(curtok == TOK_CONST || curtok == TOK_TYPE || curtok == TOK_VAR || curtok == TOK_VALUE
...
Static int p_module(int ignoreit, int isdefn /* Modula-2: 0=local module, 1=DEFINITION, 2=IMPLEMENTATION */
)
{
    Meaning* mod;
    Meaning* mp;
...
int p_search(char* fname, char* ext, int need)
{
    ...
    if(!fp)
    {
        ...
        if(logfh)
        {
            fprintf(logfh, "(Unable to open search file \"%s\")\n", infnbuf);
        }
        return 0;
    }
    flushcomments(NULL, -1, -1);
    ...
    do
    {
        ...
        if(verbose)
        {
            fprintf(logfh, "%s, %d/%d: Reading import text for \"%s\"\n", infname, inf_lnum, outf_lnum, mod->name);
        }
        pushctx(mod);
        gettok();
        skipunitheader();
...
void p_program()
{
    ...
    switch(curtok)
    {
        ...
        default:
            ...
            prog->anyvarflag = 1;
            if(requested_module && strcicmp(requested_module, prog->name) && strcicmp(requested_module, "program"))
            {
                for(;;)
                {
                    skiptomodule();
            ...
            if(curtok != TOK_EOF)
            {
                ...
                if(!nullbody)
                {
                    ...
                    if(fullprototyping > 0)
                    {
                        output(format_sss("main%s(int argc,%s%s *argv[])", spacefuncs ? " " : "",
                                          spacecommas ? " " : "", charname));
                    }
                    else
                    {
                        output("main");
