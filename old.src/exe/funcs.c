...
Static Expr* func_abs()
{
    ...
    if(lness > 0 && *absname)
    {
        ...
        if(*absname == '*' && (exprspeed(ex) >= 5 || !nosideeffects(ex, 0)))
        {
...
Static Expr* checkfilename(Expr* nex)
{
    ...
    {
        if(*filenamefilter && strcmp(filenamefilter, "0") != 0)
        {
...
{
    if(ex->nargs == 1)
    { /* Kludge for Oregon Software Pascal's delete(f) */
...
Static Stmt* proc_escape()
{
    ...
    if(which_lang == LANG_TIP)
    {
        ...
        if(curtokmeaning && curtokmeaning == curctx && curctx->kind == MK_FUNCTION)
        {
...
Static Stmt* proc_get()
{
    ...
    if(isfiletype(type, -1) && *arraygetname && filebasetype(type)->kind == TK_ARRAY)
    {
...
Static Stmt* proc_put()
{
    ...
    if(isfiletype(type, -1) && *arrayputname && filebasetype(type)->kind == TK_ARRAY)
    {
...
Static Stmt* skipeoln(Expr* fex)
{
    ...
    if(!strcmp(readlnname, "scanf") || !*readlnname)
    {
        if(checkeof(fex))
        {
