...
Meaning* makespecialproc(char* name, Stmt* (*handler)())
{
    Meaning* mp;
...
Meaning* makestandardproc(char* name, Stmt* (*handler)())
{
    Meaning* mp;
...
Meaning* makespecialfunc(char* name, Expr* (*handler)())
{
    Meaning* mp;
...
Meaning* makestandardfunc(char* name, Expr* (*handler)())
{
    Meaning* mp;
...
Meaning* makespecialvar(char* name, Expr* (*handler)())
{
    Meaning* mp;
...
void setupmeaning(Meaning* mp, Symbol* sym, enum meaningkind kind, enum meaningkind namekind)
{
    ...
    if(*externalias && !my_strchr(externalias, '%'))
    {
        ...
        i = numparams;
        while(--i >= 0 && strcmp(rctable[i].name, "ALIAS") != 0)
        {
            ;
        }
        if(i < 0 || !undooption(i, ""))
...
Expr* filenamepart(Expr* ex)
{
    ...
    if((mp = isfilevar(ex)) && mp->namedfile)
    {
        if(mp->kind == MK_FIELD)
        {
...
Type* promote_type(Type* tp)
{
       ...
       || tp->kind == TK_BOOLEAN)
    {
        ...
        if(tp2 == tp_sbyte || tp2 == tp_ubyte || tp2 == tp_abyte || tp2 == tp_char
    ...
    for(tp = type; tp && tp != basetype; tp = tp->basetype)
    {
        switch(tp->kind)
        {
            ...
            case TK_ARRAY:
                ...
                if(tp->structdefd)
                { /* conformant array */
                    if(!variablearrays && !(tp->basetype->kind == TK_ARRAY && tp->basetype->structdefd))
                    { /* avoid mult. notes */
...
Type* canonicaltype(Type* type)
{
    ...
    if(type->kind == TK_POINTER)
    {
        ...
        if(type->basetype->kind == TK_ARRAY || type->basetype->kind == TK_STRING || type->basetype->kind == TK_SET)
        {
...
void makevarstruct(Meaning* func)
{
    ...
    if(!flag)
    { /* Avoid generating an empty struct */
...
void ignore_attributes()
{
    while(attrlist)
    {
        if(strcmp(attrlist->s, "HIDDEN") != 0 && strcmp(attrlist->s, "INHERIT")
           && strcmp(attrlist->s, "ENVIRONMENT") != 0)
        {
...
Meaning* maketempvar(Type* type, char* name)
{
    ...
    while((tv = *tvp)
          && (!similartypes(tv->tvar->type, type) || tv->tvar->refcount == 0
              || strcmp(tv->tvar->val.s, name) != 0))
    {
        tvp = &(tv->next);
    }
    if(!tv)
    {
        ...
        while((tv = *tvp) && (tv->tvar->refcount > 0 || strcmp(tv->tvar->val.s, name) != 0))
        {
            tvp = &(tv->next);
        }
    }
