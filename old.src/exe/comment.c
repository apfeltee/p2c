...
{
    return (cmt && getcommentkind(cmt) != CMT_DONE
...
Strlist* fixbeginendcomment(Strlist* cmt)
{
    ...
    if(!strcincmp(cp, "procedure ", 10))
    { /* remove "PROCEDURE" keyword */
        strcpy(cp, cp + 10);
    }
...
void addnote(char* msg, long serial)
{
    ...
    Strlist* sl;
    Strlist* base = NULL;
    Strlist** pbase = (defer) ? &curcomments : &base;
    char* prefix;
    /*
    static const char formatstyle_defer[] = "\"%s\", line %d: %s\n";
    static const char formatstyle_outf[]  = "\"%s\", line %d,%d: %s\n";
    */
    static const char formatstyle_defer[] = "at \"%s\", line %d: %s\n";
    static const char formatstyle_outf[]  = "at \"%s\", line %d,%d: %s\n"; 
    ...
    if(!quietmode)
    {
        if(defer)
        {
            if(outf == stdout)
            {
                fprintf(stderr, formatstyle_defer, infname, inf_lnum, msg);
            }
            else
            {
                printf(formatstyle_defer, infname, inf_lnum, msg);
            }
        }
        else if(outf != stdout)
        {
            printf(formatstyle_outf, infname, inf_lnum, outf_lnum, msg);
        }
    }
    if(verbose)
    {
...
Strlist* grabcomment(int kind)
{
    ...
    *cp2 = 0;
    if(kind == CMT_ONBEGIN || kind == CMT_ONEND)
    {
        ...
        {
            *cp = 0;
            if(!strcicmp(cp3, "procedure") || !strcicmp(cp3, "function") || !strcicmp(cp3, "if")
               || !strcicmp(cp3, "else") || !strcicmp(cp3, "for") || !strcicmp(cp3, "while")
               || !strcicmp(cp3, "with") || !strcicmp(cp3, "case")
               || (curctx && !strcicmp(cp3, curctx->name) && kind == CMT_ONBEGIN))
            {
                grabbed_comment = 1;
                return NULL;
            }
...
void changecomments(Strlist* cmt, int okind, long ostamp, int kind, long stamp)
{
    while(cmt)
    {
        if(matchcomment(cmt, okind, ostamp))
        {
            if(cmtdebug > 1)
            {
                fprintf(outf, "Changed comment [%s:%ld] \"%s\" ", CMT_NAMES[getcommentkind(cmt)],
                        cmt->value & CMT_MASK, cmt->s);
            }
            if(kind >= 0)
            {
