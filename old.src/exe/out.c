...
#define ISBREAK(ch) \
    ((ch) == '\001' || (ch) == '\002' || (ch) == '\006' || (ch) == '\011' || (ch) == '\017')

#define putc_outf(ch) \
    ((outfilebufptr == outfilebufend) ? grow_outfilebuf() : 0, *outfilebufptr++ = (ch))
...
Static int deltaindent;
Static int thisindent;
Static int thisfutureindent;
Static int sectionsize;
Static int blanklines;
Static int codesectsize;
Static int hdrsectsize;
Static int codelnum;
Static int hdrlnum;

Static int numbreaks, bestnumbreaks;
Static double bestbadness;
Static int breakpos[MAXBREAKS];
Static int breakindent[MAXBREAKS];
Static int breakcount[MAXBREAKS];
Static int breakparen[MAXBREAKS];
Static int bestbreakpos[MAXBREAKS];
Static int bestbreakindent[MAXBREAKS];
Static int breakerrorflag;

Static int numedits;
Static int bestnumedits;
Static int editpos[MAXEDITS];
Static int besteditpos[MAXEDITS];
Static char editold[MAXEDITS];
Static char editnew[MAXEDITS];
Static char besteditold[MAXEDITS];
Static char besteditnew[MAXEDITS];

Static Paren* parenlist;

Static long numalts;
Static long bestnumalts;
Static int randombreaks;

Static char* outbuf;
Static char* outfilebuf;
Static char* outfilebufptr;
Static char* outfilebufend;
Static int outbufpos;
Static int outbufcount;
Static int outbufsize;
Static int outfilebufsize;
Static int suppressnewline;
...
int numbreaks, const int* breakpos, int* breakindent, int numedits, const int* editpos, char* editold, const char* editnew)
{
    unsigned char ch;
    unsigned char ch2;
    char* cp;
...
#define randtest(lim)                                                           \
    (!randombreaks ? -1 :                                                       \
                     randombreaks > 0 ? parencount(parens) < randombreaks - 1 : \
                                        randombreaks == -2 ? 0 : (rand() & 0xfff) < (lim))
...
Static int trybreakline(int pos, int count, int indent, double badness, int flags, Paren* parens)
{
    ...
    for(;;)
    {
        ...
        if((testinglinebreaker > 1 && showbadlimit > 0) ? (badness + extra >= showbadlimit) :
		...
		while (pos <= pos2) {
		    ...
		    pos++;
                ...
                if(r != 1)
                {
                    j &= trybreakline(pos, count, indent + extraindent,
                                      badness + MAX(extraindentpenalty, 0), flags | TB_EXTRAIND, parens);
                }
            ...
            case '\017': /* "break point for final : operator" */
                ...
                else
                {
                    if(ch == '\011')
                    {
                        i = breakindent[numbreaks - 1] + count - breakcount[numbreaks - 1] + 2;
                    }
                    else
                    {
                        i = indent;
                    ...
                    {
                        j = trybreakline(pos, count, i, badness, f, parens) & jmask;
                ...
                switch(ch)
                {
                    ...
                    case '\017':
                        ...
                        else if(((outbuf[pos] == '&' || outbuf[pos] == '|') && outbuf[pos + 1] == outbuf[pos])
                                    ...
                                    && outbuf[pos - 3] == outbuf[pos - 2]))
                        {
                            penalty += logbreakpenalty + logbreakextrapenalty * i;
                                ...
                                || ((outbuf[pos] == '=' || outbuf[pos] == '!') && outbuf[pos + 1] == '=')
    ...
    {
        if(badness >= bestbadness && (badness < showbadlimit || showbadlimit == 0))
        {
        ...
        if((bestbadness < showbadlimit || showbadlimit == 0) && bestnumalts > 0)
        {
            fprintf(outf, "\n#if 0   /* rejected #%ld, badness = %g > %g */\n", bestnumalts, bestbadness, badness);
            flush_outbuf(bestnumbreaks, bestbreakpos, bestbreakindent, bestnumedits, besteditpos,
        ...
        if(ch == '\n' || ch == '\r')
        {
            ...
            if(thisindent + outbufcount >= linewidth && !dontbreaklines)
            {
                ...
                if(thisindent + outbufcount > linewidth * 3 / 2)
                {
                    ...
                    maxdp = maxdepth;
                    for(;;)
                    { /* try some simple fixed methods first... */
                        for(i = 1; i <= 20; i++)
                        {
                            randombreaks = -1;
                ...
                flush_outbuf(bestnumbreaks, bestbreakpos, bestbreakindent, bestnumedits,
                             besteditpos, besteditold, besteditnew);
                ...
                {
                    fprintf(logfh, "%s, %d/%d: Line breaker spent %ld tries\n", infname, inf_lnum,
                            outf_lnum, alts);
                }
                if(testinglinebreaker)
                {
...
int isembedcomment(Strlist* cmt)
{
    ...
    return (cmt && len > 0 && !strncmp(cmt->s, embedcomment, len)
            && (isspace(cmt->s[len])
                || (!cmt->s[len] && cmt->next && (*cmt->next->s == '\002' || *cmt->next->s == '\003'))));
}
...
char* makeCstring(char* s, int len)
{
    if(s == NULL)
    {
        fprintf(stderr, "makeCstring: s is NULL!!!\n");
    }
    if(len <= 0)
    {
        fprintf(stderr, "makeCstring: len (%d) is <= 0!!!\n", len);
    }
