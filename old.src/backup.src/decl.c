...
Meaning* makespecialproc(char* name, Stmt* (*handler)(void))
{
    Meaning* mp;
...
Meaning* makestandardproc(char* name, Stmt* (*handler)(void))
{
    Meaning* mp;
...
Meaning* makespecialfunc(char* name, Expr* (*handler)(void))
{
    Meaning* mp;
...
Meaning* makestandardfunc(char* name, Expr* (*handler)(void))
{
    Meaning* mp;
...
Meaning* makespecialvar(char* name, Expr* (*handler)(void))
{
    Meaning* mp;
...
void setup_decl(void)
{
    ...
    tp_integer
    = makestandardtype(TK_INTEGER, makestandardmeaning(MK_TYPE, (integer16) ? "LONGINT" : "INTEGER"));
    ...
    {
        tp_int
        = makestandardtype(TK_INTEGER, (integer16 > 1) ? makestandardmeaning(MK_TYPE, "INTEGER") : NULL);
    ...
    tp_sshort
    = makestandardtype(TK_SUBR, makestandardmeaning(MK_TYPE, (integer16 == 1) ? "INTEGER" : "SWORD"));
    ...
    mp->type = makesettype(makesubrangetype(tp_integer, makeexpr_long(0), makeexpr_long(setbits - 1)));
    mp->type->meaning = mp;

    mp = makestandardmeaning(MK_TYPE, "INTSET");
    mp->type
    = makesettype(makesubrangetype(tp_integer, makeexpr_long(0), makeexpr_long(defaultsetsize - 1)));
    mp->type->meaning = mp;
    ...
    mp_maxint->name = stralloc((integer16) ? "SHORT_MAX" : (sizeof_int >= 32) ? "INT_MAX" : "LONG_MAX");
    ...
    mp_minint->name = stralloc((integer16) ? "SHORT_MIN" : (sizeof_int >= 32) ? "INT_MIN" : "LONG_MIN");
...
void activatemeaning(Meaning* mp)
{
    Meaning* mp2;
...
void forget_ctx(Meaning* ctx, int all)
{
    ...
    {
        if(all || (mp->kind != MK_PARAM && mp->kind != MK_VARPARAM))
        {
            *mpprev = mp->cnext;
            mpp2 = &mp->sym->mbase;
...
void handle_nameof(void)
{
    ...
    for(sl = nameoflist; sl; sl = sl->next)
    {
        ...
        if(cp)
        {
            ...
            sl2 = strlist_add(&sp->symbolnames, format_ds("%.*s", (int)(cp - sl->s), sl->s));
        }
        else
        {
...
int issafename(Symbol* sp, int isglobal, int isdefine, int isfield)
{
    Meaning* mp;
    ...
    if((sp->flags & AVOIDNAME) || (isdefine && (sp->flags & AVOIDFIELD)) || (isglobal && (sp->flags & AVOIDGLOB)))
        return 0;
    else
        return 1;
}
...
void setupmeaning(Meaning* mp, Symbol* sym, enum meaningkind kind, enum meaningkind namekind)
{
    ...
    if(sym->mbase && sym->mbase->ctx == curctx && curctx != NULL && !silentalreadydef && !distinctdef)
        alreadydef(sym);
    mp->sym = sym;
    mp->snext = sym->mbase;
    sym->mbase = mp;
    ...
    if(pascalcasesens && curctx && curctx->sym && kind != MK_SYNONYM
       && strlist_cifind(permimports, curctx->sym->name))
    { /* a built-in name */
        Meaning* mp2;
    ...
    if(kind == MK_VAR || kind == MK_VARREF || kind == MK_VARMAC || kind == MK_TYPE
       || kind == MK_CONST || kind == MK_FUNCTION)
    {
    ...
    isglobal = (!curctx || curctx->kind != MK_FUNCTION || namekind == MK_FUNCTION
                || namekind == MK_TYPE || namekind == MK_VARIANT || isdefine)
               && (curctx != nullctx);
    ...
    do
    {
        ...
        if(*editfmt)
            if(editfmt == enumformat)
                cp = format_ss(editfmt, cp, enum_tname ? enum_tname->name : "ENUM");
            else
                cp = format_ss(editfmt, cp, curctx ? curctx->name : "");
        if(dollar_idents == 2)
        {
            for(cp2 = cp; *cp2; cp2++)
                if(*cp2 == '$' || *cp2 == '%')
    ...
    } while(!issafename(sym2, isglobal, isdefine, 0) && namekind != MK_MODULE && !wasaliased);
    ...
    {
        switch(namekind)
        { /* prevent further name conflicts */
...
Meaning* addmeaningas(Symbol* sym, enum meaningkind kind, enum meaningkind namekind)
{
    Meaning* mp;
...
Meaning* addmeaning(Symbol* sym, enum meaningkind kind)
{
    return addmeaningas(sym, kind, kind);
}

Meaning* addmeaningafter(Meaning* mpprev, Symbol* sym, enum meaningkind kind)
{
    Meaning* mp;
...
Meaning* addfield(Symbol* sym, Meaning*** flast, Type* rectype, Meaning* tname)
{
    Meaning* mp;
    int altnum;
    Symbol* sym2;
    ...
    if(sym)
    {
        ...
        name = name2;
        if(tname)
            sl = strlist_find(fieldmacros, format_ss("%s.%s", tname->sym->name, sym->name));
        else
            sl = NULL;
        if(sl)
        {
        ...
        else
        {
            ...
            do
            {
                altnum++;
                if(*fieldformat)
                    name = format_ss(fieldformat, name2, tname && tname->name ? tname->name : "FIELD");
...
int isfiletype(Type* type, int big /* 0=TK_FILE, 1=TK_BIGFILE, -1=either */
)
{
    return ((type->kind == TK_POINTER && type->basetype->kind == TK_FILE && big != 1)
            || (type->kind == TK_BIGFILE && big != 0));
}

int israndomfile(Type* type)
{
    if(type->kind == TK_POINTER)
        type = type->basetype;
    return ((type->kind == TK_FILE || type->kind == TK_BIGFILE) && type->issigned);
}

Meaning* isfilevar(Expr* ex)
{
    Meaning* mp;
...
Expr* filenamepart(Expr* ex)
{
    ...
    if(ex->val.type->kind == TK_BIGFILE)
        ...
        if(mp->kind == MK_FIELD)
            return makeexpr_dotq(copyexpr(ex->args[0]), format_s(name_FNVAR, mp->name), tp_str255);
        else
...
int fileisbuffered(Expr* ex, int maybe)
{
    Meaning* mp;

    return (ex->val.type->kind == TK_BIGFILE || ((mp = isfilevar(ex)) && (maybe || mp->bufferedfile)));
}

Type* findbasetype_(Type* type, int flags)
{
    long smin, smax;
    ...
    for(;;)
    {
        if(type->preserved && (type->kind != TK_POINTER) && !(flags & ODECL_NOPRES))
            return type;
        switch(type->kind)
        {
            ...
            case TK_POINTER:
                ...
                switch(type->basetype->kind)
                {
                    ...
                    case TK_SET: /*  => int (*a)[3]; */
                        if(stararrays == 1 || !(flags & ODECL_FREEARRAY) || type->basetype->structdefd)
                        {
                ...
                if(type->preserved && !(flags & ODECL_NOPRES))
                    return type;
                if(type->fbase && type->fbase->wasdeclared && type->basetype->kind != TK_POINTER
                   && (flags & ODECL_DECL))
                {
            ...
            case TK_ARRAY:
                if(type->meaning && type->meaning->kind == MK_TYPE && type->meaning->wasdeclared)
                    return type;
                if(type->fbase && type->fbase->wasdeclared && (flags & ODECL_DECL))
                {
                    typename.meaning = type->fbase;
                    typename.preserved = 1;
                    return &typename;
            ...
            case TK_ENUM:
                if(useenum)
                    return type;
                else if(!enumbyte || type->smax->kind != EK_CONST || type->smax->val.i > 255)
                    return tp_sshort;
            ...
            case TK_SUBR:
                if(type == tp_abyte || type == tp_ubyte || type == tp_sbyte || type == tp_ushort || type == tp_sshort)
                {
                    return type;
                }
                    ...
                    if(ord_range(type, &smin, &smax))
                    {
                        if(squeezesubr != 0)
                        {
                            if(smin >= 0 && smax <= max_schar)
                                ...
                                return tp_ubyte;
                            else if(smin >= min_schar && smax <= max_schar && (signedchars == 1 || hassignedchar))
                                return tp_sbyte;
            ...
            case TK_CHAR:
                if(type == tp_schar && (signedchars != 1 && !hassignedchar))
                {
                    return tp_sshort;
                }
                return type;
...
Type* findbasetype(Type* type, int flags)
{
    if(debug > 1)
    {
...
Expr* arraysize(Type* tp, int incskipped)
{
    Expr *ex, *minv, *maxv;
    int denom;

    ord_range_expr(tp->indextype, &minv, &maxv);
    if(maxv->kind == EK_VAR && maxv->val.i == (long)mp_maxint && !exprdependsvar(minv, mp_maxint))
    {
        return NULL;
    }
    else
    {
        ex = makeexpr_plus(makeexpr_minus(copyexpr(maxv), copyexpr(minv)), makeexpr_long(1));
        ...
        if(tp->smax)
        {
            ...
            denom >>= tp->escale;
            ex = makeexpr_div(makeexpr_plus(ex, makeexpr_long(denom - 1)), makeexpr_long(denom));
        }
        return ex;
    }
}

Type* promote_type(Type* tp)
{
    Type* tp2;

    if(tp->kind == TK_ENUM)
    {
        if(promote_enums == 0 || (promote_enums < 0 && (useenum)))
            return tp;
    }
    if(tp->kind == TK_ENUM || tp->kind == TK_SUBR || tp->kind == TK_INTEGER || tp->kind == TK_CHAR
       || tp->kind == TK_BOOLEAN)
    {
        tp2 = findbasetype(tp, ODECL_NOPRES);
        if(tp2 == tp_ushort && sizeof_int == 16)
            return tp_uint;
        else if(tp2 == tp_sbyte || tp2 == tp_ubyte || tp2 == tp_abyte || tp2 == tp_char
                || tp2 == tp_sshort || tp2 == tp_ushort || tp2 == tp_boolean || tp2->kind == TK_ENUM)
        {
            return tp_int;
        }
    }
...
Type* promote_type_bin(Type* t1, Type* t2)
{
    t1 = promote_type(t1);
    t2 = promote_type(t2);
    ...
    {
        if((t1 == tp_uint || t2 == tp_uint) && sizeof_int > 0 && sizeof_int < (sizeof_long > 0 ? sizeof_long : 32))
            return tp_uint;
        return tp_integer;
    }
...
#if 0
void 
predeclare_varstruct (Meaning *mp)
{
    if (mp->ctx &&
...
Static void declare_args(Type* type, int isheader, int isforward)
{
    ...
    dopromote = (promoteargs == 1 || (promoteargs < 0 && (usePP || !fullprototyping)));
    ...
    staticlink
    = (type->issigned
       || (type->meaning && type->meaning->ctx->kind == MK_FUNCTION && type->meaning->ctx->varstructflag));
    if(mp || staticlink)
    {
        if(usePP)
            output(" PP(");
        ...
        if(showtypes || shownames)
        {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        outdeclarator(tp, name,
                                      ODECL_CHARSTAR | ODECL_FREEARRAY | ODECL_SPACE
            ...
            if(staticlink)
            { /* sub-procedure with static link */
                if(firstflag++)
                    if(spacecommas)
                        output(",\002 ");
                    else
...
void outdeclarator(Type* type, char* name, int flags)
{
    ...
    for(tp = type; tp && tp != basetype; tp = tp->basetype)
    {
        switch(tp->kind)
        {
            ...
            case TK_POINTER:
                if(tp->basetype)
                {
                    switch(tp->basetype->kind)
                    {
                        ...
                        case TK_VOID:
                            if(tp->basetype == tp_void && tp_special_anyptr)
                            {
                        ...
                        case TK_SET:
                            if(stararrays == 1 || !(flags & ODECL_FREEARRAY)
                               || (tp->basetype->structdefd && stararrays != 2))
                            {
                                tp = tp->basetype;
                                flags &= ~ODECL_CHARSTAR;
#ifdef PRES_PTR_ARRAY
                ...
                dimen[depth++] = NULL;
                anyptrs++;
                if(tp->kind == TK_POINTER && tp->basetype->kind != TK_POINTER && tp->fbase
                   && tp->fbase->wasdeclared)
                    break;
                continue;

            case TK_ARRAY:
                flags &= ~ODECL_CHARSTAR;
                if(tp->meaning && tp->meaning->kind == MK_TYPE && tp->meaning->wasdeclared)
                    break;
                if(tp->structdefd)
                { /* conformant array */
                    if(!variablearrays && !(tp->basetype->kind == TK_ARRAY && tp->basetype->structdefd)) /* avoid mult. notes */
            ...
            case TK_SET:
                ...
                dimen[depth++] = makeexpr_plus(makeexpr_div(maxv, makeexpr_setbits()), makeexpr_long(2));
                break;
    ...
    if(!*name && depth && (spaceexprs > 0 || (spaceexprs != 0 && !dimen[depth - 1])))
        output(" "); /* spacing for abstract declarator */
    if((flags & ODECL_FUNCTION) && anyptrs)
        output(" ");
    spaceafter
    = ((spacestars == 1 || spacestars == 3 || (spacestars == 2 && (flags & ODECL_REF) && depth == 1))
    ...
    for(i = depth; --i >= 0;)
    {
        ...
        if(i > 0 && ((dimen[i] && !dimen[i - 1]) || (dimen[i - 1] && !dimen[i] && extraparens > 0)))
            output("(");
    }
        ...
        if(dimen[i])
        {
            if(dimen[i] == &funcdummy)
            {
                if(lookback(1) == ')')
                    output("\002");
                if(functype)
                    declare_args(functype, (flags & ODECL_HEADER) != 0, (flags & ODECL_FORWARD) != 0);
                else if(spacefuncs)
                    output(" ()");
                else
                    output("()");
...
{
    if(type->kind == TK_SUBR || type->kind == TK_ENUM || type->kind == TK_PROCPTR)
    ...
    if(type->kind == TK_POINTER)
    {
        if(type->smin)
            return type;
        else if(type->basetype->kind == TK_ARRAY || type->basetype->kind == TK_STRING
...
int identicaltypes(Type* t1, Type* t2)
{
    if(t1 == t2)
        return 1;
    if(t1->kind == t2->kind)
    {
        if(t1->kind == TK_SUBR)
            return (identicaltypes(t1->basetype, t2->basetype) && exprsame(t1->smin, t2->smin, 2)
                    && exprsame(t1->smax, t2->smax, 2));
        if(t1->kind == TK_SET || t1->kind == TK_SMALLSET)
            return (exprsame(t1->indextype->smax, t2->indextype->smax, 2));
        if(t1->kind == TK_ARRAY || t1->kind == TK_STRING || t1->kind == TK_SMALLARRAY)
            return (identicaltypes(t1->basetype, t2->basetype)
                    && identicaltypes(t1->indextype, t2->indextype) && t1->structdefd == t2->structdefd
                    && ((!t1->smin && !t2->smin) || (t1->smin && t2->smin && exprsame(t1->smin, t2->smin, 2)))
                    && ((!t1->smax && !t2->smax)
                        || (t1->smax && t2->smax && exprsame(t1->smax, t2->smax, 2)
                            && t1->escale == t2->escale && t1->issigned == t2->issigned)));
    }
    return 0;
}

int similartypes(Type* t1, Type* t2)
{
    if(debug > 3)
    {
        fprintf(outf, "similartypes(");
        dumptypename(t1, 1);
        fprintf(outf, ",");
        dumptypename(t2, 1);
...
{
    return (mp->kind == MK_VAR && mp->constdefn && mp->constdefn->kind == EK_CONST
            && (mp->constdefn->val.type->kind == TK_ARRAY || mp->constdefn->val.type->kind == TK_RECORD));
}

int tinyexpr(Expr* ex)
{
    if(ex->kind == EK_CONST || ex->kind == EK_LONGCONST)
    {
        if(ex->val.type->kind == TK_INTEGER || (ex->val.type->kind == TK_REAL && strlen(ex->val.s) < 8)
           || ex->val.type->kind == TK_CHAR || (ex->val.type->kind == TK_POINTER && ex->val.i == 0)
           || (ex->val.type->kind == TK_SMALLSET && ex->val.i == 0)
           || ex->val.type->kind == TK_BOOLEAN || (ex->val.type->kind == TK_STRING && ex->val.i <= 3))
            return 1;
    }
...
Static int mixable(Meaning* mp1, Meaning* mp2, int args, int flags)
{
    ...
    if(mixvars == 0)
        return 0;
    if(mp1->kind == MK_FIELD && (mp1->val.i || mp2->val.i) && mixfields == 0)
        return 0;
       ...
       || (mp2->constdefn && (mp2->kind == MK_VAR || mp2->kind == MK_VARREF)))
    {
        if(mixinits == 0)
            return 0;
        if(mixinits != 1 && ((!mp1->constdefn && tinyexpr(mp2->constdefn)) || !mp2->constdefn))
            return 0;
    }
    if(args)
    {
...
void declarefiles(Strlist* fnames)
{
    Meaning* mp;
    char* cp;
    ...
    while(fnames)
    {
        ...
        if(mp->kind == MK_VAR || mp->kind == MK_FIELD)
        {
            ...
            {
                output(storageclassname(varstorageclass(mp)));
                output(format_ss("%s %s", charname, format_s(name_FNVAR, fnames->s)));
...
{
    return (tp->kind == TK_RECORD && tp->fbase && tp->fbase->kind == MK_VARIANT);
}
...
void outfieldlist(Meaning* mp)
{
    ...
    while(mp && mp->kind != MK_VARIANT)
    {
        ...
        for(;;)
        {
            ...
            mp->wasdeclared = 1;
            if(mp->kind == MK_FUNCTION && mp->val.s && mp->val.s[0] == 'D' && mp->bufferedfile)
                virtdestr = mp->rectype;
            if(!mp->cnext || mp->cnext->kind == MK_VARIANT || mp->dtype != mp->cnext->dtype
               || varstorageclass(mp) != varstorageclass(mp->cnext) || !mixable(mp, mp->cnext, 0, 0))
                break;
            mp = mp->cnext;
            if(spacecommas)
                output(",\001 ");
...
void outbasetype(Type* type, int flags)
{
    ...
    switch(type->kind)
    {
        ...
        default:
            ...
            else if(type->meaning && type->meaning->kind == MK_TYPE && type->meaning->wasdeclared)
            {
                output(type->meaning->name);
            }
            else
                ...
                switch(type->kind)
                {
                    ...
                    case TK_BIGFILE:
                        ...
                        if(!type->structdefd || (!type->meaning && !type->smin))
                        {
                            ...
                            type->structdefd = 1;
                            output("{\n");
                            if(type->issigned && type->kind == TK_RECORD && type->fbase
                               && !type->fbase->isreturn)
                                output("public:\n");
                            saveindent = outindent;
                            moreindent(tabsize);
...
void out_type(Type* type, int flags)
{
...
int onewordtype(Type* type, int flags)
{
    Type* tp;
    ...
    if(tp != type)
    {
        if(type->kind == TK_POINTER || type->kind == TK_ARRAY || type->kind == TK_STRING
           || type->kind == TK_STRING || type->kind == TK_CPROCPTR || type->kind == TK_FUNCTION)
            return 0;
    }
    if(((tp == tp_ubyte || tp == tp_uchar) && !onewordstring(ucharname))
       ...
       || (tp->kind == TK_BOOLEAN && !onewordstring((*name_BOOLEAN) ? name_BOOLEAN : ucharname))
       || tp == tp_unsigned || tp == tp_ushort || tp == tp_special_anyptr || tp->kind == TK_PROCPTR
       || tp->kind == TK_FILE || tp->kind == TK_SPECIAL)
        return 0;
...
int varstorageclass(Meaning* mp)
{
    int sclass;

    if(mp->kind == MK_PARAM || mp->kind == MK_VARPARAM || mp->kind == MK_FIELD)
        sclass = 0;
    else if(useextern >= 0)
        if(useextern > 0)
            sclass = 2; /* extern */
        ...
        if(usevextern)
            if(mp->constdefn && (mp->kind == MK_VAR || mp->kind == MK_VARREF))
                sclass = 2; /* extern */
            else
                sclass = 1; /* vextern */
        else
        ...
        sclass = 2; /* extern */
            ...
            && (var_static != 0 || (findsymbol(mp->name)->flags & NEEDSTATIC)) && !mp->exported
            && !mp->istemporary && blockkind != TOK_END)
    ...
    if(mp->constqual)
        sclass |= 0x20;
    if(debug > 2)
        fprintf(outf, "varstorageclass(%s) = %d\n", mp->name, sclass);
    return sclass;
}

char* storageclassname(int i)
{
    char* scname;
...
void declarevar(Meaning* mp, int which /* VDECL_... */
)
{
    ...
    if(which & VDECL_BODY)
    {
        ...
        flags = 0;
        if(mp->constdefn && blockkind != TOK_EXPORT && (mp->kind == MK_VAR || mp->kind == MK_VARREF))
        {
            if(mp->varstructflag)
            { /* move init code into function body */
                intwarning("declarevar", format_s("Variable %s initializer not removed [125]", mp->name));
            }
            else
            {
                if(isstructconst)
                ...
                else if(mp->type->kind == TK_ARRAY && mp->constdefn->val.type->kind == TK_STRING && !initpacstrings)
                {
                    if(mp->ctx->kind == MK_MODULE)
                    {
                        sl = strlist_append(
                        &initialcalls,
                        format_sss("memcpy(%s,\002 %s,\002 sizeof(%s))", mp->name,
                                   makeCstring(mp->constdefn->val.s, mp->constdefn->val.i), mp->name));
                        sl->value = 1;
                    }
                    else if(mp->isforward)
                    {
...
Static int checkvarmacdef(Expr* ex, Meaning* mp)
{
    ...
    if((ex->kind == EK_NAME || ex->kind == EK_BICALL) && !strcmp(ex->val.s, mp->name))
    {
        ex->kind = EK_VAR;
        ex->val.i = (long)mp;
        ex->val.type = mp->type;
...
int declarevars(Meaning* ctx, int invarstruct)
{
    Meaning *mp, *mp0, *mp2;
    Strlist *fnames, *fn;
    int flag, first;
    ...
    if(debug > 2)
    {
        ...
        {
            fprintf(outf, "  %-22s%-15s%3d", mp->name, meaningkindname(mp->kind), mp->refcount);
            if(mp->wasdeclared)
                fprintf(outf, " [decl]");
            if(mp->varstructflag)
                fprintf(outf, " [struct]");
    ...
    for(;;)
    {
        mp = ctx->cbase;
        while(mp
              && (!(varkind(mp->kind) || checkvarmac(mp)) || mp->wasdeclared
        ...
        while(mp)
        {
            if((varkind(mp->kind) || checkvarmac(mp)) && !mp->wasdeclared && var_mixable
               && mp->dtype == mp0->dtype && varstorageclass(mp) == varstorageclass(mp0)
               && mp->varstructflag == invarstruct && mp->refcount > 0)
            {
            ...
            if(first)
            {
                intwarning("declarevars", format_s("Unable to declare %s [126]", mp->name));
                mp->wasdeclared = 1;
                first = 0;
            }
            if(mixvars == 0)
...
void redeclarevars(Meaning* ctx)
{
    Meaning* mp;

    for(mp = ctx->cbase; mp; mp = mp->cnext)
    {
        if((mp->kind == MK_VAR || mp->kind == MK_VARREF) && mp->constdefn)
        {
...
void out_argdecls(Type* ftype)
{
    ...
    do
    {
        ...
        if(mp)
        {
            ...
            if(mp->dtype)
                ...
                outbasetype(mp->othername ? mp->rectype : mp->type, ODECL_CHARSTAR | ODECL_FREEARRAY);
            oflags = ODECL_SPACE | ODECL_SPMRG;
            while(mp)
            {
                if(!mp->wasdeclared)
                {
                    if(mp == mp0 || (mp->dtype == mp0->dtype && mixable(mp0, mp, 1, ODECL_CHARSTAR | ODECL_FREEARRAY)))
                    {
                        if(mp != mp0)
                            if(spacecommas)
                                output(",\001 ");
                        ...
                        tp = (mp->othername) ? mp->rectype : mp->type;
                        outdeclarator(tp, name, ODECL_CHARSTAR | ODECL_FREEARRAY | oflags);
                        oflags = 0;
                        mp->wasdeclared = 1;
                    }
                    else if(mixvars != 1)
    ...
    for(mp0 = ftype->fbase; mp0 && (mp0->type != tp_strptr || !mp0->anyvarflag); mp0 = mp0->xnext)
        ;
    if(mp0)
    {
        output("int ");
    ...
    if(ftype->meaning && ftype->meaning->ctx->kind == MK_FUNCTION && ftype->meaning->ctx->varstructflag)
    {
        if(flag)
            output("\n");
        output("struct ");
...
void makevarstruct(Meaning* func)
{
    int flag = 0;
    int saveindent;
...
Type* maketype(enum typekind kind)
{
    Type* tp;
...
Type* makesubrangetype(Type* type, Expr* smin, Expr* smax)
{
    Type* tp;
...
Type* makestringtype(int len)
{
    ...
    else
    {
        ...
        type->indextype = makesubrangetype(tp_integer, makeexpr_long(0), makeexpr_long(len));
...
int typebits(long smin, long smax)
{
    unsigned long size;
    int bits;
...
int packedsize(char* fname, Type** typep, long* sizep, int mode)
{
    Type* tp = *typep;
    long smin, smax;
    int res, issigned;
    ...
    if(packing == 0) /* suppress packing */
        return 0;
    if(tp->kind != TK_SUBR && tp->kind != TK_INTEGER && tp->kind != TK_ENUM && tp->kind != TK_CHAR
       && tp->kind != TK_BOOLEAN)
        return 0;
    if(tp == tp_unsigned)
        return 0;
    if(!ord_range(tp, &smin, &smax))
    {
        ...
        if(res)
        {
            note(format_s("Field width for %s is based on expansion of #defines [103]", fname));
        }
        else
        {
            note(format_ss("Cannot compute size of field %s; assuming %s [104]", fname, integername));
            return 0;
        }
    }
    else
    {
        if(tp->kind == TK_ENUM)
            note(format_ssd("Field width for %s assumes enum%s has %d elements [105]", fname,
                            (tp->meaning) ? format_s(" %s", tp->meaning->name) : "", smax + 1));
    }
    issigned = (smin < 0);
...
Static void fielddecl(Meaning* mp, Type** type, Type** tp2, long* val, int ispacked, int* aligned)
{
    ...
    *val = 0;
    if(ispacked && !mp->constdefn && *type != tp_unsigned)
    {
        ...
        if(*aligned && *val && (ord_type(*type)->kind == TK_CHAR || ord_type(*type)->kind == TK_INTEGER)
           && ord_range(findbasetype(*type, ODECL_NOPRES), &smin, &smax))
        {
            if(ord_range(*type, &smin2, &smax2))
            {
                if(typebits(smin, smax) == 16 && typebits(smin2, smax2) == 8 && *val == 8)
                {
                    *tp2 = tp_abyte;
                }
            }
            if(typebits(smin, smax) == *val && *val != 7)
            { /* don't be fooled by tp_abyte */
        ...
        if(*aligned && *val == 8 && (ord_type(*type)->kind == TK_BOOLEAN || ord_type(*type)->kind == TK_ENUM))
        {
            *val = 0;
            *tp2 = tp_ubyte;
        }
...
static void tryrealignfields(Meaning* firstmp)
{
    ...
    for(mp = firstmp; mp; mp = mp->cnext)
    {
        if(mp->kind == MK_FIELD)
        {
            if((mp->val.i == 8 && (mp->type == tp_uint || hassignedchar || signedchars == 1))
               || mp->val.i == 16)
            {
                if(!head)
                    head = mp;
...
void makestructtag(Type* type, char* tname)
{
    Symbol* sym;
    char* name;
    int altnum;
...
Static void p_fieldlist(Type* tp, Meaning** flast, int ispacked, Meaning* tname)
{
    ...
    for(;;)
    {
        ...
        if(curtok == TOK_PROCEDURE || curtok == TOK_FUNCTION || curtok == TOK_CONSTRUCTOR || curtok == TOK_DESTRUCTOR)
        {
            int isfunc = (curtok == TOK_FUNCTION);
            char* special = ((curtok == TOK_CONSTRUCTOR) ? "C" : (curtok == TOK_DESTRUCTOR) ? "D" : NULL);
            Meaning *func, *func2;
            gettok();
            ...
            curtoksym = findsymbol(format_ss("%s::%s", tname->name, func->name));
    ...
    if(curtok == TOK_CASE)
    {
        ...
        while(curtok != TOK_END && curtok != TOK_RPAR)
        {
            ...
            else if(!wneedtok(TOK_COLON) || (!modula2 && !wneedtok(TOK_LPAR)))
            {
                skiptotoken2(TOK_END, TOK_RPAR);
                goto bounce;
            }
...
Static Type* p_arraydecl(char* tname, int ispacked, Meaning*** confp)
{
    ...
    if(confp == NULL)
    {
        ...
        {
            if(ord_range(tp->indextype, &smin, NULL) && smin > 0 && smin <= skipindices && !ispacked)
            {
                tp->smin = makeexpr_val(make_ord(tp->indextype->basetype, smin));
                ex = makeexpr_val(make_ord(tp->indextype->basetype, 0));
                tp->indextype = makesubrangetype(tp->indextype->basetype, ex, copyexpr(tp->indextype->smax));
            }
        }
    }
    else
    {
        if(modula2 || (which_lang == LANG_TIP && (curtok != TOK_IDENT || curtokmeaning)))
        {
            ...
            *confp = &mp->xnext->xnext;
            ...
            else
            {
                ...
                tp2->smax = makeexpr_minus(makeexpr_var(mp->xnext), makeexpr_var(mp));
            }
            tp->indextype = tp2;
            tp->structdefd = 1;
        }
        else
        {
            wexpecttok(TOK_IDENT);
            tp2 = maketype(TK_SUBR);
            if(peeknextchar() != ',' && (!curtokmeaning || curtokmeaning->kind != MK_TYPE))
            {
    ...
    else
    {
        ...
        if(confp != NULL && (curtok == TOK_ARRAY || curtok == TOK_PACKED || curtok == TOK_VARYING))
        {
        ...
        if(!size || size > 8 || (issigned && !packsigned)
           || (size > 4 && (!issigned || (signedchars == 1 || hassignedchar))))
            return tp;
        bpower = 0;
        while((1 << bpower) < size)
        ...
        hasrange = ord_range(tp->indextype, &smin, &smax) && (smax < 100000); /* don't be confused by giant arrays */
        if(hasrange && (bitsize = (smax - smin + 1) * size) <= ((sizeof_integer > 0) ? sizeof_integer : 32))
        {
            ...
            if(ord_range(tp->indextype, &smin, NULL) && smin > 0 && smin <= fullbitsize - bitsize)
            {
                tp->smin = makeexpr_val(make_ord(tp->indextype->basetype, smin));
                ex = makeexpr_val(make_ord(tp->indextype->basetype, 0));
                tp->indextype = makesubrangetype(tp->indextype->basetype, ex, copyexpr(tp->indextype->smax));
...
Static Type* p_conformant_array(char* tname, Meaning*** confp)
{
    ...
    if(wneedtok(TOK_ARRAY) && (modula2 || wneedtok(TOK_LBR)))
    {
...
void p_attributes(void)
{
    ...
    while(curtok == TOK_LBR)
    {
        ...
        do
        {
            ...
            {
                gettok();
                if(!strcmp(l1->s, "CHECK") || !strcmp(l1->s, "OPTIMIZE") || !strcmp(l1->s, "KEY")
                   || !strcmp(l1->s, "COMMON") || !strcmp(l1->s, "PSECT")
                   || !strcmp(l1->s, "EXTERNAL") || !strcmp(l1->s, "GLOBAL")
                   || !strcmp(l1->s, "WEAK_EXTERNAL") || !strcmp(l1->s, "WEAK_GLOBAL"))
                {
                ...
                else if(!strcmp(l1->s, "INHERIT") || !strcmp(l1->s, "IDENT") || !strcmp(l1->s, "ENVIRONMENT"))
                {
                    p_expr(NULL);
                    while(curtok == TOK_COMMA)
                    {
...
void ignore_attributes(void)
{
    while(attrlist)
    {
        if(strcmp(attrlist->s, "HIDDEN") && strcmp(attrlist->s, "INHERIT") && strcmp(attrlist->s, "ENVIRONMENT"))
            warning(format_s("Type attribute %s ignored [128]", attrlist->s));
        strlist_eat(&attrlist);
    }
}

int size_attributes(void)
{
    int size = -1;
    Strlist* l1;
...
void p_mech_spec(int doref)
{
    if(curtok == TOK_IDENT && doref && !strcicmp(curtokbuf, "%REF"))
    {
    ...
    if(curtok == TOK_IDENT
       && (!strcicmp(curtokbuf, "%REF") || !strcicmp(curtokbuf, "%IMMED")
           || !strcicmp(curtokbuf, "%DESCR") || !strcicmp(curtokbuf, "%STDESCR")))
    {
...
Type* p_modula_subrange(Type* basetype)
{
    Type* tp;
    Value val;
    ...
    if(wexpecttok(TOK_DOTS))
    {
        gettok();
        tp->smax = p_ord_expr();
        if(tp->smax->val.type->kind == TK_REAL && tp->smax->kind == EK_CONST && strlen(tp->smax->val.s) == 12
           && strcmp(tp->smax->val.s, "2147483648.0") >= 0 && strcmp(tp->smax->val.s, "4294967295.0") <= 0)
        {
            tp = tp_unsigned;
        }
        else if(basetype)
...
void makefakestruct(Type* tp, Meaning* tname)
{
    Symbol* sym;
    ...
    switch(curtok)
    {
        ...
        case TOK_OBJECT: /* Turbo 6.0 or Object Pascal objects */
            ...
            if(curtok == TOK_LPAR)
            {
                gettok();
                expecttok(TOK_IDENT);
                if(curtokmeaning && curtokmeaning->kind == MK_TYPE && curtokmeaning->type->kind == TK_RECORD)
        ...
        case TOK_VARYING:
            ...
            if(val.type)
            {
                ...
                {
                    note(format_d("Strings longer than %d may have problems [109]", stringceiling));
                }
                if(stringceiling != 255 && (val.i >= 255 || val.i > stringceiling))
                {
                    freeexpr(ex);
        ...
        case TOK_SET:
            ...
            if(tp == tp_integer || tp == tp_unsigned)
                tp = makesubrangetype(tp, makeexpr_long(0), makeexpr_long(defaultsetsize - 1));
        ...
        case TOK_FILE:
            gettok();
            if(structfilesflag || (tname && strlist_cifind(structfiles, tname->name)))
        ...
        case TOK_FUNCTION:
            ...
            proctypecount++;
            mp = addmeaning(findsymbol(format_d("__PROCPTR%d", proctypecount)), MK_FUNCTION);
            pushctx(mp);
        ...
        case TOK_POINTER:
            ...
            if(curtok == TOK_IDENT
               && (!curtokmeaning || curtokmeaning->kind != MK_TYPE
                   || (deferallptrs && curtokmeaning->ctx != curctx && curtokmeaning->ctx != nullctx)))
            {
                struct ptrdesc* pd;
        ...
        case TOK_IDENT:
            ...
            else if(curtokmeaning == mp_string)
            {
                ...
                val = eval_expr(ex);
                if(val.type && stringceiling != 255 && (val.i >= 255 || val.i > stringceiling))
                {
                    freeexpr(ex);
                    ex = makeexpr_long(stringceiling);
                }
            ...
            else if(curtokmeaning->kind == MK_TYPE)
            {
                    ...
                    if(ord_type(tp)->kind == TK_INTEGER && sizespec <= 32)
                    {
                        if(checkconst(tp->smin, 0))
                        {
                            if(sizespec == 32)
                                ...
                                tp = makesubrangetype(tp_unsigned, makeexpr_long(0),
                                                      makeexpr_long((1L << sizespec) - 1));
                        }
                        else
                        {
                            tp = makesubrangetype(tp_integer, makeexpr_long(-((1L << (sizespec - 1)))),
                ...
                if(tp->kind == TK_RECORD && tp->issigned && !turboobjects)
                    tp = makepointertype(tp);
                if(tp == tp_text && (structfilesflag || (tname && strlist_cifind(structfiles, tname->name))))
                    tp = tp_bigtext;
                break;
            }
        ...
        default:
            ...
            if(wexpecttok(TOK_DOTS))
            {
                gettok();
                tp->smax = p_ord_expr();
                if(tp->smax->val.type->kind == TK_REAL && tp->smax->kind == EK_CONST
                   && strlen(tp->smax->val.s) == 12 && strcmp(tp->smax->val.s, "2147483648.0") >= 0
                ...
                {
                    long smin, smax;
                    if(ord_range(tp, &smin, &smax) && typebits(smin, smax) == sizespec)
                        sizespec = -1;
                }
            }
            else
...
Type* p_funcdecl(int* isfunc, int istype)
{
        ...
        do
        {
            ...
            *prevm = firstmp;
            ...
            else
            {
                if(curtok == TOK_COLON)
                    gettok();
                if(curtok == TOK_IDENT && !curtokmeaning && !strcicmp(curtokbuf, "UNIV"))
                {
                    if(parkind == MK_PARAM)
                ...
                if((l1 = strlist_find(attrlist, "UNSAFE")) != NULL && parkind == MK_VARPARAM)
                {
                    anyvarflag = 1;
                    strlist_delete(&attrlist, l1);
                }
                ...
                checkkeyword(TOK_VARYING);
                if(curtok == TOK_IDENT && curtokmeaning == mp_string && !anyvarflag && parkind == MK_VARPARAM)
                {
                ...
                else if(curtok == TOK_ARRAY || curtok == TOK_PACKED || curtok == TOK_VARYING)
                {
                    prevm = oldprevm;
                ...
                if(parkind == MK_VARPARAM)
                {
                    if(userefs && !anyvarflag && tp != tp_anyptr && tp->kind != TK_STRING && tp->kind != TK_ARRAY
                       && tp->kind != TK_SMALLARRAY && tp->kind != TK_SET && tp->kind != TK_SMALLSET)
                    {
                        parkind = MK_PARAM;
                        refflag = 1;
                    }
            ...
            { /* check for parameter default */
                gettok();
                p_mech_spec(0);
                defval = gentle_cast(p_expr(tp), tp);
                if((tp->kind == TK_STRING || tp->kind == TK_ARRAY) && tp->basetype->kind == TK_CHAR
            ...
            while(firstmp)
            {
                ...
                if(parkind == MK_PARAM
                   && (tp->kind == TK_STRING || tp->kind == TK_ARRAY || tp->kind == TK_SET
                       || ((tp->kind == TK_RECORD || tp->kind == TK_BIGFILE || tp->kind == TK_PROCPTR)
                           && copystructs < 2)))
                {
...
Symbol* findlabelsym(void)
{
    if(curtok == TOK_IDENT && curtokmeaning && curtokmeaning->kind == MK_LABEL && !curtokmeaning->isreturn)
    {
#if 0
...
void p_labeldecl(void)
{
    ...
    do
    {
        ...
        mp->val.i = 0;
        mp->xnext = addmeaning(findsymbol(format_s(name_LABVAR, mp->name)), MK_VAR);
...
Meaning* findfieldname(Symbol* sym, Meaning** variants, int* nvars)
{
    Meaning *mp, *mp0;
...
Expr* p_constrecord(Type* type, int style /* 0=HP, 1=Turbo, 2=Oregon+VAX */
)
{
    ...
    for(;;)
    {
        ...
        ex = p_subconst(mp->type, style);
        if(ex->kind == EK_CONST && (ex->val.type->kind == TK_RECORD || ex->val.type->kind == TK_ARRAY))
            ex = (Expr*)ex->val.i;
        cex->args[i] = ex;
    ignorefield:
        if(curtok == TOK_COMMA || curtok == TOK_SEMI)
    ...
    if(style != 2)
    {
        ...
        for(i = 0; i < cex->nargs; i++)
        {
            ...
            if(!cex->args[i])
            {
                ...
                if(mp->type->kind == TK_INTEGER || mp->type->kind == TK_SUBR || mp->type->kind == TK_CHAR
                   || mp->type->kind == TK_ENUM || mp->type->kind == TK_BOOLEAN
                   || mp->type->kind == TK_REAL || mp->type->kind == TK_POINTER)
...
Expr* p_constarray(Type* type, int style)
{
    Value val;
    Expr *ex, *cex;
    int nvals, skipped;
    ...
    for(;;)
    {
        ...
        {
            ex = p_expr(type->basetype);
            if(ex->kind == EK_CONST && ex->val.type->kind == TK_STRING && ex->val.i > 1 && !skipped && style == 0
               && !cex && type->basetype->kind == TK_CHAR && checkconst(type->indextype->smin, 1))
            {
                if(!wneedtok(TOK_RBR))
...
Expr* p_conststring(Type* type, int style)
{
    Expr* ex;
...
Expr* p_subconst(Type* type, int style)
{
    Value val;
    ...
    if(curtok == TOK_IDENT && !strcicmp(curtokbuf, "ZERO") && !curtokmeaning)
    { /* VAX Pascal foolishness */
        gettok();
        ...
        val.type = type;
        if(type->kind == TK_RECORD || type->kind == TK_ARRAY || type->kind == TK_SET)
...
void p_constdecl(void)
{
    ...
    while(curtok == TOK_IDENT)
    {
        ...
        else
        {
            ...
            gettok();
            if(curtok == TOK_IDENT && curtokmeaning && curtokmeaning->kind == MK_TYPE
               && (curtokmeaning->type->kind == TK_RECORD || curtokmeaning->type->kind == TK_SMALLARRAY
            ...
            else if(mp->kind == MK_CONST && outflag)
            {
                ...
                switch(ex->val.type->kind)
                {
                    ...
                    case TK_RECORD:
                        ...
                        outdeclarator(mp->type, mp->name, ODECL_CHARSTAR | ODECL_FREEARRAY | ODECL_SPACE);
                        output(" = {");
                        outtrailcomment(mp->comments, -1, declcommentindent);
                        saveindent = outindent;
                        moreindent(tabsize);
                        ...
                        if(blockkind == TOK_EXPORT)
                        {
                            ...
                            outdeclarator(mp->type, mp->name, ODECL_CHARSTAR | ODECL_SPACE);
                            output(";\n");
                        }
                        break;
...
void declaresubtypes(Meaning* mp)
{
    Meaning* mp2;
    Type* tp;
    struct ptrdesc* pd;
    ...
    while(mp)
    {
        ...
        else
        {
            ...
            while(tp->basetype && !tp->meaning && tp->kind != TK_POINTER)
                tp = tp->basetype;
            if(tp->meaning && !tp->meaning->wasdeclared && (tp->kind == TK_RECORD || tp->kind == TK_ENUM)
...
void declaretype(Meaning* mp)
{
    ...
    switch(mp->type->kind)
    {
        ...
        case TK_BIGFILE:
            ...
            {
                output(format_ss("typedef %s %s;", mp->type->meaning->name, mp->name));
            }
            else
            {
                ...
                output(" {\n");
                if(mp->type->issigned && mp->type->fbase && !mp->type->fbase->isreturn)
                    output("public:\n");
                saveindent = outindent;
                moreindent(tabsize);
                moreindent(structindent);
        ...
        case TK_SMALLARRAY:
            ...
            if(mp->type->meaning != mp)
            {
                output(format_ss("%s %s", mp->type->meaning->name, mp->name));
            }
            else
            {
                outbasetype(mp->type, 0);
        ...
        default:
            ...
            {
                output("typedef ");
                if(mp->type->meaning != mp && mp->dtype && mp->dtype != mp)
                {
                    output(mp->dtype->name);
                }
                else
...
void p_typedecl(void)
{
    ...
    while(curtok == TOK_IDENT)
    {
        ...
        else
        {
            ...
            if(!mp->type->meaning)
                mp->type->meaning = mp;
            if(mp->type->kind == TK_RECORD || mp->type->kind == TK_BIGFILE)
                mp->type->structdefd = 1;
...
Static void handleabsolute(Meaning* mp, int skip)
{
    ...
    if(curtok == TOK_ABSOLUTE)
    {
        ...
        if(curtok == TOK_IDENT && curtokmeaning
           && (curtokmeaning->kind != MK_CONST || ord_type(curtokmeaning->type)->kind != TK_INTEGER))
        {
...
Meaning* validatedtype(Meaning* dtype, Type* type)
{
    if(dtype
       && (!type->preserved || !type->meaning || dtype->kind != MK_TYPE || dtype->type != type
           || type->meaning == dtype))
        return NULL;
    return dtype;
}

void p_vardecl(int iscommon)
{
    ...
    while(curtok == TOK_IDENT)
    {
        ...
        if((l1 = strlist_find(attrlist, "GLOBAL")) != NULL || (l1 = strlist_find(attrlist, "WEAK_GLOBAL")) != NULL)
        {
            globalflag = 1;
            if(l1->value != -1)
        ...
        dtype = (curtok == TOK_IDENT) ? curtokmeaning : NULL;
        if(curtok == TOK_IDENT && !curtokmeaning && strcicmp(curtokbuf, "EXTERNAL"))
        {
            externflag = 1;
        }
        tp = p_type(firstmp);
        ...
        for(;;)
        {
            ...
            if((globalflag && (curctx->kind != MK_MODULE || mainlocals)) || (iscommon && firstmp->kind != MK_VARMAC))
            {
                declarevar(firstmp, (VDECL_ALL | (iscommon ? 0 : VDECL_VARSTRUCT)));
...
void p_valuedecl(void)
{
    ...
    while(curtok == TOK_IDENT)
    {
        if(!curtokmeaning || curtokmeaning->kind != MK_VAR)
        {
            warning(format_s("Initializer ignored for variable %s [139]", curtokbuf));
            skippasttoken(TOK_SEMI);
        }
        else
        {
...
Meaning* maketempvar(Type* type, char* name)
{
    ...
    while((tv = *tvp)
          && (!similartypes(tv->tvar->type, type) || tv->tvar->refcount == 0 || strcmp(tv->tvar->val.s, name)))
        tvp = &(tv->next);
    if(!tv)
    {
        tvp = &tempvars; /* take over a now-cancelled temporary */
        while((tv = *tvp) && (tv->tvar->refcount > 0 || strcmp(tv->tvar->val.s, name)))
            tvp = &(tv->next);
    }
    if(tv)
    {
    ...
    else
    {
        ...
        if(debug > 1)
        {
            fprintf(outf, "maketempvar creates %s\n", mp->name);
        }
    }
    return mp;
}
...
Meaning* makestmttempvar(Type* type, char* name)
{
    struct tempvarlist* tv;
    Meaning* tvar;
...
Meaning* markstmttemps(void)
{
    return (stmttempvars) ? stmttempvars->tvar : NULL;
}

void freestmttemps(Meaning* mark)
{
    struct tempvarlist* tv;
...
void canceltempvar(Meaning* tvar)
{
    if(debug > 1)
    {
