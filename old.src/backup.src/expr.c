...
char* value_name(Value val, char* intfmt, int islong)
{
    ...
    switch(type->kind)
    {
        ...
        case TK_BOOLEAN:
            if(!intfmt)
                if(val.i == 1 && *name_TRUE && strcmp(name_TRUE, "1") && !islong)
                    intfmt = name_TRUE;
                else if(val.i == 0 && *name_FALSE && strcmp(name_FALSE, "0") && !islong)
                    intfmt = name_FALSE;
                else
                    intfmt = "%ld";
            if(islong)
...
void freeexpr(register Expr* ex)
{
    register int i;
...
Expr* copyexpr(register Expr* ex)
{
    register int i;
    register Expr* ex2;
...
int exprsame(register Expr* a, register Expr* b, int strict)
{
    register int i;
...
int exprequiv(register Expr* a, register Expr* b)
{
    register int i, j, k;
    enum exprkind kind2;
...
void deletearg(Expr** ex, register int n)
{
...
void insertarg(Expr** ex, register int n, Expr* arg)
{
    register Expr *ex1 = *ex, *ex2;
    register int i;
...
int realint(register char* s, int i)
{
    if(i == 0)
        return realzero(s);
    if(*s == '-')
            ...
            for(i = 0; i < ex->nargs; i++)
            {
                ...
                if(ex2->kind == EK_TIMES)
                {
                    for(j = ex2->nargs; --j >= 0;)
                    {
                        ...
                        else if(ex2->args[j]->kind == EK_NEG && exprsame(ex2->args[j]->args[0], common, 1))
                        {
                            freeexpr(ex2->args[j]);
    ...
    if(isfiletype(a->val.type, -1))
    {
        ...
        else
        {
            ...
            if(*arraygetfbufname && filebasetype(a->val.type)->kind == TK_ARRAY)
                return makeexpr_bicall_2(arraygetfbufname, filebasetype(a->val.type),
                                         filebasename(a), makeexpr_type(filebasetype(a->val.type)));
            else
...
Expr* makeexpr_assign(Expr* a, Expr* b)
{
       ...
       || (a->val.type->kind == TK_RECORD && (copystructs < 1 || a->val.type != b->val.type)))
    {
        ...
        if(!exprsame(ex, ex2, 1))
        {
            ...
            else if(!(a->val.type->kind == TK_ARRAY && b->val.type->kind != TK_ARRAY))
