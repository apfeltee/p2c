...
int isvar(Expr* ex, Meaning* mp)
{
    return (ex->kind == EK_VAR && (Meaning*)ex->val.i == mp);
}

char* getstring(Expr* ex)
{
...
Type* choosetype(Type* t1, Type* t2)
{
    if(t1 == tp_void || (type_sizeof(t2, 1) && !type_sizeof(t1, 1)))
        return t2;
    else
        return t1;
}

Expr* convert_offset(Type* type, Expr* ex2)
{
    long size;
    int i;
    Value val;
    ...
    if(type->kind == TK_POINTER || type->kind == TK_ARRAY || type->kind == TK_SET || type->kind == TK_STRING)
        type = type->basetype;
    ...
    { /* look for terms like "n*sizeof(foo)" */
        ...
        if(ex2->kind == EK_TIMES)
        {
            ...
            for(i = 0; i < ex2->nargs && ex2->args[i]->kind != EK_SIZEOF; i++)
                ;
            if(i < ex2->nargs)
            {
...
Expr* convert_size(Type* type, Expr* ex, char* name)
{
    long size;
    Expr* ex2;
    int i, okay;
    ...
    if(val.type)
    {
        ...
        {
            freeexpr(ex);
            return makeexpr_times(makeexpr_long(val.i / size), makeexpr_sizeof(makeexpr_type(type), 0));
        }
    }
    if(name)
    {
...
Static Expr* func_abs(void)
{
    ...
    else
    {
        ...
        else if(lness > 0 && *absname)
        {
            ...
            else if(*absname == '*' && (exprspeed(ex) >= 5 || !nosideeffects(ex, 0)))
            {
                ...
                return makeexpr_comma(makeexpr_assign(makeexpr_var(tvar), ex),
                                      makeexpr_bicall_1(absname, tp_integer, makeexpr_var(tvar)));
            }
            else
            {
        ...
        {
            return makeexpr_cond(makeexpr_rel(EK_LT, copyexpr(ex), makeexpr_long(0)),
                                 makeexpr_neg(copyexpr(ex)), ex);
        }
        else
        {
            tvar = makestmttempvar(tp_integer, name_TEMP);
            return makeexpr_cond(makeexpr_rel(EK_LT, makeexpr_assign(makeexpr_var(tvar), ex), makeexpr_long(0)),
                                 makeexpr_neg(makeexpr_var(tvar)), makeexpr_var(tvar));
        }
    }
}

Static Expr* func_addr(void)
{
    Expr *ex, *ex2, *ex3;
    Type *type, *tp2;
    int haspar;
    ...
    {
        if((ex->val.type->kind != TK_PROCPTR && ex->val.type->kind != TK_CPROCPTR)
           || (ex->kind == EK_VAR && ex->val.type == ((Meaning*)ex->val.i)->type))
            ex = makeexpr_addrf(ex);
    }
    if(haspar)
    {
...
Static Expr* func_iaddress(void)
{
    return makeexpr_cast(func_addr(), tp_integer);
}

Static Expr* func_addtopointer(void)
{
    Expr *ex, *ex2, *ex3;
    Type *type, *tp2;
...
Stmt* wrapopencheck(Stmt* sp, Expr* fex)
{
    Stmt* sp2;
    ...
    if(FCheck(checkfileisopen) && !is_std_file(fex))
    {
        ...
        if(iocheck_flag)
        {
            sp2->stm2 = makestmt_call(
            makeexpr_bicall_1(name_ESCIO, tp_integer, makeexpr_name(filenotopenname, tp_int)));
        }
        else
        {
            sp2->stm2 = makestmt_assign(makeexpr_var(mp_ioresult), makeexpr_name(filenotopenname, tp_int));
        }
        return sp2;
    }
    else
...
Static Expr* checkfilename(Expr* nex)
{
    ...
    if(nex->kind == EK_CONST && nex->val.type->kind == TK_STRING)
    {
        switch(which_lang)
        {
            ...
            case LANG_HP:
                if(!strncmp(nex->val.s, "#1:", 3) || !strncmp(nex->val.s, "console:", 8)
                   || !strncmp(nex->val.s, "CONSOLE:", 8))
                {
                    freeexpr(nex);
                    nex = makeexpr_string("/dev/tty");
                }
                else if(!strncmp(nex->val.s, "#2:", 3) || !strncmp(nex->val.s, "systerm:", 8)
                        || !strncmp(nex->val.s, "SYSTERM:", 8))
                {
                    freeexpr(nex);
                    nex = makeexpr_string("/dev/tty"); /* should do more? */
                }
                else if(!strncmp(nex->val.s, "#6:", 3) || !strncmp(nex->val.s, "printer:", 8)
            ...
            case LANG_TURBO:
                if(checkstring(nex, "con") || checkstring(nex, "CON") || checkstring(nex, ""))
                {
                    freeexpr(nex);
                    nex = makeexpr_string("/dev/tty");
                }
                else if(checkstring(nex, "nul") || checkstring(nex, "NUL"))
                {
                    freeexpr(nex);
                    nex = makeexpr_string("/dev/null");
                }
                else if(checkstring(nex, "lpt1") || checkstring(nex, "LPT1") || checkstring(nex, "lpt2")
                        || checkstring(nex, "LPT2") || checkstring(nex, "lpt3")
                        || checkstring(nex, "LPT3") || checkstring(nex, "com1") || checkstring(nex, "COM1")
                        || checkstring(nex, "com2") || checkstring(nex, "COM2"))
                {
...
Static Stmt* handleopen(int code)
{
    ...
    if(curtok == TOK_COMMA)
    {
        ...
        ex = NULL;
    if(ex && (ex->val.type->kind == TK_STRING || ex->val.type->kind == TK_ARRAY))
    {
    ...
    if(readwriteopen == 2 || (israndomfile(fex->val.type) && code != 0)
       || (readwriteopen && fex->val.type != tp_text && fex->val.type != tp_bigtext))
    {
        if(!my_strchr(modebuf, '+'))
            strcat(modebuf, "+");
    }
    if(fex->val.type != tp_text && fex->val.type != tp_bigtext && binarymode != 0)
    {
        if(binarymode == 1)
            strcat(modebuf, "b");
        else
    ...
    if(!nex && fmp && !is_std_file(fex) && literalfilesflag > 0
       && (literalfilesflag == 1 || strlist_cifind(literalfiles, fmp->name)))
    {
    ...
    if(!nex || (isfiletype(fex->val.type, 1) && !truenex))
    {
        ...
        {
            sp1 = makestmt_call(makeexpr_bicall_1("rewind", tp_void, filebasename(copyexpr(fex))));
            if(code == 0 || is_std_file(fex))
            {
                ...
                sp1 = makestmt_if(
                makeexpr_rel(EK_NE, filebasename(copyexpr(fex)), makeexpr_nil()), sp1,
                makestmt_assign(filebasename(copyexpr(fex)), makeexpr_bicall_0("tmpfile", tp_text)));
        }
    }
    if(nex || isfiletype(fex->val.type, 1))
    {
        needcheckopen = 1;
        if(!strcmp(freopenname, "fclose") || !strcmp(freopenname, "fopen"))
        {
            sp2 = makestmt_assign(filebasename(copyexpr(fex)),
                                  makeexpr_bicall_2("fopen", tp_text, copyexpr(nex), makeexpr_string(modebuf)));
            if(!strcmp(freopenname, "fclose"))
            {
                sp2 = makestmt_seq(
                makestmt_if(makeexpr_rel(EK_NE, filebasename(copyexpr(fex)), makeexpr_nil()),
                            makestmt_call(makeexpr_bicall_1("fclose", tp_void, filebasename(copyexpr(fex)))), NULL),
                sp2);
            }
        }
        else
        {
            ...
            if(fex->kind == EK_VAR && fex->val.type == tp_text
               && ((Meaning*)fex->val.i == mp_input || (Meaning*)fex->val.i == mp_output))
            {
                tfex = makeexpr_var(makestmttempvar(tp_text, name_TEMP));
                frname = "freopen";
            }
            sp2
            = makestmt_assign(filebasename(copyexpr(tfex)),
                              makeexpr_bicall_3((*frname) ? frname : "freopen", tp_text, copyexpr(nex),
                                                makeexpr_string(modebuf), filebasename(copyexpr(fex))));
            if(!*frname)
            {
                sp2 = makestmt_if(makeexpr_rel(EK_NE, filebasename(copyexpr(fex)), makeexpr_nil()), sp2,
                                  makestmt_assign(filebasename(copyexpr(fex)),
                                                  makeexpr_bicall_2("fopen", tp_text, copyexpr(nex),
    ...
    {
        sp = makestmt_if(makeexpr_rel(EK_NE, copyexpr(nex), makeexpr_string("")), sp2, sp1);
    }
    if(code == 2 && !*openmode && nex)
    {
        sp = makestmt_seq(
        sp, makestmt_if(makeexpr_rel(EK_EQ, filebasename(copyexpr(fex)), makeexpr_nil()),
                        makestmt_assign(filebasename(copyexpr(fex)),
                                        makeexpr_bicall_2("fopen", tp_text, copyexpr(nex), makeexpr_string("w+"))),
                        NULL));
    }
    if(FCheck(checkfileopen) && needcheckopen)
    {
        sp = makestmt_seq(
        sp, makestmt_call(makeexpr_SETIO(makeexpr_rel(EK_NE, filebasename(copyexpr(tfex)), makeexpr_nil()),
                                         filenotfoundname, filenamepart(tfex) ? tfex : nex)));
    }
    ...
    if(*cp && /* (may be eaten later, if buffering isn't needed) */
       fileisbuffered(fex, 1))
        sp = makestmt_seq(sp, makestmt_call(makeexpr_bicall_2(cp, tp_void, filebasename(fex),
...
Static Expr* func_arctan(Expr* ex)
{
    ...
    if(atan2flag && ex->kind == EK_DIVIDE)
        return makeexpr_bicall_2("atan2", tp_longreal, ex->args[0], ex->args[1]);
...
Static Stmt* proc_argv(void)
{
    ...
    return makestmt_call(makeexpr_bicall_3("P_sun_argv", tp_void, aex, lex, makeexpr_arglong(ex, 0)));
}

Static Expr* func_asr(void)
{
    Expr* ex;
    ...
    if(skipcomma())
    {
        ...
        {
            ex = makeexpr_bicall_2("P_asr", ex->val.type, ex, p_expr(tp_unsigned));
        }
        else
        {
            ex = force_signed(ex);
...
Static Expr* func_binary(Expr* ex)
{
    ...
    {
        return makeexpr_bicall_3("strtol", tp_integer, ex, makeexpr_nil(), makeexpr_long(2));
    }
}
...
Static Expr* handle_bitsize(int next)
{
    ...
    if(psize)
        ...
        return makeexpr_times(makeexpr_sizeof(ex, 0), makeexpr_long(sizeof_char ? sizeof_char : 8));
}

Static Expr* func_bitsize(void)
{
    return handle_bitsize(0);
}

Static Expr* func_bitnext(void)
{
    return handle_bitsize(1);
}

Static Expr* func_blockread(void)
{
    ...
    if(curtok == TOK_COMMA)
    {
        ...
        sex = doseek(copyexpr(fex), makeexpr_times(sex, makeexpr_long(512)))->exp1;
    }
    else
        sex = NULL;
    skipcloseparen();
    type = vex->val.type;
    ex = makeexpr_bicall_4("fread", tp_integer, makeexpr_addr(vex), makeexpr_long(512),
                           convert_size(type, ex2, "BLOCKREAD"), filebasename(copyexpr(fex)));
    return makeexpr_comma(sex, ex);
}

Static Expr* func_blockwrite(void)
{
    Expr *ex, *ex2, *vex, *sex, *fex;
    Type* type;
    ...
    skipcloseparen();
    type = vex->val.type;
    ex = makeexpr_bicall_4("fwrite", tp_integer, makeexpr_addr(vex), makeexpr_long(512),
                           convert_size(type, ex2, "BLOCKWRITE"), filebasename(copyexpr(fex)));
    return makeexpr_comma(sex, ex);
}

Static Stmt* proc_blockread(void)
{
    ...
    if(rex)
    {
        ex = makeexpr_bicall_4("fread", tp_integer, makeexpr_addr(vex), makeexpr_long(1),
                               convert_size(type, ex2, "BLOCKREAD"), filebasename(copyexpr(fex)));
        ex = makeexpr_assign(rex, ex);
        if(!iocheck_flag)
            ex = makeexpr_comma(ex, makeexpr_assign(makeexpr_var(mp_ioresult), makeexpr_long(0)));
    }
    else
    {
        ex = makeexpr_bicall_4("fread", tp_integer, makeexpr_addr(vex), convert_size(type, ex2, "BLOCKREAD"),
                               makeexpr_long(1), filebasename(copyexpr(fex)));
        if(checkeof(fex))
        {
            ex = makeexpr_SETIO(makeexpr_rel(EK_EQ, ex, makeexpr_long(1)), endoffilename, fex);
        }
    }
    return wrapopencheck(makestmt_call(ex), fex);
}

Static Stmt* proc_blockwrite(void)
{
    Expr *ex, *ex2, *vex, *rex, *fex;
    Type* type;
    ...
    if(rex)
    {
        ex = makeexpr_bicall_4("fwrite", tp_integer, makeexpr_addr(vex), makeexpr_long(1),
                               convert_size(type, ex2, "BLOCKWRITE"), filebasename(copyexpr(fex)));
        ex = makeexpr_assign(rex, ex);
        if(!iocheck_flag)
            ex = makeexpr_comma(ex, makeexpr_assign(makeexpr_var(mp_ioresult), makeexpr_long(0)));
    ...
    {
        ex = makeexpr_bicall_4("fwrite", tp_integer, makeexpr_addr(vex), convert_size(type, ex2, "BLOCKWRITE"),
                               makeexpr_long(1), filebasename(copyexpr(fex)));
        if(FCheck(checkfilewrite))
        {
            ex = makeexpr_SETIO(makeexpr_rel(EK_EQ, ex, makeexpr_long(1)), filewriteerrorname, fex);
        }
    }
    return wrapopencheck(makestmt_call(ex), fex);
}

Static Stmt* proc_bclr(void)
{
    Expr *ex, *ex2;
    ...
    ex2 = p_expr(tp_integer);
    skipcloseparen();
    return makestmt_assign(
    ex, makeexpr_bin(EK_BAND, ex->val.type, copyexpr(ex),
                     makeexpr_un(EK_BNOT, ex->val.type,
                                 makeexpr_bin(EK_LSH, tp_integer, makeexpr_arglong(makeexpr_long(1), 1), ex2))));
}

Static Stmt* proc_bset(void)
{
    Expr *ex, *ex2;
    ...
    skipcloseparen();
    return makestmt_assign(
    ex, makeexpr_bin(EK_BOR, ex->val.type, copyexpr(ex),
                     makeexpr_bin(EK_LSH, tp_integer, makeexpr_arglong(makeexpr_long(1), 1), ex2)));
}

Static Expr* func_bsl(void)
{
    Expr *ex, *ex2;
...
Static Expr* func_btst(void)
{
    ...
    return makeexpr_rel(
    EK_NE,
    makeexpr_bin(EK_BAND, tp_integer, ex,
                 makeexpr_bin(EK_LSH, tp_integer, makeexpr_arglong(makeexpr_long(1), 1), ex2)),
    makeexpr_long(0));
}

Static Expr* func_byteread(void)
{
    Expr *ex, *ex2, *vex, *sex, *fex;
    Type* type;
    ...
    type = vex->val.type;
    ex = makeexpr_bicall_4("fread", tp_integer, makeexpr_addr(vex), makeexpr_long(1),
                           convert_size(type, ex2, "BYTEREAD"), filebasename(copyexpr(fex)));
    return makeexpr_comma(sex, ex);
}

Static Expr* func_bytewrite(void)
{
    Expr *ex, *ex2, *vex, *sex, *fex;
    Type* type;
    ...
    type = vex->val.type;
    ex = makeexpr_bicall_4("fwrite", tp_integer, makeexpr_addr(vex), makeexpr_long(1),
                           convert_size(type, ex2, "BYTEWRITE"), filebasename(copyexpr(fex)));
    return makeexpr_comma(sex, ex);
}

Static Expr* func_byte_offset(void)
{
    Type* tp;
    Meaning* mp;
    Expr* ex;
    ...
    gettok();
    skipcloseparen();
    return makeexpr_bicall_2("OFFSETOF", (size_t_long) ? tp_integer : tp_int, makeexpr_type(tp), ex);
}

Static Stmt* proc_call(void)
{
    Expr *ex, *ex2, *ex3;
    Type *type, *tp;
    Meaning* mp;
    ...
    if(type->escale != 0)
        ex->args[0] = makeexpr_cast(makeexpr_dotq(ex2, "proc", tp_anyptr), makepointertype(type->basetype));
    ...
    ex2 = makeexpr_dotq(ex2, "link", tp_anyptr), ex3 = copyexpr(ex);
    ...
    return makestmt_if(makeexpr_rel(EK_NE, ex2, makeexpr_nil()), makestmt_call(ex3), makestmt_call(ex));
}

Static Expr* func_chr(void)
{
    Expr* ex;
...
Static Stmt* proc_close(void)
{
    ...
    if(!skipopenparen())
        return NULL;
    fex = p_expr(tp_text);
    sp = makestmt_if(makeexpr_rel(EK_NE, filebasename(copyexpr(fex)), makeexpr_nil()),
                     makestmt_call(makeexpr_bicall_1("fclose", tp_void, filebasename(copyexpr(fex)))),
                     (FCheck(checkfileisopen)) ? makestmt_call(
                     makeexpr_bicall_1(name_ESCIO, tp_integer, makeexpr_name(filenotopenname, tp_int))) :
                                                 NULL);
    if(curtok == TOK_COMMA)
    {
        ...
        opt = "";
        if(curtok == TOK_IDENT
           && (!strcicmp(curtokbuf, "LOCK") || !strcicmp(curtokbuf, "PURGE")
               || !strcicmp(curtokbuf, "NORMAL") || !strcicmp(curtokbuf, "CRUNCH")))
        {
...
{
    if(isliteralconst(ex->args[3], NULL) == 2 && ex->args[3]->val.i >= stringceiling)
    {
        return makeexpr_bicall_3("sprintf", ex->val.type, ex->args[0], makeexpr_string("%s"),
                                 bumpstring(ex->args[1], makeexpr_unlongcast(ex->args[2]), 1));
    }
    if(checkconst(ex->args[2], 1))
    {
        return makeexpr_addr(makeexpr_substring(ex->args[0], ex->args[1], ex->args[2], ex->args[3]));
    }
    return makeexpr_bicall_4(strsubname, ex->val.type, ex->args[0], ex->args[1],
                             makeexpr_arglong(ex->args[2], 0), makeexpr_arglong(ex->args[3], 0));
}
...
Static Stmt* proc_cycle(void)
{
    return makestmt(SK_CONTINUE);
}

Static Stmt* proc_date(void)
{
    Expr* ex;
...
Static Stmt* proc_delete(Expr* ex)
{
    ...
    return makestmt_call(makeexpr_bicall_3(strdeletename, tp_void, ex->args[0],
...
void parse_special_variant(Type* tp, char* buf)
{
    ...
    while(curtok == TOK_COMMA)
    {
        ...
        *cp++ = '.';
        ...
        if(curtok == TOK_INTLIT || curtok == TOK_HEXLIT || curtok == TOK_OCTLIT)
        {
        ...
        else if(curtok == TOK_HAT || curtok == TOK_STRLIT)
        {
            ...
            if(ex->kind == EK_CONST)
            {
                if(ex->val.i <= 32 || ex->val.i > 126 || ex->val.i == '\'' || ex->val.i == '\\'
                   || ex->val.i == '=' || ex->val.i == '}')
...
char* find_special_variant(char* buf, char* spname, Strlist* splist, int need)
{
    Strlist* best = NULL;
    int len, bestlen = -1;
    char *cp, *cp2;
    ...
    if(bestlen != strlen(buf) && my_strchr(buf, '.'))
    {
        if((need & 1) || bestlen >= 0)
        {
            if(need & 2)
                return NULL;
            if(spname)
                note(format_ss("No %s form known for %s [187]", spname, strupper(buf)));
        }
    }
    if(bestlen >= 0)
...
Static int usenewdelete(Type* tp)
{
    if(newdelete)
        return 1;
    if(tp->kind == TK_POINTER && tp->basetype->kind == TK_RECORD && tp->basetype->issigned)
        return 1; /* Always use new/delete for object types */
    return 0;
}

Static Stmt* proc_dispose(void)
{
    Expr *ex, *destr = NULL;
    Type* type;
    Stmt* sp;
    ...
    {
        if(type->kind == TK_ARRAY || type->kind == TK_SET || type->kind == TK_STRING)
            sp = makestmt_call(makeexpr_bin(EK_DELETE, tp_void, ex, makeexpr_name("", tp_integer)));
        else
...
Static Expr* func_expo(Expr* ex)
{
    ...
    return makeexpr_comma(
    makeexpr_bicall_2("frexp", tp_longreal, grabarg(ex, 0), makeexpr_addr(makeexpr_var(tvar))),
    makeexpr_var(tvar));
}

int is_std_file(Expr* ex)
{
    return isvar(ex, mp_input) || isvar(ex, mp_output) || isvar(ex, mp_stderr);
}
...
Static Expr* iofunc(Expr* ex, int code)
{
    ...
    {
        if(isfiletype(ex->val.type, 1) || (exprspeed(ex) < 5 && nosideeffects(ex, 0)))
        {
    ...
    switch(code)
    {
        ...
        case 0: /* eof */
            if(fileisbuffered(ex, 0) && *eofbufname)
                ...
                ex = makeexpr_rel(EK_NE, makeexpr_bicall_1("feof", tp_int, ex), makeexpr_long(0));
            break;
    ...
    if(ex2)
    {
        ex = makeexpr_bicall_4((iocheck_flag) ? "~~CHKIO" : name_CHKIO,
                               ...
                               makeexpr_name("FileNotOpen", tp_int), ex, makeexpr_long(0));
    }
    if(ex3)
...
Static Stmt* proc_escape(void)
{
    ...
    if(which_lang == LANG_TIP)
    {
        ...
        else if(curtokmeaning && curtokmeaning == curctx && curctx->kind == MK_FUNCTION)
        {
        ...
        {
            gettok();
            return makestmt_call(makeexpr_bicall_1("exit", tp_void, makeexpr_name("EXIT_SUCCESS", tp_integer)));
        }
        else
        {
    ...
    if(curtok == TOK_LPAR)
        ...
        ex = makeexpr_long(0);
    return makestmt_call(makeexpr_bicall_1(name_ESCAPE, tp_int, makeexpr_arglong(ex, 0)));
}

Static Stmt* proc_excl(void)
{
    Expr *vex, *ex;
    ...
    if(vex->val.type->kind == TK_SMALLSET)
        return makestmt_assign(
        vex, makeexpr_bin(EK_BAND, vex->val.type, copyexpr(vex),
                          makeexpr_un(EK_BNOT, vex->val.type,
                                      makeexpr_bin(EK_LSH, vex->val.type,
                                                   makeexpr_longcast(makeexpr_long(1), 1), ex))));
    else
        return makestmt_call(makeexpr_bicall_2(setremname, tp_void, vex, makeexpr_arglong(enum_to_int(ex), 0)));
}
...
Stmt* proc_exit(void)
{
    ...
    {
        gettok();
        if(curtok == TOK_PROGRAM || (curtok == TOK_IDENT && curtokmeaning && curtokmeaning->kind == MK_MODULE))
        {
            gettok();
            skipcloseparen();
            return makestmt_call(makeexpr_bicall_1("exit", tp_void, makeexpr_name("EXIT_SUCCESS", tp_integer)));
...
Static Expr* file_iofunc(int code, long base)
{
    ...
    if(curtok == TOK_LPAR)
        ...
        ex = makeexpr_var(mp_input);
    if(!ex->val.type || !ex->val.type->basetype || !filebasetype(ex->val.type))
        basetype = tp_char;
    else
        basetype = filebasetype(ex->val.type);
    return makeexpr_plus(makeexpr_div(iofunc(ex, code), makeexpr_sizeof(makeexpr_type(basetype), 0)),
                         makeexpr_long(base));
}

Static Expr* func_fcall(void)
{
    Expr *ex, *ex2, *ex3;
    Type *type, *tp;
    ...
    mp = type->basetype->fbase;
    if(mp && mp->isreturn)
    { /* pointer to buffer for return value */
    ...
    tp->issigned = 1;
    ex3->args[0]->val.type = makepointertype(tp);
    return makeexpr_cond(makeexpr_rel(EK_NE, ex2, makeexpr_nil()), ex3, ex);
}

Static Expr* func_filepos(void)
{
    return file_iofunc(2, seek_base);
}

Static Expr* func_filesize(void)
{
    return file_iofunc(3, 0L);
}

Static Stmt* proc_fillchar(void)
{
    Expr *vex, *ex, *cex;
    ...
    skipcloseparen();
    return makestmt_call(makeexpr_bicall_3("memset", tp_void, vex, makeexpr_arglong(cex, 0),
                                           makeexpr_arglong(ex, (size_t_long != 0))));
}

Static Expr* func_sngl(void)
{
    Expr* ex;
...
Static Stmt* proc_flush(void)
{
    ...
    if(iocheck_flag)
        sp = makestmt_seq(sp, makestmt_assign(makeexpr_var(mp_ioresult), makeexpr_long(0)));
    return sp;
}
...
Static Expr* func_frac(Expr* ex)
{
    ...
    return makeexpr_bicall_2("modf", tp_longreal, grabarg(ex, 0), makeexpr_addr(makeexpr_var(tvar)));
}
...
Static Stmt* proc_freemem(Expr* ex)
{
    ...
    if(newdelete)
    {
        sp = makestmt_call(makeexpr_bin(EK_DELETE, tp_void, copyexpr(vex), makeexpr_name("", tp_integer)));
    }
    else
    {
        sp = makestmt_call(makeexpr_bicall_1(choose_free_func(vex), tp_void, copyexpr(vex)));
    }
    if(alloczeronil)
    {
        sp = makestmt_if(makeexpr_rel(EK_NE, vex, makeexpr_nil()), sp, NULL);
    }
    else
        freeexpr(vex);
    return sp;
...
Static Stmt* proc_get(void)
{
    ...
    type = ex->val.type;
    if(isfiletype(type, -1) && *chargetname && filebasetype(type)->kind == TK_CHAR)
        return makestmt_call(makeexpr_bicall_1(chargetname, tp_void, filebasename(ex)));
    else if(isfiletype(type, -1) && *arraygetname && filebasetype(type)->kind == TK_ARRAY)
        return makestmt_call(makeexpr_bicall_2(arraygetname, tp_void, filebasename(ex),
                                               makeexpr_type(filebasetype(type))));
    else
        return makestmt_call(
        makeexpr_bicall_2(getname, tp_void, filebasename(ex), makeexpr_type(filebasetype(type))));
}
...
Static Stmt* proc_getmem(Expr* ex)
{
    ...
    if(newdelete)
    {
        ex2 = makeexpr_cast(makeexpr_bin(EK_NEW, tp_anyptr, makeexpr_type(tp_abyte), ex2), type);
    }
    else
    ...
    if(malloccheck)
    {
        sp = makestmt_seq(sp, makestmt_if(makeexpr_rel(EK_EQ, copyexpr(vex), makeexpr_nil()),
                                          makestmt_call(makeexpr_bicall_0(name_OUTMEM, tp_int)), NULL));
    }
    if(sz && !isconstantexpr(sz))
    {
        if(alloczeronil == 2)
            note("Called GETMEM with variable argument [189]");
        sp = makestmt_if(makeexpr_rel(EK_NE, sz, makeexpr_long(0)), sp, makestmt_assign(vex, makeexpr_nil()));
    }
    else
        freeexpr(vex);
    return sp;
...
{
    return makestmt_call(makeexpr_bicall_2("gotoxy", tp_void, makeexpr_arglong(ex->args[0], 0),
                                           makeexpr_arglong(ex->args[1], 0)));
}

Static Stmt* proc_halt(void)
{
    if(which_lang == LANG_TIP)
    {
        return makestmt_call(makeexpr_bicall_1("exit", tp_void, makeexpr_name("EXIT_FAILURE", tp_integer)));
    }
    else
        return proc_escape();
}
...
Static Expr* handle_vax_hex(Expr* ex, char* fmt, int scale)
{
    ...
    if(lex->kind == EK_CONST && dex->kind == EK_CONST && lex->val.i < dex->val.i)
        lex = NULL;
    skipcloseparen();
    ...
    if(lex)
    {
        if(dex)
            ex = makeexpr_bicall_5("sprintf", tp_str255, vex,
                                   makeexpr_string(format_s("%%*.*%s", fmt)), lex, dex, ex);
        else
            ex = makeexpr_bicall_4("sprintf", tp_str255, vex, makeexpr_string(format_s("%%*%s", fmt)), lex, ex);
    }
    else
    {
        if(dex)
            ex = makeexpr_bicall_4("sprintf", tp_str255, vex,
                                   makeexpr_string(format_s("%%.*%s", fmt)), dex, ex);
        else
            ex = makeexpr_bicall_3("sprintf", tp_str255, vex, makeexpr_string(format_s("%%%s", fmt)), ex);
    }
    return ex;
}

Static Expr* func_hex(void)
{
    Expr* ex;
    char* cp;
    ...
    if((ex->val.type->kind == TK_STRING || ex->val.type == tp_strptr) && curtok != TOK_COMMA)
    {
        skipcloseparen();
        if(ex->kind == EK_CONST)
        { /* HP Pascal */
        ...
        {
            return makeexpr_bicall_3("strtol", tp_integer, ex, makeexpr_nil(), makeexpr_long(16));
        }
    }
    else
    { /* VAX Pascal */
...
Static Expr* func_hi(void)
{
    Expr* ex;

    ex = force_unsigned(p_parexpr(tp_integer));
    return makeexpr_bin(EK_RSH, tp_ubyte, ex, makeexpr_long(8));
}

Static Expr* func_high(void)
{
    Expr* ex;
    Type* type;
    ...
    if(type->kind == TK_POINTER)
        type = type->basetype;
    if(type->kind == TK_ARRAY || type->kind == TK_SMALLARRAY)
    {
        ex = makeexpr_minus(copyexpr(type->indextype->smax), copyexpr(type->indextype->smin));
    }
    else
    {
...
Static Expr* func_hiword(void)
{
    Expr* ex;

    ex = force_unsigned(p_parexpr(tp_unsigned));
    return makeexpr_bin(EK_RSH, tp_unsigned, ex, makeexpr_long(16));
}

Static Stmt* proc_inc(void)
{
    Expr *vex, *ex;
...
Static Stmt* proc_incl(void)
{
    ...
    if(vex->val.type->kind == TK_SMALLSET)
        return makestmt_assign(
        vex, makeexpr_bin(EK_BOR, vex->val.type, copyexpr(vex),
                          makeexpr_bin(EK_LSH, vex->val.type, makeexpr_longcast(makeexpr_long(1), 1), ex)));
    else
        return makestmt_call(makeexpr_bicall_2(setaddname, tp_void, vex, makeexpr_arglong(enum_to_int(ex), 0)));
}

Static Stmt* proc_insert(Expr* ex)
{
    return makestmt_call(makeexpr_bicall_3(strinsertname, tp_void, ex->args[0], ex->args[1],
                                           makeexpr_arglong(ex->args[2], 0)));
}

Static Expr* func_int(void)
{
    Expr* ex;
    Meaning* tvar;
    ...
    { /* Turbo Pascal INT */
        ...
        return makeexpr_comma(
        makeexpr_bicall_2("modf", tp_longreal, grabarg(ex, 0), makeexpr_addr(makeexpr_var(tvar))),
        makeexpr_var(tvar));
    }
    else
    { /* VAX Pascal INT */
...
Static Stmt* proc_leave(void)
{
    return makestmt(SK_BREAK);
}

Static Expr* func_lint(void)
{
    Expr* ex;
...
Static Expr* func_lo(void)
{
    Expr* ex;

    ex = gentle_cast(p_parexpr(tp_integer), tp_ushort);
    return makeexpr_bin(EK_BAND, tp_ubyte, ex, makeexpr_long(255));
}

Static Expr* func_loophole(void)
{
    Type* type;
    Expr* ex;
...
Static Expr* func_loword(void)
{
    Expr* ex;

    ex = p_parexpr(tp_integer);
    return makeexpr_bin(EK_BAND, tp_ushort, ex, makeexpr_long(65535));
}
...
Static Expr* func_max(void)
{
    ...
    while(curtok == TOK_COMMA)
    {
        ...
        ex = makeexpr_bicall_2((tp->kind == TK_REAL) ? "P_rmax" : "P_imax", tp, ex, ex2);
    }
    skipcloseparen();
    return ex;
}
...
Static Expr* func_min(void)
{
    ...
    while(curtok == TOK_COMMA)
    {
        ...
        ex = makeexpr_bicall_2((tp->kind == TK_REAL) ? "P_rmin" : "P_imin", tp, ex, ex2);
    }
    skipcloseparen();
    return ex;
}
...
Static Stmt* proc_move(Expr* ex)
{
    ...
    ex->args[2]
    = convert_size(choosetype(argbasetype(ex->args[0]), argbasetype(ex->args[1])), ex->args[2], "MOVE");
    return makestmt_call(makeexpr_bicall_3("memmove", tp_void, ex->args[1], ex->args[0],
                                           makeexpr_arglong(ex->args[2], (size_t_long != 0))));
}

Static Stmt* proc_move_fast(void)
{
    Expr *ex, *ex2, *ex3, *ex4;
    ...
    skipcloseparen();
    ex = convert_size(choosetype(argbasetype(ex2), argbasetype(ex3)), ex, "MOVE_FAST");
    return makestmt_call(makeexpr_bicall_3("memmove", tp_void, makeexpr_addr(ex3), makeexpr_addr(ex2),
                                           makeexpr_arglong(ex, (size_t_long != 0))));
}

Static Stmt* proc_new(void)
{
    Expr* ex;
    Stmt *sp, **spp;
    Type* type;
...
Static Expr* func_new_turbo(void)
{
    Meaning* tvar;
    Expr* ex;
...
Static Expr* handle_new(Expr* ex)
{
    ...
    {
        if(type->kind == TK_ARRAY || type->kind == TK_SET || type->kind == TK_STRING)
            ex2 = makeexpr_bin(EK_NEW, makepointertype(type), makeexpr_type(type->basetype),
                               arraysize(type, 1));
        else
            ex2 = makeexpr_un(EK_NEW, makepointertype(type), makeexpr_type(type));
    }
    else
    {
        ex2 = makeexpr_bicall_1(mallocname, tp_anyptr, makeexpr_sizeof(makeexpr_type(type), 1));
    }
    ex2 = makeexpr_assign(copyexpr(ex), ex2);
    if(malloccheck)
    {
        ex2 = makeexpr_comma(ex2, makeexpr_cond(makeexpr_rel(EK_EQ, copyexpr(ex), makeexpr_nil()),
                                                makeexpr_bicall_0(name_OUTMEM, tp_int), makeexpr_long(0)));
    }
    if(init)
...
Static Expr* func_octal(Expr* ex)
{
    ...
    {
        return makeexpr_bicall_3("strtol", tp_integer, ex, makeexpr_nil(), makeexpr_long(8));
    }
}
...
Static Stmt* proc_open(void)
{
    return handleopen(2);
}

Static Expr* func_ord(void)
{
    Expr* ex;
...
Static Stmt* proc_pack(void)
{
    ...
    exd = p_expr(NULL);
    skipcloseparen();
    if(exs->val.type->kind != TK_ARRAY || (exd->val.type->kind != TK_ARRAY && exd->val.type->kind != TK_SMALLARRAY))
    {
        warning("Bad argument types for PACK/UNPACK [325]");
        return makestmt_call(makeexpr_bicall_3("pack", tp_void, exs, exi, exd));
    }
    if(exs->val.type->smax || exd->val.type->smax)
    {
        ...
        sp->exp1 = makeexpr_assign(makeexpr_var(tvar), copyexpr(mind));
        sp->exp2 = makeexpr_rel(EK_LE, makeexpr_var(tvar), copyexpr(exd->val.type->indextype->smax));
        sp->exp3 = makeexpr_assign(makeexpr_var(tvar), makeexpr_plus(makeexpr_var(tvar), makeexpr_long(1)));
        exi = makeexpr_minus(exi, copyexpr(mind));
        sp->stm1 = makestmt_assign(p_index(exd, makeexpr_var(tvar)),
                                   p_index(exs, makeexpr_plus(makeexpr_var(tvar), exi)));
        return sp;
    }
    else
    {
        exi = gentle_cast(exi, exs->val.type->indextype);
        return makestmt_call(makeexpr_bicall_3("memcpy", exd->val.type, exd, makeexpr_addr(p_index(exs, exi)),
...
Static Expr* func_pad(Expr* ex)
{
    ...
    {
        return makeexpr_bicall_4("sprintf", tp_strptr, ex->args[0], makeexpr_string("%*s"),
                                 makeexpr_longcast(ex->args[3], 0), makeexpr_string(""));
    }
    return makeexpr_bicall_4(strpadname, tp_strptr, ex->args[0], ex->args[1], ex->args[2],
                             makeexpr_arglong(ex->args[3], 0));
}

Static Stmt* proc_page(void)
{
    Expr *fex, *ex;
    ...
    if(curtok == TOK_LPAR)
    {
        ...
        ex = makeexpr_bicall_2("fprintf", tp_int, filebasename(copyexpr(fex)), makeexpr_string("\f"));
    }
    else
    {
        fex = makeexpr_var(mp_output);
        ex = makeexpr_bicall_1("printf", tp_int, makeexpr_string("\f"));
    }
    if(FCheck(checkfilewrite))
    {
        ex = makeexpr_SETIO(makeexpr_rel(EK_GE, ex, makeexpr_long(0)), filewriteerrorname, fex);
    }
    return wrapopencheck(makestmt_call(ex), fex);
}
...
{
    freeexpr(ex);
    return makeexpr_minus(makeexpr_name(name_ARGC, tp_int), makeexpr_long(1));
}
...
Static Expr* func_paramstr(Expr* ex)
{
    ...
    ex2 = makeexpr_index(makeexpr_name(name_ARGV, makepointertype(tp_strptr)),
                         makeexpr_unlongcast(ex->args[1]), makeexpr_long(0));
    ex2->val.type = tp_str255;
    return makeexpr_bicall_3("sprintf", tp_strptr, ex->args[0], makeexpr_string("%s"), ex2);
}

Static Expr* func_pi(void)
{
    return makeexpr_name("M_PI", tp_longreal);
}

Static Expr* var_port(void)
{
    Expr* ex;
...
Static Expr* func_pos(Expr* ex)
{
    ...
    return makeexpr_bicall_3(cp, tp_int, ex->args[1], ex->args[0], makeexpr_long(1));
}
...
Static Expr* func_position(void)
{
    return file_iofunc(2, seek_base);
}

Static Expr* func_pred(void)
{
    Expr* ex;
...
Static Stmt* proc_put(void)
{
    ...
    requirefilebuffer(ex);
    type = ex->val.type;
    if(isfiletype(type, -1) && *charputname && filebasetype(type)->kind == TK_CHAR)
        return makestmt_call(makeexpr_bicall_1(charputname, tp_void, filebasename(ex)));
    else if(isfiletype(type, -1) && *arrayputname && filebasetype(type)->kind == TK_ARRAY)
        return makestmt_call(makeexpr_bicall_2(arrayputname, tp_void, filebasename(ex),
                                               makeexpr_type(filebasetype(type))));
    else
        return makestmt_call(
        makeexpr_bicall_2(putname, tp_void, filebasename(ex), makeexpr_type(filebasetype(type))));
}

Static Expr* func_pwroften(Expr* ex)
{
    return makeexpr_bicall_2("pow", tp_longreal, makeexpr_real("10.0"), grabarg(ex, 0));
}

Static Stmt* proc_reset(void)
{
    return handleopen(0);
}

Static Stmt* proc_rewrite(void)
{
    return handleopen(1);
}

Stmt* doseek(Expr* fex, Expr* ex)
{
    Expr* ex2;
    ...
    if(ansiC == 1)
        ...
        ex2 = makeexpr_long(0);
    ex = makeexpr_bicall_3("fseek", tp_int, filebasename(copyexpr(fex)),
                           makeexpr_arglong(makeexpr_times(makeexpr_minus(ex, makeexpr_long(seek_base)),
                                                           makeexpr_sizeof(makeexpr_type(basetype), 0)),
                                            1),
                           ex2);
    if(FCheck(checkfileseek))
    {
        ex = makeexpr_SETIO(makeexpr_rel(EK_EQ, ex, makeexpr_long(0)), endoffilename, fex);
    }
    return makestmt_call(ex);
}
...
Static Stmt* fixscanf(Stmt* sp, Expr* fex)
{
    ...
    if(sp->kind == SK_ASSIGN && sp->exp1->kind == EK_BICALL && !strcmp(sp->exp1->val.s, "scanf"))
    {
        if(sp->exp1->args[0]->kind == EK_CONST && !(sp->exp1->args[0]->val.i & 1) && !isstrread)
        {
            ...
            for(i = 0; cp[i] == '%' && cp[i + 1] == 'c';)
            {
                ...
                if(i == sp->exp1->args[0]->val.i)
                {
                    ...
                    for(i = 1; i < sp->exp1->nargs; i++)
                    {
                        ...
                        sp2 = makestmt_seq(sp2, makestmt_assign(copyexpr(ex), makegetchar(fex)));
                        if(checkeof(fex))
                        {
                            sp2 = makestmt_seq(sp2, makestmt_call(makeexpr_SETIO(
                                                    makeexpr_rel(EK_NE, ex, makeexpr_name("EOF", tp_char)),
                                                    endoffilename, fex)));
                        }
                        else
                            freeexpr(ex);
        ...
        if(FCheck(checkreadformat))
        {
            if(checkeof(fex) && !isstrread)
                ex = makeexpr_cond(
                makeexpr_rel(EK_NE, makeexpr_bicall_1("feof", tp_int, filebasename(copyexpr(fex))),
                             makeexpr_long(0)),
                makeexpr_name(endoffilename, tp_int), makeexpr_name(badinputformatname, tp_int));
            else
                ex = makeexpr_name(badinputformatname, tp_int);
            sp->exp1 = makeexpr_SETIO(makeexpr_rel(EK_EQ, sp->exp1, makeexpr_long(nargs)), NULL,
                                      makeexpr_bicall_2("Z", tp_int, ex, fex));
        }
        else if(checkeof(fex) && !isstrread)
        {
            sp->exp1 = makeexpr_SETIO(makeexpr_rel(EK_NE, sp->exp1, makeexpr_name("EOF", tp_int)),
                                      endoffilename, fex);
        }
    }
    return sp;
...
Static Expr* makefgets(Expr* vex, Expr* lex, Expr* fex)
{
    ...
    ex = makeexpr_bicall_3("fgets", tp_strptr, vex, lex, filebasename(copyexpr(fex)));
    if(checkeof(fex))
    {
        ex = makeexpr_SETIO(makeexpr_rel(EK_NE, ex, makeexpr_nil()), endoffilename, fex);
    }
    return ex;
}
...
Static Stmt* skipeoln(Expr* fex)
{
    ...
    {
        tvar = makestmttempvar(tp_str255, name_STRING);
        return makestmt_call(makefgets(makeexpr_var(tvar), makeexpr_long(stringceiling + 1), filebasename(fex)));
    }
    else if(!strcmp(readlnname, "scanf") || !*readlnname)
    {
        if(checkeof(fex))
            ex = makeexpr_SETIO(makeexpr_rel(EK_NE, makegetchar(fex), makeexpr_name("EOF", tp_char)),
                                endoffilename, fex);
        else
            ex = makegetchar(fex);
        return makestmt_seq(
        fixscanf(makestmt_call(makeexpr_bicall_1("scanf", tp_int, makeexpr_string("%*[^\n]"))), fex),
        makestmt_call(ex));
    }
    else
    {
        return makestmt_call(makeexpr_bicall_1(readlnname, tp_void, filebasename(copyexpr(fex))));
    }
}
...
Static Stmt* handleread_text(Expr* fex, Expr* var, int isreadln)
{
    ...
    for(;;)
    {
        ...
        if(var->val.type->kind == TK_STRING && !isstrread)
        {
            ...
            varstring = (varstrings && var->kind == EK_VAR
                         && (mp = (Meaning*)var->val.i)->kind == MK_VARPARAM && mp->type == tp_strptr);
            maxstring = (strmax(var) >= longstrsize && !varstring);
            if(isvar(fex, mp_input) && maxstring && usegets && readlnflag)
            {
                spbase = makestmt_seq(
                spbase, makestmt_call(makeexpr_bicall_1("gets", tp_str255, makeexpr_addr(var))));
                isreadln = 0;
            }
            else if(scanfmode && !varstring && (*readlnname || !isreadln))
            {
                spbase
                = makestmt_seq(spbase, makestmt_assign(makeexpr_hat(copyexpr(var), 0), makeexpr_char(0)));
            ...
            else
            {
                ...
                spbase
                = makestmt_seq(spbase, makestmt_call(makefgets(makeexpr_addr(copyexpr(var)), ex, fex)));
                if(!tempcp)
                    tempcp = makestmttempvar(tp_charptr, name_TEMP);
                spbase = makestmt_seq(
                spbase, makestmt_assign(makeexpr_var(tempcp),
                                        makeexpr_bicall_2("strchr", tp_charptr, makeexpr_addr(copyexpr(var)),
                                                          makeexpr_char('\n'))));
                sp = makestmt_assign(makeexpr_hat(makeexpr_var(tempcp), 0), makeexpr_long(0));
                if(readlnflag)
                    isreadln = 0;
                else
                    sp = makestmt_seq(sp, makestmt_call(makeexpr_bicall_2("ungetc", tp_void, makeexpr_char('\n'),
                                                                          filebasename(copyexpr(fex)))));
                spbase = makestmt_seq(
                spbase, makestmt_if(makeexpr_rel(EK_NE, makeexpr_var(tempcp), makeexpr_nil()), sp, NULL));
            }
            sp = NULL;
            spafter = NULL;
        }
        else if(var->val.type->kind == TK_ARRAY && !isstrread)
        {
            ...
            if(readlnflag)
            {
                spbase = makestmt_seq(spbase, makestmt_call(makeexpr_bicall_3(
                                              "P_readlnpaoc", tp_void, filebasename(copyexpr(fex)),
                                              makeexpr_addr(var), makeexpr_arglong(ex, 0))));
                isreadln = 0;
            }
            else
            {
                spbase = makestmt_seq(
                spbase, makestmt_call(makeexpr_bicall_3("P_readpaoc", tp_void, filebasename(copyexpr(fex)),
                                                        makeexpr_addr(var), makeexpr_arglong(ex, 0))));
            }
            sp = NULL;
            spafter = NULL;
        ...
        else
        {
            switch(ord_type(var->val.type)->kind)
            {
                ...
                case TK_INTEGER:
                    ...
                    if(curtok == TOK_IDENT && !strcicmp(curtokbuf, "HEX"))
                    {
                        fmt = "x";
                        gettok();
                    }
                    else if(curtok == TOK_IDENT && !strcicmp(curtokbuf, "OCT"))
                    {
                        fmt = "o";
                        gettok();
                    }
                    else if(curtok == TOK_IDENT && !strcicmp(curtokbuf, "BIN"))
                    {
                        fmt = "b";
                    ...
                    else if(type == tp_integer || type == tp_int || type == tp_uint || type == tp_sint)
                    ...
                    {
                        tvar = makestmttempvar(tp_int, name_TEMP);
                        spafter = makestmt_seq(spafter, makestmt_assign(var, makeexpr_var(tvar)));
                ...
                case TK_CHAR:
                    ...
                    {
                        spafter = makestmt_seq(
                        spafter, makestmt_if(makeexpr_rel(EK_EQ, copyexpr(var), makeexpr_char('\n')),
                                             makestmt_assign(copyexpr(var), makeexpr_char(' ')), NULL));
                    }
                    break;
                ...
                case TK_BOOLEAN:
                    ...
                    makestmt_assign(
                    var, makeexpr_or(
                         makeexpr_rel(EK_EQ, makeexpr_hat(makeexpr_var(tvar), 0), makeexpr_char('T')),
                         makeexpr_rel(EK_EQ, makeexpr_hat(makeexpr_var(tvar), 0), makeexpr_char('t')))));
                    var = makeexpr_var(tvar);
                ...
                case TK_REAL:
                    ...
                    if(var->val.type == tp_longreal && (strlen(floatscanfcode) == 2))
                    {
                        char buf[4];
                        strcpy(buf, ".l.");
    ...
    if(sp)
    {
        if(isstrread && !FCheck(checkreadformat)
           && ((i = 0, checkstring(sp->exp1->args[0], "%d")) || (i++, checkstring(sp->exp1->args[0], "%ld"))
               || (i++, checkstring(sp->exp1->args[0], "%hd")) || (i++, checkstring(sp->exp1->args[0], "%lg"))))
        {
            if(fullstrread != 0 && exj)
            {
                tvar = makestmttempvar(tp_strptr, name_STRING);
                sp->exp1 = makeexpr_assign(
                makeexpr_hat(sp->exp1->args[1], 0),
                (i == 3) ?
                makeexpr_bicall_2("strtod", tp_longreal, copyexpr(fex), makeexpr_addr(makeexpr_var(tvar))) :
                makeexpr_bicall_3("strtol", tp_integer, copyexpr(fex),
                                  makeexpr_addr(makeexpr_var(tvar)), makeexpr_long(10)));
                spafter = makestmt_seq(
                spafter, makestmt_assign(copyexpr(exj), makeexpr_minus(makeexpr_var(tvar),
            ...
            else
            {
                ...
                makeexpr_bicall_1((i == 1) ? "atol" : (i == 3) ? "atof" : "atoi",
                                  (i == 1) ? tp_integer : (i == 3) ? tp_longreal : tp_int, copyexpr(fex)));
            }
        }
        else if(isstrread && fullstrread != 0 && exj)
        {
            sp->exp1->args[0]
            = makeexpr_concat(sp->exp1->args[0], makeexpr_string(sizeof_int >= 32 ? "%n" : "%ln"), 0);
            insertarg(&sp->exp1, sp->exp1->nargs, makeexpr_addr(copyexpr(exj)));
        }
        else if(isreadln && scanfmode && !FCheck(checkreadformat))
        {
            isreadln = 0;
            sp->exp1->args[0] = makeexpr_concat(sp->exp1->args[0], makeexpr_string("%*[^\n]"), 0);
...
Static Stmt* handleread_bin(Expr* fex, Expr* var)
{
    ...
    for(;;)
    {
        ex = makeexpr_bicall_4("fread", tp_integer, makeexpr_addr(var),
                               makeexpr_sizeof(makeexpr_type(basetype), 0), makeexpr_long(1),
                               filebasename(copyexpr(fex)));
        if(checkeof(fex))
        {
            ex = makeexpr_SETIO(makeexpr_rel(EK_EQ, ex, makeexpr_long(1)), endoffilename, fex);
...
Static Stmt* proc_strread(void)
{
    ...
    if(fullstrread != 0 && ((ex = singlevar(exj)) == NULL || exproccurs(exi, ex)))
    {
    ...
    sp2 = handleread_text(bumpstring(copyexpr(vex), copyexpr(exi), 1), exjj, 0);
    sp = makestmt_seq(sp, sp2);
    skipcloseparen();
    if(fullstrread == 0)
    {
        sp = makestmt_seq(sp, makestmt_assign(exj, makeexpr_plus(makeexpr_bicall_1("strlen", tp_int, vex),
                                                                 makeexpr_long(1))));
        freeexpr(exjj);
        freeexpr(exi);
    }
    ...
    {
        sp = makestmt_seq(sp, makestmt_assign(exj, makeexpr_plus(exjj, exi)));
        if(fullstrread == 2)
...
Static Stmt* proc_randomize(void)
{
    if(*randomizename)
...
Static Expr* func_round(Expr* ex)
{
    ...
    {
        return makeexpr_actcast(
        makeexpr_bicall_1("floor", tp_longreal, makeexpr_plus(ex, makeexpr_real("0.5"))), tp_integer);
    }
}

Static Stmt* proc_unpack(void)
{
    ...
    exi = p_ord_expr();
    skipcloseparen();
    if(exd->val.type->kind != TK_ARRAY || (exs->val.type->kind != TK_ARRAY && exs->val.type->kind != TK_SMALLARRAY))
    {
        warning("Bad argument types for PACK/UNPACK [325]");
        return makestmt_call(makeexpr_bicall_3("unpack", tp_void, exs, exd, exi));
    }
    if(exs->val.type->smax || exd->val.type->smax)
    {
        ...
        sp->exp1 = makeexpr_assign(makeexpr_var(tvar), copyexpr(mins));
        sp->exp2 = makeexpr_rel(EK_LE, makeexpr_var(tvar), copyexpr(exs->val.type->indextype->smax));
        sp->exp3 = makeexpr_assign(makeexpr_var(tvar), makeexpr_plus(makeexpr_var(tvar), makeexpr_long(1)));
        exi = makeexpr_minus(exi, copyexpr(mins));
        sp->stm1 = makestmt_assign(p_index(exd, makeexpr_plus(makeexpr_var(tvar), exi)),
...
Static Expr* func_uround(Expr* ex)
{
    ...
    if(ex->val.type->kind != TK_REAL)
        return ex;
    return makeexpr_actcast(
    makeexpr_bicall_1("floor", tp_longreal, makeexpr_plus(ex, makeexpr_real("0.5"))), tp_unsigned);
}

Static Expr* func_scan(void)
{
    Expr *ex, *ex2, *ex3;
    char* name;
    ...
    skipcloseparen();
    return makeexpr_bicall_3(name, tp_int, makeexpr_arglong(ex, 0), makeexpr_charcast(ex2), ex3);
}

Static Expr* func_scaneq(Expr* ex)
{
    return makeexpr_bicall_3("P_scaneq", tp_int, makeexpr_arglong(ex->args[0], 0),
                             makeexpr_charcast(ex->args[1]), ex->args[2]);
}

Static Expr* func_scanne(Expr* ex)
{
    return makeexpr_bicall_3("P_scanne", tp_int, makeexpr_arglong(ex->args[0], 0),
                             makeexpr_charcast(ex->args[1]), ex->args[2]);
}

Static Stmt* proc_seek(void)
{
    Expr *fex, *ex;
    Stmt* sp;
    ...
    if(*setupbufname && fileisbuffered(fex, 1))
        sp = makestmt_seq(sp, makestmt_call(makeexpr_bicall_2(setupbufname, tp_void, filebasename(fex),
                                                              makeexpr_type(filebasetype(fex->val.type)))));
    else
        freeexpr(fex);
    return sp;
...
Static Expr* func_seekeoln(void)
{
    Expr* ex;
...
Static Stmt* proc_setstrlen(void)
{
    Expr *ex, *ex2;
    ...
    ex2 = p_expr(tp_integer);
    skipcloseparen();
    return makestmt_assign(makeexpr_bicall_1("strlen", tp_int, ex), ex2);
}

Static Stmt* proc_settextbuf(void)
{
    Expr *fex, *bex, *sex;
    ...
    return makestmt_call(makeexpr_bicall_4("setvbuf", tp_void, filebasename(fex), makeexpr_addr(bex),
                                           makeexpr_name("_IOFBF", tp_integer), sex));
}
...
Static Expr* func_sizeof(void)
{
    ...
    {
        ex = p_expr(NULL);
        if(ex->val.type->kind == TK_RECORD && ex->val.type->issigned && ex->kind != EK_VAR)
        {
            Type* tp = ex->val.type;
            while(tp)
            {
...
Static Stmt* proc_strappend(void)
{
    Expr *ex, *ex2;
...
Static Stmt* proc_strdelete(void)
{
...
Static Stmt* proc_strinsert(void)
{
    Meaning* tvari;
    Expr *exs, *exd, *exi;
    Stmt* sp;
    ...
    sp = makestmt_seq(sp, makestmt_assign(exd, makeexpr_concat(exs, copyexpr(exd), 0)));
    return sp;
}

Static Stmt* proc_strmove(void)
{
    ...
    return makestmt_call(makeexpr_bicall_5(strmovename, tp_str255, exlen, exs, exsi, exd, exdi));
}
...
Static Expr* func_strpos(Expr* ex)
{
    ...
    return makeexpr_bicall_3(cp, tp_int, ex->args[0], ex->args[1], makeexpr_long(1));
}

Static Expr* func_strrpt(Expr* ex)
{
    if(ex->args[1]->kind == EK_CONST && ex->args[1]->val.i == 1 && ex->args[1]->val.s[0] == ' ')
    {
        return makeexpr_bicall_4("sprintf", tp_strptr, ex->args[0], makeexpr_string("%*s"),
                                 makeexpr_longcast(ex->args[2], 0), makeexpr_string(""));
    }
    else
...
Static Expr* func_strrtrim(Expr* ex)
{
    return makeexpr_bicall_1(strrtrimname, tp_strptr,
                             makeexpr_assign(makeexpr_hat(ex->args[0], 0), ex->args[1]));
}

Static Expr* func_succ(void)
{
    Expr* ex;
...
Static Expr* func_typeof(void)
{
    ...
    if(curtok == TOK_IDENT && curtokmeaning && curtokmeaning->kind == MK_TYPE)
    {
        ex = makeexpr_bicall_1("typeOfType", tp_anyptr, makeexpr_type(curtokmeaning->type));
        gettok();
    }
    else
    {
...
Static Expr* func_udec(void)
{
    return handle_vax_hex(NULL, "u", 0);
}

Static Expr* func_unot(void)
{
    Expr* ex;
...
Static Expr* func_val_modula(void)
{
    Expr* ex;
    Type* tp;
...
Static Stmt* proc_val_turbo(void)
{
    ...
    ex = makeexpr_bicall_3("sscanf", tp_int, ex, fmt, makeexpr_addr(vex));
    if(code)
    {
...
Static Expr* writestrelement(Expr* ex, Expr* wid, Expr* vex, int code, int needboth)
{
    if(formatstrings && needboth)
    {
        return makeexpr_bicall_5("sprintf", tp_str255, vex,
                                 makeexpr_string(format_d("%%*.*%c", code)), copyexpr(wid), wid, ex);
    }
    else
    {
        return makeexpr_bicall_4("sprintf", tp_str255, vex, makeexpr_string(format_d("%%*%c", code)), wid, ex);
    }
}
...
Expr* writeelement(Expr* ex, Expr* wid, Expr* prec, int base)
{
    Expr *vex, *ex1, *ex2;
    Meaning* tvar;
    char* fmtcode;
    ...
    if(ex->val.type->kind == TK_POINTER && ex->val.type->basetype && ex->val.type->basetype->kind == TK_STRING)
    {
    ...
    switch(ord_type(ex->val.type)->kind)
    {
        ...
        case TK_INTEGER:
            ...
            else if(type == tp_unsigned || type == tp_uint || (type == tp_ushort && sizeof_int < 32))
                fmtcode = "u";
            else
                fmtcode = "d";
            ex = makeexpr_forcelongness(ex);
            ...
            {
                ex = makeexpr_bicall_3(
                "sprintf", tp_str255, vex,
                makeexpr_string(format_ss("%%%s%s", (exprlongness(ex) > 0) ? "l" : "", fmtcode)), ex);
            }
            else
            {
                ex = makeexpr_bicall_4(
                "sprintf", tp_str255, vex,
                makeexpr_string(format_ss("%%*%s%s", (exprlongness(ex) > 0) ? "l" : "", fmtcode)), wid, ex);
            }
            break;

        case TK_CHAR:
            ex = writestrelement(ex, wid, vex, 'c', (wid->kind != EK_CONST || wid->val.i < 1));
            break;

        case TK_BOOLEAN:
            if(!wid)
            {
                ex = makeexpr_bicall_3("sprintf", tp_str255, vex, makeexpr_string("%s"),
                                       makeexpr_cond(ex, makeexpr_string(" TRUE"), makeexpr_string("FALSE")));
            }
            else if(checkconst(wid, 1))
            {
                ex = makeexpr_bicall_3("sprintf", tp_str255, vex, makeexpr_string("%c"),
                                       makeexpr_cond(ex, makeexpr_char('T'), makeexpr_char('F')));
            }
            else
            {
                ex = writestrelement(makeexpr_cond(ex, makeexpr_string("TRUE"), makeexpr_string("FALSE")),
                                     wid, vex, 's', (wid->kind != EK_CONST || wid->val.i < 5));
            }
            break;

        case TK_ENUM:
            ex = makeexpr_bicall_3(
            "sprintf", tp_str255, vex, makeexpr_string("%s"),
            makeexpr_index(makeexpr_name(makeenumnames(ex->val.type), tp_strptr), ex, NULL));
            break;
        ...
        case TK_REAL:
            ...
            {
                ex = makeexpr_bicall_5("sprintf", tp_str255, vex, makeexpr_string("%*.*f"), wid, prec, ex);
            }
            else
            {
                if(prec)
                    ...
                    prec = makeexpr_minus(copyexpr(wid), makeexpr_long(7));
                ...
                {
                    prec = makeexpr_bicall_2("P_max", tp_integer, prec, makeexpr_long(1));
                }
                if(wid->kind == EK_CONST && wid->val.i > 21)
                {
                    ex = makeexpr_bicall_5("sprintf", tp_str255, vex, makeexpr_string("%*.*E"), wid, prec, ex);
#if 0
                ...
                {
                    ex = makeexpr_bicall_4("sprintf", tp_str255, vex, makeexpr_string("% .*E"), prec, ex);
                }
            }
            break;
        ...
        case TK_ARRAY: /* assume packed array of char */
            ...
            ex1 = makeexpr_plus(makeexpr_minus(copyexpr(ex2), copyexpr(ex1)), makeexpr_long(1));
            ...
            {
                if(isliteralconst(wid, NULL) == 2 && isliteralconst(ex1, NULL) == 2)
                {
                    if(wid->val.i > ex1->val.i)
                    {
                        fmtcode = format_ds("%*s%%.*s", wid->val.i - ex1->val.i, "");
                        wid = ex1;
                    }
                }
                else
            ...
            ex = makeexpr_bicall_4("sprintf", tp_str255, vex, makeexpr_string(fmtcode), wid, makeexpr_addr(ex));
            break;

        case TK_POINTER:
            ex = makeexpr_bicall_3("sprintf", tp_str255, vex,
                                   makeexpr_string(format_s("0x%%.8%sx", (sizeof_int < 32) ? "l" : "")),
...
Static Stmt* handlewrite_text(Expr* fex, Expr* ex, int iswriteln)
{
    ...
    for(;;)
    {
        ...
        if(curtok == TOK_IDENT && !strcicmp(curtokbuf, "OCT"))
        {
            base = 8;
            gettok();
        }
        else if(curtok == TOK_IDENT && !strcicmp(curtokbuf, "HEX"))
        {
            base = 16;
            gettok();
        }
    ...
    if(fex->val.type->kind != TK_STRING)
    { /* not strwrite */
        switch(iswriteln)
        {
            case 1:
        ...
        if((i = sprintflength(print, 0)) > 0 && print->nargs == 2 && printfonly != 1)
        {
            ...
            if(isvar(fex, mp_output))
            {
                if(i == 1)
                {
                    print = makeexpr_bicall_1("putchar", tp_int, makeexpr_charcast(print));
                }
                else
                {
                    if(printfonly == 0)
                    {
                        ...
                        {
                            print = makeexpr_bicall_2("fputs", tp_int, print, copyexpr(fex));
                        }
                    }
                    else
                    {
            ...
            else
            {
                if(i == 1)
                {
                    print = makeexpr_bicall_2("putc", tp_int, makeexpr_charcast(print),
                                              filebasename(copyexpr(fex)));
                }
                else if(printfonly == 0)
                {
                    print = makeexpr_bicall_2("fputs", tp_int, print, filebasename(copyexpr(fex)));
                }
                else
                {
                    print = makeexpr_sprintfify(print);
        ...
        if(!done)
        {
            ...
            {
                print = makeexpr_bicall_2("fputs", tp_int, grabarg(print, 2), filebasename(copyexpr(fex)));
            }
            else if(checkstring(print->args[1], "%c") && printfonly != 1 && !nosideeffects(print->args[2], 0))
            {
                print = makeexpr_bicall_2("fputc", tp_int, grabarg(print, 2), filebasename(copyexpr(fex)));
            }
            ...
            else
            {
                ...
                {
                    print = makeexpr_bicall_2("putc", tp_int, grabarg(print, 2), filebasename(copyexpr(fex)));
                }
                else
                {
        ...
        if(FCheck(checkfilewrite))
        {
            print = makeexpr_SETIO(makeexpr_rel(EK_GE, print, makeexpr_long(0)), filewriteerrorname, fex);
        }
    }
    ...
    for(;;)
    {
        if(!expr_has_address(ex) || ex->val.type != basetype)
        {
            ...
            if(!tvardef || !exprsame(tvardef, ex, 1))
            {
                freeexpr(tvardef);
                tvardef = copyexpr(ex);
                sp = makestmt_seq(sp, makestmt_assign(makeexpr_var(tvar), ex));
            }
            else
                freeexpr(ex);
            ex = makeexpr_var(tvar);
...
Static Stmt* handle_modula_write(char* fmt)
{
    ...
    if(checkconst(wid, 0) || checkconst(wid, 1))
        ex = makeexpr_bicall_2("printf", tp_str255,
                               makeexpr_string(format_ss("%%%s%s", (exprlongness(ex) > 0) ? "l" : "", fmt)), ex);
    else
        ex = makeexpr_bicall_3("printf", tp_str255,
                               makeexpr_string(format_ss("%%*%s%s", (exprlongness(ex) > 0) ? "l" : "", fmt)),
                               makeexpr_arglong(wid, 0), ex);
    skipcloseparen();
    return makestmt_call(ex);
}

Static Stmt* proc_writecard(void)
{
    return handle_modula_write("u");
}

Static Stmt* proc_writeint(void)
{
    return handle_modula_write("d");
}

Static Stmt* proc_writehex(void)
{
    return handle_modula_write("x");
}

Static Stmt* proc_writeoct(void)
{
    return handle_modula_write("o");
}

Static Stmt* proc_writereal(void)
{
    return handle_modula_write("f");
}

Static Stmt* proc_writedir(void)
{
    Expr *fex, *ex;
    Stmt* sp;
...
Static Stmt* handlewriteln(int iswriteln)
{
    ...
    if(iswriteln == 0)
    {
        sp = makestmt_seq(sp, makestmt_call(makeexpr_bicall_1("fflush", tp_void, filebasename(copyexpr(fex)))));
    }
    return wrapopencheck(sp, fex);
}

Static Stmt* proc_overprint(void)
{
    return handlewriteln(2);
}

Static Stmt* proc_prompt(void)
{
    return handlewriteln(0);
}

Static Stmt* proc_writeln(void)
{
    return handlewriteln(1);
}

Static Stmt* proc_message(void)
{
    return handlewriteln(3);
}

Static Stmt* proc_writev(void)
{
    Expr *vex, *ex;
    Stmt* sp;
    Meaning* mp;
    ...
    skipcloseparen();
    ex = sp->exp1;
    if(ex->kind == EK_BICALL && !strcmp(ex->val.s, "sprintf") && (mp = istempvar(ex->args[0])) != NULL)
    {
        canceltempvar(mp);
        ex->args[0] = vex;
    }
...
Static Stmt* proc_strwrite(Meaning* mp_x, Stmt* spbase)
{
    ...
    if(checkconst(exi, 1))
    {
        ...
        if(sp && sp->kind == SK_ASSIGN && sp->exp1->kind == EK_ASSIGN
           && (sp->exp1->args[0]->kind == EK_HAT || sp->exp1->args[0]->kind == EK_INDEX)
           && exprsame(sp->exp1->args[0]->args[0], vex, 1) && checkconst(sp->exp1->args[1], 0))
        {
...
Static Stmt* proc_str_turbo(void)
{
    Expr *ex, *wid, *prec;
...
Static Expr* func_xor(void)
{
    ...
    ex2 = p_expr(ex->val.type);
    skipcloseparen();
    if(ex->val.type->kind != TK_SET && ex->val.type->kind != TK_SMALLSET)
    {
    ...
    else
    {
        ...
        ex = makeexpr_bicall_3(setxorname, type, makeexpr_var(tvar), ex, ex2);
    }
    return ex;
}

void decl_builtins(void)
{
