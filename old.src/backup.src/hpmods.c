...
Static Stmt* proc_freadbytes(void)
{
    Expr *ex, *ex2, *vex, *fex;
    Type* type;
    ...
    skipcloseparen();
    type = vex->val.type;
    ex = makeexpr_bicall_4("fread", tp_integer, makeexpr_addr(vex), convert_size(type, ex2, "FREADBYTES"),
                           makeexpr_long(1), filebasename(copyexpr(fex)));
    if(checkeof(fex))
    {
        ex = makeexpr_bicall_2(name_SETIO, tp_void, makeexpr_rel(EK_EQ, ex, makeexpr_long(1)),
                               makeexpr_long(30));
    }
    return wrapopencheck(makestmt_call(ex), fex);
}

Static Stmt* proc_fwritebytes(void)
{
    Expr *ex, *ex2, *vex, *fex;
    Type* type;
    ...
    skipcloseparen();
    type = vex->val.type;
    ex = makeexpr_bicall_4("fwrite", tp_integer, makeexpr_addr(vex), convert_size(type, ex2, "FWRITEBYTES"),
                           makeexpr_long(1), filebasename(copyexpr(fex)));
    if(checkfilewrite)
    {
        ex = makeexpr_bicall_2(name_SETIO, tp_void, makeexpr_rel(EK_EQ, ex, makeexpr_long(1)),
...
Static void setup_sysglobals(void)
{
    Symbol* sym;
...
void hpmods(char* name, int defn)
{
    if(!strcmp(name, "FS"))
    {
