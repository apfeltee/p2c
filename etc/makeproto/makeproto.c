...
{
    char *bad, *good;
} warntypes[] = { { "char", "int" }, { "signed char", "int" }, { "unsigned char", "int" }, { "short", "int" }, { "signed short", "int" }, { "unsigned short", "int" }, { "boolean", "int" }, { "Boolean", "int" }, { "float", "double" }, { NULL, NULL } };

int readline(buf, inf) char* buf;
FILE* inf;
{
    char *cp, *cp2;
    int spflag;
...
int strbeginsword(s1, s2) register char *s1, *s2;
{
    while(*s2 && *s1 == *s2)
        s1++, s2++;
        ...
        while(readline(buf, inf))
        {
            ...
            else
            {
                while(cp > buf && isspace(cp[-1]))
                    cp--, cp2--;
                while(cp > buf && isidchar(cp[-1]))
                    cp--, cp2--;
            }
            while(cp2 > ftype && isspace(cp2[-1]))
                cp2--;
            *cp2 = 0;
            if(!*ftype)
                strcpy(ftype, "int");
            ...
            else
            {
                ...
                if(useextern && !strbeginsword(ftype, "extern") && !strbeginsword(ftype, "Extern"))
                {
            ...
            *cp2 = 0;
            ...
            for(;;)
            {
                ...
                if(isidchar(*cp))
                {
                    ...
                    if(*cp == ',' || *cp == ';' || *cp == '[')
                    {
                        while(cp2 > dtype && isspace(cp2[-1]))
                            cp--, cp2--;
                        while(cp2 > dtype && isidchar(cp2[-1]))
                            cp--, cp2--;
                    }
                    else if(*cp != '(' && *cp != '*')
            ...
            for(i = 0; i < nargs; i++)
            {
                ...
                for(j = 0; warntypes[j].bad && !strbeginsword(argdecls[i], warntypes[j].bad); j++)
                    ;
                if(warntypes[j].bad)
                {
                    cp = argdecls[i];
                    ...
                    if(!*cp)
                    { /* not, e.g., "char *" */
                        sprintf(temp, "%s%s", warntypes[j].good, argdecls[i] + strlen(warntypes[j].bad));
